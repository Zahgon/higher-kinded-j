...
public class HKJLifecycleParticipant extends AbstractMavenLifecycleParticipant {
    ...
    private void addDependency(MavenProject project, String artifactId, String version, String scope) {
    ...
    private void applyTestOverrides(Xpp3Dom configNode, HKJConfiguration config) {
        ...
        addHkjProcessorPaths(configNode, "testAnnotationProcessorPaths", config, /* create= */
        false);
        if (config.pathTypeMismatch()) {
            addHkjCompilerArg(configNode, "testCompilerArgs", /* create= */
            false);
        }
    }

    private void addHkjProcessorPaths(Xpp3Dom configNode, String childName, HKJConfiguration config, boolean create) {
        Xpp3Dom paths = create ? getOrCreateChild(configNode, childName) : configNode.getChild(childName);
        if (paths == null) {
            return;
        }
    ...
    private void addHkjCompilerArg(Xpp3Dom configNode, String childName, boolean create) {
        Xpp3Dom args = create ? getOrCreateChild(configNode, childName) : configNode.getChild(childName);
        if (args == null) {
            return;
        }
