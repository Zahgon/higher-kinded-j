...
public class HKJPlugin implements Plugin<Project> {
    ...
    private void configure(Project project, HKJExtension extension) {
        ...
        sourceSets.all(sourceSet -> deps.add(sourceSet.getAnnotationProcessorConfigurationName(), GROUP_ID + ":hkj-processor-plugins:" + version));
        // Preview features
        ...
        if (Boolean.TRUE.equals(extension.getChecks().getPathTypeMismatch().get())) {
            sourceSets.all(sourceSet -> deps.add(sourceSet.getAnnotationProcessorConfigurationName(), GROUP_ID + ":hkj-checker:" + version));
            project.getTasks().withType(JavaCompile.class).configureEach(task -> {
                ...
                task.getOptions().getForkOptions().getJvmArgs().addAll(List.of("--add-exports", "jdk.compiler/com.sun.tools.javac.api=ALL-UNNAMED", "--add-exports", "jdk.compiler/com.sun.tools.javac.code=ALL-UNNAMED", "--add-exports", "jdk.compiler/com.sun.tools.javac.tree=ALL-UNNAMED", "--add-exports", "jdk.compiler/com.sun.tools.javac.util=ALL-UNNAMED"));
            });
        }
        // Spring integration
    ...
    private static String readPluginVersion() {
        ...
        throw new IllegalStateException("Could not determine HKJ plugin version. " + "Please set hkj.version explicitly in your build script.");
    }
    ...
    private void registerInstallSkillsTask(Project project, HKJExtension extension) {
        ...
        project.afterEvaluate(p -> {
            if (Boolean.TRUE.equals(extension.getSkills().get())) {
                p.getTasks().named("classes").configure(classesTask -> classesTask.dependsOn(installTask));
            }
        });
    }

    private void registerDiagnosticsTask(Project project, HKJExtension extension) {
        project.getTasks().register("hkjDiagnostics", task -> {
            task.setGroup("help");
            ...
            task.doLast(t -> {
                ...
                for (var sourceSet : sourceSets) {
                    labelWidth = Math.max(labelWidth, sourceSet.getAnnotationProcessorConfigurationName().length() + 1);
                }
                final int width = labelWidth;
                List<String> depsAdded = new ArrayList<>();
                depsAdded.add(padLabel("implementation:", width) + GROUP_ID + ":hkj-core:" + version);
                sourceSets.forEach(sourceSet -> {
                    String label = sourceSet.getAnnotationProcessorConfigurationName() + ":";
                    depsAdded.add(padLabel(label, width) + GROUP_ID + ":hkj-processor-plugins:" + version);
                });
                if (checks) {
                    for (var sourceSet : sourceSets) {
                        String label = sourceSet.getAnnotationProcessorConfigurationName() + ":";
                        depsAdded.add(padLabel(label, width) + GROUP_ID + ":hkj-checker:" + version);
                    }
                }
                if (spring) {
                    depsAdded.add(padLabel("implementation:", width) + GROUP_ID + ":hkj-spring-boot-starter:" + version);
                }
                ...
                sb.append("  Preview features:   ").append(preview ? "enabled" : "disabled").append("\n");
                sb.append("  Spring integration: ").append(spring ? "enabled" : "disabled").append("\n");
                sb.append("  Claude Code skills: ").append(skills ? "enabled" : "disabled").append("\n");
                sb.append("  Compile-time checks:\n");
                sb.append("    Path type mismatch: ").append(checks ? "enabled" : "disabled").append("\n");
