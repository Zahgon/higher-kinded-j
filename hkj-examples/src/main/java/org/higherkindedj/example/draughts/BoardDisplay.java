...
public class BoardDisplay {
    ...
    private static String renderGameState(GameState state) {
        return String.join("\n", "", BOARD_HEADER, BOARD_BORDER, renderBoard(state), BOARD_BORDER, BOARD_HEADER, "", state.message(), renderCurrentPlayer(state));
    }
    ...
    private static String renderBoard(GameState state) {
        return IntStream.iterate(7, row -> row >= 0, row -> row - 1).mapToObj(row -> renderRow(state, row)).collect(Collectors.joining("\n"));
    }
    ...
    private static String renderRow(GameState state, int row) {
        String squares = IntStream.range(0, 8).mapToObj(col -> renderSquare(state, row, col)).collect(Collectors.joining(" "));
    ...
    private static String renderSquare(GameState state, int row, int col) {
        return AffinePath.of(FocusPaths.<Square, Piece>mapAt(new Square(row, col))).toMaybePath(state.board()).map(BoardDisplay::pieceToChar).getOrElse(".");
    }
    ...
    private static String pieceToChar(Piece piece) {
        ...
        char display = PieceFocus.type().get(piece) == PieceType.KING ? Character.toUpperCase(base) : base;
