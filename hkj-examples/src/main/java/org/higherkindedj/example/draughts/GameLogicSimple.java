...
public class GameLogicSimple {
    ...
    private static EitherPath<String, StateTuple<GameState, MoveResult>> validateAndApply(GameState state, MoveCommand command, Piece piece, Square from, Square to) {
        ...
        if (Math.abs(rowDiff) == 1 && Math.abs(colDiff) == 1) {
            return validateSimpleMove(piece, rowDiff).map(validPiece -> performMove(state, command, validPiece));
    ...
    private static EitherPath<String, StateTuple<GameState, MoveResult>> validateJumpMove(GameState state, MoveCommand command, Piece piece, Square from, int rowDiff, int colDiff) {
        ...
        return getPieceAt(jumpedSquare, state).mapError(err -> "Invalid jump. Must jump over an opponent's piece.").via(jumpedPiece -> {
            Player jumpedOwner = PieceFocus.owner().get(jumpedPiece);
            return jumpedOwner != currentPlayer ? Path.right(jumpedPiece) : Path.<String, Piece>left("Cannot jump over your own piece.");
    ...
    private static StateTuple<GameState, MoveResult> performJump(GameState state, MoveCommand command, Piece piece, Square jumpedSquare) {
        ...
        return checkWinCondition(finalState).map(winner -> new StateTuple<>(new MoveResult(MoveOutcome.GAME_WON, winner + " wins!"), GameStateFocus.isGameOver().set(true, GameStateFocus.message().set(winner + " has captured all pieces!", finalState)))).getOrElse(new StateTuple<>(new MoveResult(MoveOutcome.CAPTURE_MADE, "Capture successful."), finalState.togglePlayer()));
    }
