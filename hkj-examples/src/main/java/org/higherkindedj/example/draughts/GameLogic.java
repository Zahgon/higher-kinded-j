...
public class GameLogic {
    ...
    private static EitherPath<String, Piece> getPieceAt(Square square, GameState state) {
        return AffinePath.of(FocusPaths.<Square, Piece>mapAt(square)).toEitherPath(state.board(), "No piece at " + square);
    }
    ...
    private static EitherPath<String, Unit> validateDestinationEmpty(Square to, GameState state) {
        boolean isEmpty = AffinePath.of(FocusPaths.<Square, Piece>mapAt(to)).getOptional(state.board()).isEmpty();
        return isEmpty ? Path.right(Unit.INSTANCE) : Path.left("Destination square " + to + " is occupied.");
    }

    /**
     * Validates the move type and applies it if valid.
     */
    private static EitherPath<String, StateTuple<GameState, MoveResult>> validateAndApply(GameState state, MoveCommand command, Piece piece, Square from, Square to) {
    ...
    private static EitherPath<String, Piece> validateSimpleMove(Piece piece, int rowDiff) {
        ...
        if (type == PieceType.MAN) {
            boolean isBackward = (owner == Player.RED && rowDiff > 0) || (owner == Player.BLACK && rowDiff < 0);
            if (isBackward) {
     ...
     */
    private static EitherPath<String, StateTuple<GameState, MoveResult>> validateJumpMove(GameState state, MoveCommand command, Piece piece, Square from, int rowDiff, int colDiff) {
        Square jumpedSquare = new Square(from.row() + rowDiff / 2, from.col() + colDiff / 2);
        Player currentPlayer = GameStateFocus.currentPlayer().get(state);
        return validateJumpDirection(piece, rowDiff).via(p -> getPieceAt(jumpedSquare, state)).mapError(err -> "Invalid jump. Must jump over an opponent's piece.").via(jumpedPiece -> PieceFocus.owner().get(jumpedPiece) != currentPlayer ? Path.right(jumpedPiece) : Path.<String, Piece>left("Cannot jump over your own piece.")).map(jumpedPiece -> performJump(state, command, piece, jumpedSquare));
    }
    ...
    private static StateTuple<GameState, MoveResult> invalidMove(String message, GameState state) {
        return new StateTuple<>(new MoveResult(MoveOutcome.INVALID_MOVE, message), GameStateFocus.message().set(message, state));
    }
     ...
     */
    private static StateTuple<GameState, MoveResult> performMove(GameState state, MoveCommand command, Piece piece) {
        ...
        return new StateTuple<>(new MoveResult(MoveOutcome.SUCCESS, "Move successful."), finalState.togglePlayer());
    }
     ...
     * approach.
     */
    private static StateTuple<GameState, MoveResult> performJump(GameState state, MoveCommand command, Piece piece, Square jumpedSquare) {
        Map<Square, Piece> newBoard = new HashMap<>(state.board());
        newBoard.remove(command.from());
        ...
        return checkWinCondition(stateAfterKinging).map(winner -> createWinResult(winner, stateAfterKinging)).getOrElseGet(() -> checkMultiJumpOrEndTurn(stateAfterKinging, command.to()));
    }
    ...
    private static StateTuple<GameState, MoveResult> createWinResult(String winner, GameState state) {
        GameState wonState = GameStateFocus.isGameOver().set(true, GameStateFocus.message().set(winner + " has captured all pieces!", state));
     ...
     */
    private static StateTuple<GameState, MoveResult> checkMultiJumpOrEndTurn(GameState state, Square position) {
        return canPieceJump(state, position) ? new StateTuple<>(new MoveResult(MoveOutcome.CAPTURE_MADE, "Capture successful. You must jump again with the same piece."), GameStateFocus.message().set("Capture successful. You must jump again with the same piece.", state)) : new StateTuple<>(new MoveResult(MoveOutcome.CAPTURE_MADE, "Capture successful."), state.togglePlayer());
    }
    ...
    private static boolean canPieceJump(GameState state, Square from) {
        return AffinePath.of(FocusPaths.<Square, Piece>mapAt(from)).toMaybePath(state.board()).map(piece -> hasAnyValidJump(state, from, piece)).getOrElse(false);
    }
    ...
    private static boolean hasAnyValidJump(GameState state, Square from, Piece piece) {
        Player owner = PieceFocus.owner().get(piece);
        PieceType type = PieceFocus.type().get(piece);
        return generateJumpDirections().filter(dir -> isValidJumpDirection(type, owner, dir.rowOffset())).anyMatch(dir -> isValidJump(state, from, dir, owner));
    }
    ...
    private static Stream<JumpDirection> generateJumpDirections() {
        return IntStream.of(JUMP_OFFSETS).boxed().flatMap(row -> IntStream.of(JUMP_OFFSETS).mapToObj(col -> new JumpDirection(row, col)));
    }
    ...
    private static boolean isValidJump(GameState state, Square from, JumpDirection dir, Player owner) {
        ...
        boolean destEmpty = AffinePath.of(FocusPaths.<Square, Piece>mapAt(to)).getOptional(state.board()).isEmpty();
        // Must have opponent piece to jump
        boolean hasOpponentPiece = AffinePath.of(FocusPaths.<Square, Piece>mapAt(jumpedSquare)).toMaybePath(state.board()).filter(p -> PieceFocus.owner().get(p) != owner).run().isJust();
    ...
    private static MaybePath<String> checkWinCondition(GameState state) {
        ...
        boolean noRedPieces = board.values().stream().noneMatch(p -> PieceFocus.owner().get(p) == Player.RED);
        boolean noBlackPieces = board.values().stream().noneMatch(p -> PieceFocus.owner().get(p) == Player.BLACK);
        if (noRedPieces) {
    ...
    private static GameState checkAndKingPiece(GameState state, Square to) {
        return AffinePath.of(FocusPaths.<Square, Piece>mapAt(to)).toMaybePath(state.board()).filter(piece -> PieceFocus.type().get(piece) == PieceType.MAN).filter(piece -> shouldBeKinged(piece, to)).map(piece -> kingPiece(state, to, piece)).getOrElse(state);
    }
    ...
    private static GameState kingPiece(GameState state, Square to, Piece piece) {
        ...
        return GameStateFocus.message().set(owner + "'s piece at " + to + " has been kinged!", GameStateFocus.board().set(newBoard, state));
    }
}
