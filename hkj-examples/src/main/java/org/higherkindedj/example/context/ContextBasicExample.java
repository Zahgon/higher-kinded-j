...
public class ContextBasicExample {
    ...
    private static void asksExample() throws Exception {
        ...
        Context<Integer, String> getRetryMessage = Context.asks(MAX_RETRIES, retries -> "Will retry up to " + retries + " times");
    ...
    private static void mapAndFlatMapExample() throws Exception {
        ...
        Context<String, String> formatName = Context.ask(USER_NAME).flatMap(name -> Context.succeed("[" + name.toUpperCase() + "]"));
        // Chaining multiple operations
        Context<String, String> elaborate = Context.ask(USER_NAME).map(String::trim).map(String::toUpperCase).flatMap(upper -> Context.succeed("*** " + upper + " ***"));
        String result = ScopedValue.where(USER_NAME, "  charlie  ").call(() -> {
    ...
    private static void multipleValuesExample() throws Exception {
        ...
        String result = ScopedValue.where(USER_NAME, "Diana").where(MAX_RETRIES, 5).where(DEBUG_MODE, true).call(() -> {
            String name = getName.run();
            int retries = getRetries.run();
            boolean debug = isDebug.run();
            return String.format("Config: user=%s, retries=%d, debug=%s", name, retries, debug);
        });
    ...
    private static void errorHandlingExample() throws Exception {
        ...
        // Context can fail
        Context<String, String> mightFail = Context.ask(USER_NAME).flatMap(name -> {
            if (name.isEmpty()) {
        ...
        Context<String, String> withRecovery = mightFail.recover(error -> "Recovered from: " + error.getMessage());
        // mapError: Transform the error type
        Context<String, String> withMappedError = mightFail.mapError(error -> new RuntimeException("Wrapped: " + error.getMessage(), error));
        // recoverWith: Recover with another context computation
        Context<String, String> withFallback = mightFail.recoverWith(error -> Context.succeed("Fallback value"));
    ...
    private static void typeClassExample() throws Exception {
        ...
        Function<Integer, Kind<ContextKind.Witness<String>, String>> f = n -> ContextKindHelper.CONTEXT.widen(Context.succeed("Value is " + n));
