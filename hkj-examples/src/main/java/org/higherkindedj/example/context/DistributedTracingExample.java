...
public class DistributedTracingExample {
    ...
    private static void simulateIncomingRequest() throws Exception {
        ...
        ScopedValue.where(RequestContext.TRACE_ID, traceId).where(RequestContext.CORRELATION_ID, correlationId).where(RequestContext.TENANT_ID, tenantId).where(RequestContext.REQUEST_TIME, requestTime).where(RequestContext.DEADLINE, deadline).where(SecurityContext.PRINCIPAL, user).where(SecurityContext.ROLES, roles).where(CURRENT_SPAN_ID, spanId).where(PARENT_SPAN_ID, "root").run(() -> {
            // Record root span
            recordSpan("api-gateway", "GET /api/orders/123", () -> {
    ...
    private static VTask<OrderDetails> callOrderService(String orderId) {
        return VTask.of(() -> {
            ...
            return ScopedValue.where(CURRENT_SPAN_ID, newSpanId).where(PARENT_SPAN_ID, parentSpan).call(() -> {
                recordSpan("order-service", "getOrder(" + orderId + ")", () -> {
                ...
                VTask<List<ProductInfo>> productsTask = callProductService(List.of("prod-1", "prod-2"));
                VTask<ShippingInfo> shippingTask = callShippingService("ship-789");
                VTask<List<Object>> allResults = Scope.<Object>allSucceed().fork(customerTask).fork(productsTask).fork(shippingTask).join();
                List<Object> results = allResults.runSafe().fold(v -> v, e -> {
    ...
    private static VTask<CustomerInfo> callCustomerService(String customerId) {
        return VTask.of(() -> {
            ...
            String parentSpan = CURRENT_SPAN_ID.get();
            return ScopedValue.where(CURRENT_SPAN_ID, newSpanId).where(PARENT_SPAN_ID, parentSpan).call(() -> {
                recordSpan("customer-service", "getCustomer(" + customerId + ")", () -> {
    ...
    private static VTask<List<ProductInfo>> callProductService(List<String> productIds) {
        return VTask.of(() -> {
            ...
            String parentSpan = CURRENT_SPAN_ID.get();
            return ScopedValue.where(CURRENT_SPAN_ID, newSpanId).where(PARENT_SPAN_ID, parentSpan).call(() -> {
                recordSpan("product-service", "getProducts(" + productIds.size() + ")", () -> {
                    log("Fetching " + productIds.size() + " products");
                    simulateWork(60);
                });
                return productIds.stream().map(id -> new ProductInfo(id, "Product " + id, 29.99)).toList();
            });
        });
    }
    ...
    private static VTask<ShippingInfo> callShippingService(String shippingId) {
        return VTask.of(() -> {
            ...
            String parentSpan = CURRENT_SPAN_ID.get();
            return ScopedValue.where(CURRENT_SPAN_ID, newSpanId).where(PARENT_SPAN_ID, parentSpan).call(() -> {
                recordSpan("shipping-service", "getShipping(" + shippingId + ")", () -> {
                    log("Fetching shipping " + shippingId);
                    // Shipping service calls inventory service
                    VTask<InventoryStatus> inventoryTask = callInventoryService("warehouse-1");
                    InventoryStatus inventory = inventoryTask.runSafe().fold(v -> v, e -> {
                        throw new RuntimeException(e);
                    });
    ...
    private static VTask<InventoryStatus> callInventoryService(String warehouseId) {
        return VTask.of(() -> {
            ...
            String parentSpan = CURRENT_SPAN_ID.get();
            return ScopedValue.where(CURRENT_SPAN_ID, newSpanId).where(PARENT_SPAN_ID, parentSpan).call(() -> {
                recordSpan("inventory-service", "checkInventory(" + warehouseId + ")", () -> {
    ...
    private static void recordSpan(String service, String operation, Runnable work) {
        ...
        try {
            ...
            traceCollector.add(new SpanRecord(traceId, spanId, parentSpanId, service, operation, tenantId, startTime, duration));
        }
    }
    ...
    private static void log(String message) {
        ...
        System.out.printf("[trace=%s span=%s tenant=%s] %s%n", traceId.substring(0, Math.min(8, traceId.length())), spanId, tenant, message);
    }
        ...
        for (Map.Entry<String, List<SpanRecord>> entry : byTrace.entrySet()) {
            ...
            for (SpanRecord span : entry.getValue()) {
                if ("root".equals(span.parentSpanId())) {
                    ...
                    children.computeIfAbsent(span.parentSpanId(), k -> new CopyOnWriteArrayList<>()).add(span);
                }
            }
            // Print tree
            if (root != null) {
    ...
    private static void printSpanTree(SpanRecord span, Map<String, List<SpanRecord>> children, int depth) {
        String indent = "  ".repeat(depth);
        System.out.printf("%s├─ %s [%s] %dms%n", indent, span.service(), span.operation(), span.duration().toMillis());
    ...
    // Data records
    // ============================================================
    record SpanRecord(String traceId, String spanId, String parentSpanId, String service, String operation, String tenantId, Instant startTime, Duration duration) {
    }

    record OrderDetails(String orderId, CustomerInfo customer, List<ProductInfo> products, ShippingInfo shipping) {
    }
