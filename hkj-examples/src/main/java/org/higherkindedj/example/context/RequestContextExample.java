...
public class RequestContextExample {
    ...
    private static void traceIdExample() throws Exception {
        ...
        Context<String, String> logPrefix = Context.asks(RequestContext.TRACE_ID, id -> "[trace=" + id + "] ");
    ...
    private static void correlationIdExample() throws Exception {
        ...
        Context<String, String> serviceHeader = Context.asks(RequestContext.CORRELATION_ID, id -> "X-Correlation-ID: " + id);
    ...
    private static void multiTenantExample() throws Exception {
        ...
        Context<String, String> dataSource = Context.asks(RequestContext.TENANT_ID, id -> "datasource_" + id);
        ...
        for (String tenant : new String[] { "acme-corp", "initech", "umbrella" }) {
            ScopedValue.where(RequestContext.TENANT_ID, tenant).run(() -> {
    ...
    private static void deadlineExample() throws Exception {
        ...
        Context<Instant, Duration> getRemainingTime = Context.asks(RequestContext.DEADLINE, d -> Duration.between(Instant.now(), d));
        ScopedValue.where(RequestContext.REQUEST_TIME, requestTime).where(RequestContext.DEADLINE, deadline).run(() -> {
            ...
            // Simulate some work
            simulateWork(10);
            System.out.println("After work, remaining: " + getRemainingTime.run().toMillis() + "ms");
        });
        System.out.println();
    }
    ...
    private static void localeExample() throws Exception {
        ...
        Context<Locale, String> getGreeting = Context.asks(RequestContext.LOCALE, locale -> {
        ...
        for (Locale locale : new Locale[] { Locale.ENGLISH, Locale.FRENCH, Locale.GERMAN, Locale.JAPANESE }) {
            ScopedValue.where(RequestContext.LOCALE, locale).run(() -> {
    ...
    private static void simulatedServiceChain() throws Exception {
        ...
        System.out.println();
        ScopedValue.where(RequestContext.TRACE_ID, traceId).where(RequestContext.CORRELATION_ID, correlationId).where(RequestContext.REQUEST_TIME, requestTime).where(RequestContext.DEADLINE, deadline).where(RequestContext.TENANT_ID, tenantId).run(() -> {
    ...
    private static void logWithContext(String message) {
        ...
        Duration remaining = Duration.between(Instant.now(), RequestContext.DEADLINE.orElse(Instant.MAX));
        System.out.printf("[trace=%s remaining=%dms] %s%n", traceId.substring(0, 8), remaining.toMillis(), message);
    }
