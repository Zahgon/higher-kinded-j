...
public class SecurityContextExample {
    ...
    private static void authenticationExample() throws Exception {
        ...
        ScopedValue.where(SecurityContext.PRINCIPAL, alice).run(() -> {
        ...
        ScopedValue.where(SecurityContext.PRINCIPAL, (Principal) null).run(() -> {
    ...
    private static void roleCheckingExample() throws Exception {
        ...
        Context<Set<String>, Boolean> hasAdminOrModerator = SecurityContext.hasAnyRole("admin", "moderator");
        Context<Set<String>, Boolean> hasAllRequired = SecurityContext.hasAllRoles("user", "verified");
        System.out.println("Admin user:");
        ScopedValue.where(SecurityContext.ROLES, adminRoles).run(() -> {
        ...
        ScopedValue.where(SecurityContext.ROLES, userRoles).run(() -> {
            System.out.println("  Is admin: " + isAdmin.run());
            System.out.println("  Is user: " + isUser.run());
            try {
                requireAdmin.run();
    ...
    private static void permissionCheckingExample() throws Exception {
        ...
        Set<String> editorPermissions = Set.of("documents:read", "documents:write", "documents:delete", "comments:read", "comments:write");
        ...
        ScopedValue.where(SecurityContext.PERMISSIONS, editorPermissions).run(() -> {
        ...
        ScopedValue.where(SecurityContext.PERMISSIONS, viewerPermissions).run(() -> {
            System.out.println("  Can read: " + canRead.run());
            System.out.println("  Can write: " + canWrite.run());
            System.out.println("  Can delete: " + canDelete.run());
            try {
    ...
    private static void combinedSecurityExample() throws Exception {
        ...
        ScopedValue.where(SecurityContext.PRINCIPAL, admin).where(SecurityContext.ROLES, roles).where(SecurityContext.PERMISSIONS, permissions).where(SecurityContext.AUTH_TOKEN, authToken).where(SecurityContext.SESSION_ID, sessionId).run(() -> {
            ...
            Context<Set<String>, Unit> requireReports = SecurityContext.requirePermission("reports:generate");
            requireReports.run();
    ...
    private static void errorHandlingExample() throws Exception {
        ...
        Context<Set<String>, String> adminFeature = SecurityContext.requireRole("admin").map(unit -> "Admin feature accessed").recover(error -> "Feature unavailable: insufficient permissions");
        // Using recoverWith for alternative behaviour
        Context<Set<String>, String> premiumContent = SecurityContext.requireRole("premium").map(unit -> "Premium content here").recoverWith(error -> Context.succeed("Subscribe to access premium content"));
        // Using mapError to transform exceptions
        Context<Set<String>, Unit> auditedAccess = SecurityContext.requireRole("auditor").mapError(error -> new SecurityException("Audit access denied", error));
        ScopedValue.where(SecurityContext.ROLES, userRoles).run(() -> {
            ...
            try {
                ...
                System.out.println("Audited access: " + e.getClass().getSimpleName() + " - " + e.getMessage());
            }
        });
        System.out.println();
    }
    ...
    private static void anonymousUserExample() throws Exception {
        ...
        Context<Principal, String> getWelcome = SecurityContext.isAuthenticated().flatMap(isAuth -> {
            if (isAuth) {
                return SecurityContext.requireAuthenticated().map(principal -> "Welcome back, " + principal.getName() + "!");
            } else {
                return Context.succeed("Welcome, guest! Please log in.");
            }
        });
        // Conditional feature access
        Context<Principal, String> userProfile = SecurityContext.isAuthenticated().flatMap(isAuth -> {
            if (isAuth) {
                return SecurityContext.requireAuthenticated().map(p -> "Profile for: " + p.getName());
            } else {
        ...
        ScopedValue.where(SecurityContext.PRINCIPAL, bob).run(() -> {
            System.out.println("  " + getWelcome.run());
            System.out.println("  " + userProfile.run());
        });
        System.out.println();
