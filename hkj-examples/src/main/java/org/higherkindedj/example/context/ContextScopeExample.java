...
public class ContextScopeExample {
    ...
    private static void contextToVTaskExample() throws Exception {
        ...
        Context<String, String> getTraceContext = Context.asks(RequestContext.TRACE_ID, id -> "Executing with trace: " + id);
        ...
        ScopedValue.where(RequestContext.TRACE_ID, traceId).where(RequestContext.TENANT_ID, "acme").run(() -> {
            ...
            VTask<String> combined = VTask.succeed("Request Info:").flatMap(header -> traceCtx.toVTask().map(trace -> header + "\n  Trace: " + trace)).flatMap(msg -> tenantCtx.toVTask().map(tenant -> msg + "\n  Tenant: " + tenant));
    ...
    private static void parallelWithSharedContext() throws Exception {
        ...
        ScopedValue.where(RequestContext.TRACE_ID, traceId).where(RequestContext.TENANT_ID, tenantId).run(() -> {
            // Create tasks that read from context
            // Each task runs on a virtual thread that inherits the scoped values
            VTask<String> fetchUser = VTask.of(() -> {
                String trace = RequestContext.TRACE_ID.get();
                String tenant = RequestContext.TENANT_ID.get();
                Thread.sleep(100);
                return String.format("User[trace=%s,tenant=%s]", trace.substring(0, 8), tenant);
            });
            VTask<String> fetchOrders = VTask.of(() -> {
                String trace = RequestContext.TRACE_ID.get();
                String tenant = RequestContext.TENANT_ID.get();
                Thread.sleep(100);
                return String.format("Orders[trace=%s,tenant=%s]", trace.substring(0, 8), tenant);
            });
            VTask<String> fetchPrefs = VTask.of(() -> {
                String trace = RequestContext.TRACE_ID.get();
                String tenant = RequestContext.TENANT_ID.get();
                Thread.sleep(100);
                return String.format("Prefs[trace=%s,tenant=%s]", trace.substring(0, 8), tenant);
            });
            long start = System.currentTimeMillis();
            // Fork all tasks in a scope - they run in parallel on virtual threads
            VTask<List<String>> allResults = Scope.<String>allSucceed().fork(fetchUser).fork(fetchOrders).fork(fetchPrefs).join();
    ...
    private static void securityContextInScope() throws Exception {
        ...
        ScopedValue.where(SecurityContext.PRINCIPAL, admin).where(SecurityContext.ROLES, roles).run(() -> {
            // Each parallel task can check security
            VTask<String> adminTask = VTask.of(() -> {
                // Verify admin access
                if (!SecurityContext.ROLES.get().contains("admin")) {
                    throw new SecurityException("Admin required");
                }
                Thread.sleep(50);
                return "Admin operation completed by " + SecurityContext.PRINCIPAL.get().getName();
            });
            VTask<String> userTask = VTask.of(() -> {
                Principal p = SecurityContext.PRINCIPAL.get();
                Thread.sleep(50);
                return "User operation for " + p.getName();
            });
            VTask<String> auditTask = VTask.of(() -> {
                Principal p = SecurityContext.PRINCIPAL.get();
                Set<String> r = SecurityContext.ROLES.get();
                Thread.sleep(50);
                return String.format("Audit: %s with roles %s", p.getName(), r);
            });
            VTask<List<String>> results = Scope.<String>allSucceed().fork(adminTask).fork(userTask).fork(auditTask).join();
    ...
    private static void deadlinePropagationExample() throws Exception {
        ...
        ScopedValue.where(RequestContext.REQUEST_TIME, requestTime).where(RequestContext.DEADLINE, deadline).run(() -> {
            // Tasks can check remaining time
            VTask<String> task1 = VTask.of(() -> {
                Duration remaining = Duration.between(Instant.now(), RequestContext.DEADLINE.get());
                Thread.sleep(100);
                return "Task1 completed, " + remaining.toMillis() + "ms was remaining";
            });
            VTask<String> task2 = VTask.of(() -> {
                Duration remaining = Duration.between(Instant.now(), RequestContext.DEADLINE.get());
                Thread.sleep(150);
            ...
            VTask<List<String>> results = Scope.<String>allSucceed().timeout(scopeTimeout).fork(task1).fork(task2).join();
        ...
        ScopedValue.where(RequestContext.DEADLINE, tightDeadline).run(() -> {
            VTask<String> slowTask = VTask.of(() -> {
                Thread.sleep(200);
                return "Slow task done";
            });
            Duration remaining = Duration.between(Instant.now(), tightDeadline);
            VTask<List<String>> results = Scope.<String>allSucceed().timeout(remaining).fork(slowTask).join().recover(e -> List.of("Deadline exceeded, using default"));
            List<String> values = results.runSafe().orElse(List.of());
            values.forEach(v -> System.out.println("  " + v));
        });
        System.out.println();
    ...
    private static void realWorldDashboardExample() throws Exception {
        ...
        System.out.println();
        long start = System.currentTimeMillis();
        ScopedValue.where(SecurityContext.PRINCIPAL, user).where(SecurityContext.ROLES, roles).where(RequestContext.TRACE_ID, traceId).where(RequestContext.TENANT_ID, tenantId).where(RequestContext.REQUEST_TIME, requestTime).where(RequestContext.DEADLINE, deadline).run(() -> {
            // Dashboard components, each requiring different context
            VTask<String> userProfile = VTask.of(() -> {
                Principal p = SecurityContext.PRINCIPAL.get();
                Thread.sleep(100);
                return "Profile: " + p.getName();
            });
            VTask<String> notifications = VTask.of(() -> {
                String tenant = RequestContext.TENANT_ID.get();
                Thread.sleep(100);
                return "Notifications: 5 unread [" + tenant + "]";
            });
            VTask<String> premiumFeatures = VTask.of(() -> {
                Set<String> r = SecurityContext.ROLES.get();
                Thread.sleep(100);
            ...
            VTask<String> auditLog = VTask.of(() -> {
                String trace = RequestContext.TRACE_ID.get();
                Principal p = SecurityContext.PRINCIPAL.get();
                Thread.sleep(50);
                return String.format("Audit[%s]: %s accessed dashboard", trace.substring(0, 8), p.getName());
            });
            Duration timeout = Duration.between(Instant.now(), RequestContext.DEADLINE.get());
            VTask<List<String>> dashboard = Scope.<String>allSucceed().timeout(timeout).fork(userProfile).fork(notifications).fork(premiumFeatures).fork(auditLog).join();
            Try<List<String>> result = dashboard.runSafe();
            result.fold(components -> {
