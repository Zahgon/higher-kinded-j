...
public class DeepUpdateRecipes {
    ...
    private static void recipeUpdateDeeplyNestedField() {
        ...
        User verboseUpdate = new User(user.name(), new Address(new Street("New Street", user.address().street().number()), user.address().city(), user.address().postcode()));
        ...
        Lens<Address, Street> streetLens = Lens.of(Address::street, (a, s) -> new Address(s, a.city(), a.postcode()));
        Lens<Street, String> streetNameLens = Lens.of(Street::name, (s, name) -> new Street(name, s.number()));
    ...
    private static void recipeModifyWithFunction() {
        ...
        Lens<User, Address> addressLens = Lens.of(User::address, (u, addr) -> new User(u.name(), addr));
        Lens<Address, Street> streetLens = Lens.of(Address::street, (a, s) -> new Address(s, a.city(), a.postcode()));
        Lens<Street, Integer> streetNumberLens = Lens.of(Street::number, (s, num) -> new Street(s.name(), num));
        Lens<User, Integer> userStreetNumber = addressLens.andThen(streetLens).andThen(streetNumberLens);
    ...
    private static void recipeComposeReusablePaths() {
        ...
        // Define paths once
        Lens<User, Address> addressLens = Lens.of(User::address, (u, addr) -> new User(u.name(), addr));
        Lens<Address, String> cityLens = Lens.of(Address::city, (a, c) -> new Address(a.street(), c, a.postcode()));
        Lens<Address, String> postcodeLens = Lens.of(Address::postcode, (a, p) -> new Address(a.street(), a.city(), p));
        // Reusable paths
