...
public class ListTraversalsExample {
    ...
    private static void demonstrateComposition() {
        ...
        List<Product> products = List.of(new Product("SKU001", "Widget", 10.0, 100), new Product("SKU002", "Gadget", 25.0, 50), new Product("SKU003", "Gizmo", 15.0, 75), new Product("SKU004", "Doohickey", 30.0, 25), new Product("SKU005", "Thingamajig", 20.0, 60));
        System.out.println("Products: " + products.stream().map(Product::name).toList());
        // Lens for product price
        Lens<Product, Double> priceLens = Lens.of(Product::price, (p, newPrice) -> new Product(p.sku(), p.name(), newPrice, p.stock()));
        // Compose: first 3 products → their prices
        Traversal<List<Product>, Double> first3Prices = ListTraversals.<Product>taking(3).andThen(priceLens.asTraversal());
        ...
        Traversal<List<Product>, Double> last2Prices = ListTraversals.<Product>takingLast(2).andThen(priceLens.asTraversal());
    ...
    private static void demonstrateWithFilteredTraversals() {
        ...
        List<Product> products = List.of(// low stock
        new Product("SKU001", "Widget", 10.0, 20), new Product("SKU002", "Gadget", 25.0, 150), // low stock
        new Product("SKU003", "Gizmo", 15.0, 30), new Product("SKU004", "Doohickey", 30.0, 200), // low stock
        new Product("SKU005", "Thingamajig", 20.0, 10), // low stock
        new Product("SKU006", "Whatsit", 35.0, 5), new Product("SKU007", "Contraption", 40.0, 180));
        System.out.println("Products with stock: " + products.stream().map(p -> p.name() + "(" + p.stock() + ")").toList());
        // First 5 products that are also low stock (< 50 units)
        Traversal<List<Product>, Product> first5LowStock = ListTraversals.<Product>taking(5).filtered(p -> p.stock() < 50);
        List<Product> lowStockInFirst5 = Traversals.getAll(first5LowStock, products);
        System.out.println("Low stock products in first 5: " + lowStockInFirst5.stream().map(Product::name).toList());
    ...
    private static void demonstrateStructuralPreservation() {
        ...
        System.out.println("Original first product price: £" + products.get(0).price() + " (not £" + modified.get(0).price() + ")");
        System.out.println();
