...
public class TraversalExtensionsExample {
    ...
    private static void demonstrateGetAllMaybe() {
        ...
        List<TEOrderItem> items = List.of(new TEOrderItem("SKU001", "Laptop", new BigDecimal("999.99"), 1, "pending"), new TEOrderItem("SKU002", "Mouse", new BigDecimal("29.99"), 2, "pending"), new TEOrderItem("SKU003", "Keyboard", new BigDecimal("79.99"), 1, "pending"));
        Lens<TEOrderItem, BigDecimal> priceLens = TEOrderItemLenses.price();
        Traversal<List<TEOrderItem>, BigDecimal> allPrices = Traversals.<TEOrderItem>forList().andThen(priceLens.asTraversal());
        // Extract all prices
    ...
    private static void demonstrateModifyAllMaybe() {
        ...
        List<TEOrderItem> items = List.of(new TEOrderItem("SKU001", "Laptop", new BigDecimal("100.00"), 1, "pending"), new TEOrderItem("SKU002", "Mouse", new BigDecimal("20.00"), 2, "pending"), new TEOrderItem("SKU003", "Keyboard", new BigDecimal("50.00"), 1, "pending"));
        Lens<TEOrderItem, BigDecimal> priceLens = TEOrderItemLenses.price();
        Traversal<List<TEOrderItem>, BigDecimal> allPrices = Traversals.<TEOrderItem>forList().andThen(priceLens.asTraversal());
        // Successful modification (all prices > £10)
        Maybe<List<TEOrderItem>> updated = modifyAllMaybe(allPrices, price -> price.compareTo(new BigDecimal("10")) >= 0 ? Maybe.just(price.multiply(new BigDecimal("1.1"))) : Maybe.nothing(), items);
        System.out.println("10% price increase: " + (updated.isJust() ? "Success" : "Failed"));
        // Failed modification (one price too low)
        List<TEOrderItem> withLowPrice = List.of(new TEOrderItem("SKU001", "Laptop", new BigDecimal("100.00"), 1, "pending"), new TEOrderItem("SKU002", "Cheap Item", new BigDecimal("5.00"), 2, "pending"));
        Maybe<List<TEOrderItem>> failed = modifyAllMaybe(allPrices, price -> price.compareTo(new BigDecimal("10")) >= 0 ? Maybe.just(price.multiply(new BigDecimal("1.1"))) : Maybe.nothing(), withLowPrice);
        System.out.println("With invalid price: " + (failed.isJust() ? "Success" : "Failed (rolled back)"));
        System.out.println();
    }

    private static void demonstrateModifyAllEither() {
        System.out.println("--- modifyAllEither: Fail-Fast Validation ---");
        List<TEOrderItem> items = List.of(new TEOrderItem("SKU001", "Laptop", new BigDecimal("999.99"), 1, "pending"), new TEOrderItem("SKU002", "Mouse", new BigDecimal("-10.00"), 2, "pending"), new TEOrderItem("SKU003", "Keyboard", new BigDecimal("79.99"), 1, "pending"));
        Lens<TEOrderItem, BigDecimal> priceLens = TEOrderItemLenses.price();
        Traversal<List<TEOrderItem>, BigDecimal> allPrices = Traversals.<TEOrderItem>forList().andThen(priceLens.asTraversal());
        // Fail-fast validation (stops at first error)
        Either<String, List<TEOrderItem>> result = modifyAllEither(allPrices, price -> {
    ...
    private static void demonstrateModifyAllValidated() {
        ...
        List<TEOrderItem> items = List.of(new TEOrderItem("SKU001", "Laptop", new BigDecimal("-100.00"), 1, "pending"), new TEOrderItem("SKU002", "Mouse", new BigDecimal("29.99"), -5, "pending"), new TEOrderItem("SKU003", "Keyboard", new BigDecimal("-50.00"), 1, "pending"));
        Lens<TEOrderItem, BigDecimal> priceLens = TEOrderItemLenses.price();
        Traversal<List<TEOrderItem>, BigDecimal> allPrices = Traversals.<TEOrderItem>forList().andThen(priceLens.asTraversal());
        // Accumulate ALL validation errors
        Validated<List<String>, List<TEOrderItem>> result = modifyAllValidated(allPrices, price -> {
            if (price.compareTo(BigDecimal.ZERO) < 0) {
    ...
    private static void demonstrateModifyWherePossible() {
        ...
        List<TEOrderItem> items = List.of(new TEOrderItem("SKU001", "Laptop", new BigDecimal("999.99"), 1, "pending"), new TEOrderItem("SKU002", "Mouse", new BigDecimal("29.99"), 2, "shipped"), new TEOrderItem("SKU003", "Keyboard", new BigDecimal("79.99"), 1, "pending"));
        Lens<TEOrderItem, String> statusLens = TEOrderItemLenses.status();
        Traversal<List<TEOrderItem>, String> allStatuses = Traversals.<TEOrderItem>forList().andThen(statusLens.asTraversal());
        // Update only pending items (best-effort)
        List<TEOrderItem> updated = modifyWherePossible(allStatuses, status -> status.equals("pending") ? Maybe.just("processing") : Maybe.nothing(), items);
    ...
    private static void demonstrateCountValid() {
        ...
        List<TEOrderItem> items = List.of(new TEOrderItem("SKU001", "Laptop", new BigDecimal("999.99"), 1, "pending"), new TEOrderItem("SKU002", "Mouse", new BigDecimal("-10.00"), 2, "pending"), new TEOrderItem("SKU003", "Keyboard", new BigDecimal("79.99"), 1, "pending"), new TEOrderItem("SKU004", "Monitor", new BigDecimal("-50.00"), 1, "pending"));
        Lens<TEOrderItem, BigDecimal> priceLens = TEOrderItemLenses.price();
        Traversal<List<TEOrderItem>, BigDecimal> allPrices = Traversals.<TEOrderItem>forList().andThen(priceLens.asTraversal());
        int validCount = countValid(allPrices, price -> price.compareTo(BigDecimal.ZERO) >= 0 ? Either.right(price) : Either.left("Negative price"), items);
    ...
    private static void demonstrateCollectErrors() {
        ...
        Lens<TEOrderItem, BigDecimal> priceLens = TEOrderItemLenses.price();
        Traversal<List<TEOrderItem>, BigDecimal> allPrices = Traversals.<TEOrderItem>forList().andThen(priceLens.asTraversal());
        List<String> errors = collectErrors(allPrices, price -> price.compareTo(BigDecimal.ZERO) >= 0 ? Either.right(price) : Either.left("Negative price: " + price), items);
        if (errors.isEmpty()) {
            System.out.println("✅ All prices valid");
        } else {
    ...
    private static void demonstrateRealWorldScenario() {
        ...
        TEOrder order = new TEOrder("ORD-12345", List.of(new TEOrderItem("SKU001", "Laptop", new BigDecimal("999.99"), 1, "pending"), new TEOrderItem("SKU002", "Mouse", new BigDecimal("29.99"), 2, "pending"), new TEOrderItem("SKU003", "USB Cable", new BigDecimal("-5.00"), 5, "pending"), new TEOrderItem("SKU004", "Keyboard", new BigDecimal("79.99"), 0, "pending")), "customer@example.com");
        ...
        Lens<TEOrderItem, BigDecimal> priceLens = TEOrderItemLenses.price();
        Traversal<List<TEOrderItem>, BigDecimal> allPrices = Traversals.<TEOrderItem>forList().andThen(priceLens.asTraversal());
        List<String> priceErrors = collectErrors(allPrices, price -> validatePrice(price), order.items());
        ...
        Traversal<List<TEOrderItem>, Integer> allQuantities = Traversals.<TEOrderItem>forList().andThen(quantityLens.asTraversal());
        List<String> quantityErrors = collectErrors(allQuantities, qty -> validateQuantity(qty), order.items());
        ...
        List<TEOrderItem> discounted = modifyWherePossible(allPrices, price -> price.compareTo(BigDecimal.ZERO) > 0 ? Maybe.just(price.multiply(new BigDecimal("0.9"))) : Maybe.nothing(), order.items());
