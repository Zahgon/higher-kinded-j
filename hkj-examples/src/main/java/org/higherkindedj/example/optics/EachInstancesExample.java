...
public class EachInstancesExample {
    ...
    private static void demonstrateIndexedTraversal() {
        ...
        listEach.<Integer>eachWithIndex().ifPresent(indexed -> {
            ...
            List<String> numbered = IndexedTraversals.imodify(indexed, (index, value) -> (index + 1) + ". " + value, items);
    ...
    private static void demonstrateFocusDslIntegration() {
        ...
        Lens<User, Map<String, Order>> ordersLens = Lens.of(User::orders, (u, o) -> new User(u.name(), o));
        // Create lens for Order -> List<Product>
        Lens<Order, List<Product>> productsLens = Lens.of(Order::products, (o, p) -> new Order(o.id(), p));
        // Sample data
        User user = new User("Alice", Map.of("ORD-1", new Order("ORD-1", List.of(new Product("Laptop", 999.99))), "ORD-2", new Order("ORD-2", List.of(new Product("Mouse", 29.99), new Product("Keyboard", 79.99)))));
        // Navigate using Each instances with Focus DSL
        // User -> Map<String, Order> -> Order (values) -> List<Product> -> Product
        TraversalPath<User, Order> allOrders = FocusPath.of(ordersLens).each(EachInstances.mapValuesEach());
        ...
        TraversalPath<User, Product> allProducts = allOrders.via(productsLens).each(EachInstances.listEach());
    ...
    private static void demonstrateMapValuesEachCollecting() {
        ...
        Each<TreeMap<String, Integer>, Integer> sortedValues = EachInstances.mapValuesEachCollecting(map -> new TreeMap<>(map));
        TreeMap<String, Integer> prices = new TreeMap<>(Map.of("eur", 100, "gbp", 120, "usd", 110));
        TreeMap<String, Integer> withVat = Traversals.modify(sortedValues.each(), p -> Math.round(p * 1.2f), prices);
        ...
        Each<CurrencyExposure, Integer> exposureValues = EachInstances.mapValuesEachCollecting(CurrencyExposure::byCurrency, CurrencyExposure::new);
    ...
    private static void demonstrateCustomEach() {
        ...
        fromIndexed.<Integer>eachWithIndex().ifPresent(indexed -> {
            List<String> numbered = IndexedTraversals.imodify(indexed, (i, s) -> "[" + i + "] " + s, List.of("a", "b", "c"));
