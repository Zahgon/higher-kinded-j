...
public class GetterExtensionsExample {
    ...
    record UserProfile(String username, String displayName, @Nullable String bio, @Nullable Address address, ContactInfo contactInfo) {
    }
    ...
    record Config(String appName, @Nullable String apiKey, @Nullable Integer port, @Nullable String logLevel) {
    }
    ...
    private static void demonstrateBasicNullSafety() {
        ...
        UserProfile completeProfile = new UserProfile("alice", "Alice Smith", "Software Engineer", address1, contact1);
    ...
    private static void demonstrateChaining() {
        ...
        // Chain to get street
        Maybe<String> street = getMaybe(addressGetter, profile).flatMap(addr -> getMaybe(streetGetter, addr));
        System.out.println("Street: " + street.orElse("Unknown"));
        // Chain to get ZIP code
        Maybe<String> zipCode = getMaybe(addressGetter, profile).flatMap(addr -> getMaybe(zipCodeGetter, addr));
        System.out.println("ZIP Code: " + zipCode.orElse("Unknown"));
        // Chain with map for transformation
        Maybe<String> formattedAddress = getMaybe(addressGetter, profile).map(addr -> addr.city() + ", " + addr.country()).map(String::toUpperCase);
        ...
        Maybe<String> streetMissing = getMaybe(addressGetter, profileNoAddress).flatMap(addr -> getMaybe(streetGetter, addr));
    ...
    private static void demonstrateFallbackStrategies() {
        ...
        String primaryContact = getMaybe(phoneGetter, contact).orElseGet(() -> getMaybe(emailGetter, contact).orElseGet(() -> getMaybe(faxGetter, contact).orElse("No contact available")));
        ...
        String dynamicDefault = getMaybe(apiKeyGetter, config).orElseGet(() -> "api-key-for-" + config.appName().toLowerCase());
    ...
    private static void demonstrateComposition() {
        ...
        Maybe<String> ceoZipCode = getMaybe(ceoGetter, company).flatMap(c -> getMaybe(addressGetter, c)).flatMap(a -> getMaybe(Getter.of(Address::zipCode), a));
    ...
    private static void demonstrateRealWorldScenarios() {
        ...
        ContactInfo contact1 = new ContactInfo("123-456-7890", "alice@example.com", null);
        Address address1 = new Address("123 Main St", "New York", "10001", "USA");
        UserProfile profile1 = new UserProfile("alice", "Alice Smith", "Software Engineer", address1, contact1);
        ContactInfo contact2 = new ContactInfo(null, "bob@example.com", null);
        ...
        UserProfile ceo = new UserProfile("jdoe", "Jane Doe", "Visionary leader", ceoAddress, ceoContact);
    ...
    private static void displayUserProfile(UserProfile profile, Getter<UserProfile, String> bioGetter, Getter<UserProfile, Address> addressGetter) {
        ...
        String location = getMaybe(addressGetter, profile).map(a -> a.city() + ", " + a.country()).orElse("Location not specified");
    ...
    private static void displayCompanyInfo(Company company, Getter<Company, UserProfile> ceoGetter, Getter<Company, Address> hqGetter) {
        System.out.println("  Company: " + company.name());
        String ceoName = getMaybe(ceoGetter, company).map(UserProfile::displayName).orElse("No CEO assigned");
        System.out.println("    CEO: " + ceoName);
        String hqLocation = getMaybe(hqGetter, company).map(a -> a.city() + ", " + a.country()).orElse("HQ not established");
