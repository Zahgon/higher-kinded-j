...
public class PrismConvenienceMethodsExample {
    ...
    private static void demonstrateMatches() {
        ...
        List<HttpResponse> responses = List.of(new Success("User data", 200), new ValidationError(List.of("Invalid email"), "email"), new Success("Order data", 200), new ServerError("Database connection failed", "trace-123"));
        // Use matches() for filtering
        List<HttpResponse> successfulResponses = responses.stream().filter(successPrism::matches).collect(Collectors.toList());
    ...
    private static void demonstrateMapOptional() {
        ...
        Optional<String> upperCaseData = successPrism.mapOptional(s -> s.data().toUpperCase(), response);
        ...
        Optional<String> noTransform = successPrism.mapOptional(s -> s.data().toUpperCase(), errorResponse);
    ...
    private static void demonstrateModify() {
        ...
        HttpResponse uppercased = successPrism.modify(s -> new Success(s.data().toUpperCase(), s.statusCode()), response);
        ...
        HttpResponse unchanged = successPrism.modify(s -> new Success(s.data().toUpperCase(), s.statusCode()), errorResponse);
    ...
    private static void demonstrateModifyWhen() {
        ...
        Prism<HttpResponse, Success> successPrism = HttpResponsePrisms.success();
        List<HttpResponse> responses = List.of(new Success("short", 200), new Success("This is a longer response message", 200), new Success("ok", 200), new ServerError("Error", "trace-abc"));
        // Only modify success responses with data longer than 10 characters
        List<HttpResponse> processed = responses.stream().map(r -> successPrism.modifyWhen(s -> s.data().length() > 10, s -> new Success("[LONG] " + s.data(), s.statusCode()), r)).collect(Collectors.toList());
    ...
    private static void demonstrateSetWhen() {
        ...
        Prism<HttpResponse, Success> successPrism = HttpResponsePrisms.success();
        List<HttpResponse> responses = List.of(new Success("valid data", 200), // Empty data
        new Success("", 200), new Success("another valid response", 200), new ValidationError(List.of("Error"), "field"));
        Success replacement = new Success("SANITISED", 200);
        // Only set if current data is empty
        List<HttpResponse> sanitised = responses.stream().map(r -> successPrism.setWhen(s -> s.data().isEmpty(), replacement, r)).collect(Collectors.toList());
        System.out.println("Original responses:");
        responses.forEach(r -> System.out.println("  " + r));
        System.out.println("\nSanitised responses:");
    ...
    private static void demonstrateOrElse() {
        ...
        List<HttpResponse> responses = List.of(new Success("Operation completed", 200), new ServerError("Database error", "trace-def"), // Won't match either prism
        new ValidationError(List.of("Invalid"), "field"), new Success("User created", 201));
        System.out.println("Extracting messages:");
        for (HttpResponse response : responses) {
            // Extract messages from either Success or ServerError using mapOptional
            String message = successPrism.mapOptional(Success::data, response).or(() -> serverErrorPrism.mapOptional(ServerError::message, response)).orElse("No message available");
