...
public class PortfolioRiskExample {
    ...
    public record Position(String ticker, int quantity, // JDK Optional  → AffinePath
    Optional<StopLoss> stopLoss, // HKJ Either  → AffinePath
    Either<PricingError, MarketPrice> livePrice) {
    }
     ...
     */
    @GenerateFocus(generateNavigators = true)
    public record AssetClass(String className, // Eclipse Collections → TraversalPath
    ImmutableList<Position> positions, // HKJ Try            → AffinePath
    Try<ValuationResult> latestValuation, // JDK Map            → TraversalPath
     ...
     */
    @GenerateFocus(generateNavigators = true)
    public record Portfolio(String portfolioId, // Eclipse Collections → TraversalPath
    ImmutableList<AssetClass> holdings, // HKJ Validated      → AffinePath
    Validated<List<String>, RiskReport> risk, // HKJ Either         → AffinePath
    Either<ComplianceViolation, Approval> status) {
    }
    ...
    public record WidenedAssetClass(String className, // Map → TraversalPath (auto-widened by widenCollections)
    Map<String, Double> exposures) {
    }
    ...
    private static void basicFocusPaths(Portfolio portfolio) {
        ...
        statusPath.getOptional(portfolio).ifPresent(a -> System.out.println("Approved by: " + a.approvedBy()));
        // risk: Validated<List<String>, RiskReport> → AffinePath (ZERO_OR_ONE via SPI)
        var riskPath = PortfolioFocus.risk();
        riskPath.getOptional(portfolio).ifPresent(r -> System.out.printf("VaR(95): %.2f%%%n", r.var95()));
        System.out.println();
    }
    ...
    private static void affineContainerAccess(Portfolio portfolio) {
        ...
        List<ValuationResult> successfulValuations = Traversals.getAll(valuations.toTraversal(), portfolio);
    ...
    private static void widenCollectionsComparison() {
        ...
        WidenedAssetClass equities = new WidenedAssetClass("Equities", Map.of("USD", 0.85, "EUR", 0.10, "GBP", 0.05));
        // With widenCollections = true: exposures() returns TraversalPath directly
        var exposuresPath = WidenedAssetClassFocus.exposures();
        System.out.println("WidenedAssetClassFocus.exposures() type: " + exposuresPath.getClass().getSimpleName());
    ...
    private static void fullRiskPipeline(Portfolio portfolio) {
        ...
        System.out.printf("Risk methodology: %s, VaR(95): %.2f%%%n", riskReport.get().methodology(), riskReport.get().var95());
        ...
        System.out.printf("Total mid-market value (from %d prices): %.2f%n", prices.size(), totalMidValue);
    ...
    private static Portfolio createSamplePortfolio() {
        var equities = new AssetClass("Equities", Lists.immutable.of(new Position("AAPL", 100, Optional.of(new StopLoss(145.00, "trailing")), Either.right(new MarketPrice(150.25, 150.50, "NYSE"))), new Position("GOOGL", 50, Optional.empty(), Either.right(new MarketPrice(2750.00, 2752.00, "NASDAQ"))), new Position("TSLA", 25, Optional.of(new StopLoss(200.00, "fixed")), Either.left(new PricingError("TIMEOUT", "Exchange not responding"))), new Position("MSFT", 75, Optional.of(new StopLoss(380.00, "trailing")), Either.right(new MarketPrice(390.10, 390.30, "NYSE")))), Try.of(() -> new ValuationResult(125_000.00, 3_200.00, Instant.now())), Map.of("USD", 0.85, "EUR", 0.10, "GBP", 0.05));
        var fixedIncome = new AssetClass("Fixed Income", Lists.immutable.of(new Position("US10Y", 200, Optional.empty(), Either.right(new MarketPrice(98.50, 98.75, "CME"))), new Position("DE10Y", 150, Optional.of(new StopLoss(95.00, "fixed")), Either.right(new MarketPrice(101.25, 101.50, "EUREX")))), Try.failure(new RuntimeException("Valuation service unavailable")), Map.of("USD", 0.60, "EUR", 0.40));
        return new Portfolio("PF-2026-001", Lists.immutable.of(equities, fixedIncome), Validated.valid(new RiskReport(2.35, 8.7, "Historical VaR")), Either.right(new Approval("Risk Committee", Instant.now())));
    }
}
