...
public class BatchProcessingExample {
    ...
    public record DataRecord(String id, String payload, ProcessingStatus status, int retryCount, LocalDateTime processedAt) {
    ...
    private static void demonstrateChunkProcessing(List<DataRecord> records) {
        ...
        for (int chunk = 0; chunk < totalChunks; chunk++) {
            ...
            System.out.printf("  Chunk %d: Processed %d records (indices %d-%d) in %dms%n", chunk + 1, chunkSize, start, end - 1, duration);
        }
        // Verify all processed
        long processedCount = processed.stream().filter(r -> r.status() == ProcessingStatus.PROCESSED).count();
    ...
    private static void demonstratePriorityProcessing(List<DataRecord> records) {
        ...
        List<DataRecord> step1 = Traversals.modify(highPriority, r -> r.transformPayload("URGENT"), records);
        List<DataRecord> step2 = Traversals.modify(mediumPriority, r -> r.transformPayload("NORMAL"), step1);
        List<DataRecord> step3 = Traversals.modify(lowPriority, r -> r.transformPayload("BATCH"), step2);
    ...
    private static void demonstrateProgressiveTransformation(List<DataRecord> records) {
        ...
        List<DataRecord> afterExtract = Traversals.modify(extractBatch, DataRecord::markProcessing, records);
        ...
        List<DataRecord> afterTransform = Traversals.modify(extractBatch, r -> r.transformPayload("ETL"), afterExtract);
        System.out.println("  2. TRANSFORM: Applied ETL prefix to payloads");
        // Stage 3: Load - Mark as processed
        List<DataRecord> afterLoad = Traversals.modify(extractBatch, DataRecord::markProcessed, afterTransform);
        ...
        boolean allProcessed = processed.stream().allMatch(r -> r.status() == ProcessingStatus.PROCESSED);
        ...
        boolean remainingPending = unprocessed.stream().allMatch(r -> r.status() == ProcessingStatus.PENDING);
        System.out.println("  Remaining " + unprocessed.size() + " records still PENDING: " + remainingPending);
        System.out.println();
    }
    ...
    private static void demonstrateFailureRecovery(List<DataRecord> records) {
        ...
        System.out.printf("  PROCESSED: %d records%n", step2.stream().filter(r -> r.status() == ProcessingStatus.PROCESSED).count());
        System.out.printf("  FAILED: %d records%n", step2.stream().filter(r -> r.status() == ProcessingStatus.FAILED).count());
        System.out.printf("  PENDING: %d records%n", step2.stream().filter(r -> r.status() == ProcessingStatus.PENDING).count());
        // Recovery: Retry failed records
        System.out.println("\nRetrying failed records...");
        // Only modify the failed slice - attempt to process them again
        List<DataRecord> afterRetry = Traversals.modify(failedBatch, r -> {
        ...
        System.out.printf("  PROCESSED: %d records%n", afterRetry.stream().filter(r -> r.status() == ProcessingStatus.PROCESSED).count());
        System.out.printf("  FAILED: %d records (max retries exceeded)%n", afterRetry.stream().filter(r -> r.status() == ProcessingStatus.FAILED).count());
        // Show retry counts for failed records
        List<DataRecord> stillFailed = Traversals.getAll(failedBatch, afterRetry);
        stillFailed.stream().filter(r -> r.status() == ProcessingStatus.FAILED).forEach(r -> System.out.printf("    Record %s: %d retries%n", r.id(), r.retryCount()));
        System.out.println();
    }
    ...
    private static void demonstrateParallelChunkPreparation(List<DataRecord> records) {
        ...
        List<List<DataRecord>> chunks = List.of(Traversals.getAll(chunk1, records), Traversals.getAll(chunk2, records), Traversals.getAll(chunk3, records), Traversals.getAll(chunk4, records));
        for (int i = 0; i < chunks.size(); i++) {
            System.out.printf("  Chunk %d: %d records ready for Worker-%d%n", i + 1, chunks.get(i).size(), i + 1);
        }
        ...
        List<Traversal<List<DataRecord>, DataRecord>> chunkTraversals = List.of(chunk1, chunk2, chunk3, chunk4);
        for (int i = 0; i < chunkTraversals.size(); i++) {
            String workerId = workerIds[i];
            finalResult = Traversals.modify(chunkTraversals.get(i), r -> r.transformPayload(workerId).markProcessed(), finalResult);
            System.out.printf("  %s completed chunk %d%n", workerId, i + 1);
        }
        // Verify all processed
        long totalProcessed = finalResult.stream().filter(r -> r.status() == ProcessingStatus.PROCESSED).count();
