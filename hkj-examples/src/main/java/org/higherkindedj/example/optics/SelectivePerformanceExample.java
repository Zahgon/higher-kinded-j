...
public class SelectivePerformanceExample {
    ...
    private static void shortCircuitComparison() {
        ...
        List<Item> items = List.of(new Item("ITEM-001", true, 10), new Item("ITEM-002", true, 20), // First item that doesn't need processing
        new Item("ITEM-003", false, 30), new Item("ITEM-004", true, 40), new Item("ITEM-005", true, 50));
        // Standard Applicative approach - processes all items
        System.out.println("Applicative approach (processes all items):");
        operationCount.set(0);
        Kind<IdKind.Witness, List<Item>> applicativeResult = Traversals.traverseList(items, item -> {
        ...
        operationCount.set(0);
        Kind<IdKind.Witness, List<Item>> selectiveResult = Traversals.traverseListUntil(items, // Stop condition
        item -> !item.needsProcessing(), item -> {
            operationCount.incrementAndGet();
            System.out.println("  Processing: " + item.id());
            return Id.of(item);
    ...
    private static void conditionalProcessingComparison() {
        ...
        Batch batch = new Batch("Mixed Batch", List.of(new Item("ITEM-001", true, 10), new Item("ITEM-002", false, 20), new Item("ITEM-003", true, 30), new Item("ITEM-004", false, 40), new Item("ITEM-005", true, 50)));
        ...
        operationCount.set(0);
        Kind<IdKind.Witness, Batch> allProcessed = itemsTraversal.modifyF(item -> {
            operationCount.incrementAndGet();
        ...
        operationCount.set(0);
        Kind<IdKind.Witness, Batch> selectiveProcessed = itemsTraversal.modifyWhen(Item::needsProcessing, item -> {
            operationCount.incrementAndGet();
            System.out.println("  Processing: " + item.id());
            return Id.of(new Item(item.id(), item.needsProcessing(), item.value() * 2));
        }, batch, IdSelective.instance());
        System.out.println("Total items processed: " + operationCount.get());
        System.out.println("Note: With selective, only matching items execute the expensive operation\n");
    }
    ...
    private static void branchingComparison() {
        ...
        List<Item> items = List.of(new Item("SMALL-001", true, 5), new Item("LARGE-001", true, 500), new Item("SMALL-002", true, 8), new Item("LARGE-002", true, 750), new Item("SMALL-003", true, 3));
        // Standard approach - checks happen inside the function
        System.out.println("Standard approach (implicit branching):");
        operationCount.set(0);
        Traversals.traverseList(items, item -> {
            operationCount.incrementAndGet();
            if (item.value() > 100) {
        ...
        operationCount.set(0);
        Traversals.speculativeTraverseList(items, item -> item.value() > 100, item -> {
            operationCount.incrementAndGet();
            System.out.println("  Heavy processing for " + item.id());
            return Id.of(item);
        }, item -> {
