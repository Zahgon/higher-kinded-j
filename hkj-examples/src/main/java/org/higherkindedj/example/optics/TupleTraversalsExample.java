...
public class TupleTraversalsExample {
    ...
    private static void demonstrateBasicTupleOperations() {
        ...
        Tuple2<String, String> capitalised = Traversals.modify(bothStrings, s -> s.substring(0, 1).toUpperCase() + s.substring(1), names);
    ...
    private static void demonstrateGeographicCoordinates() {
        ...
        Location londonRounded = new Location(london.name(), Traversals.modify(bothCoords, coord -> Math.round(coord * roundingFactor) / roundingFactor, london.coordinates()));
        Location parisRounded = new Location(paris.name(), Traversals.modify(bothCoords, coord -> Math.round(coord * roundingFactor) / roundingFactor, paris.coordinates()));
        ...
        // Small offset
        double offset = 0.001;
        Location londonOffset = new Location(london.name() + " (offset)", Traversals.modify(bothCoords, coord -> coord + offset, london.coordinates()));
        System.out.println("\nWith offset +0.001:");
        System.out.println("  " + londonOffset);
        // Scale coordinates (useful for projection transformations)
        // Convert to microDegrees
        double scale = 1000000.0;
        Tuple2<Double, Double> scaled = Traversals.modify(bothCoords, coord -> coord * scale, london.coordinates());
    ...
    private static void demonstrateStatisticalRanges() {
        ...
        DataRange tempFahrenheit = new DataRange("Temperature (°F)", Traversals.modify(bothValues, celsius -> celsius * 9.0 / 5.0 + 32.0, temperature.range()));
        ...
        Tuple2<Double, Double> normalised = Traversals.modify(bothValues, temp -> (temp - minTemp) / range, temperature.range());
        System.out.println("\nNormalised temperature range:");
        System.out.println("  " + normalised);
        // Expand range by 10% on both sides
        DataRange expanded = new DataRange(humidity.metric() + " (expanded)", Traversals.modify(bothValues, value -> {
            double spread = humidity.range._2() - humidity.range._1();
            return value == humidity.range._1() ? // Expand min
    ...
    private static void demonstrateBoundingBoxes() {
        ...
        Tuple2<Integer, Integer> scaledTopLeft = Traversals.modify(bothCoords, coord -> coord * 2, box.topLeft());
        ...
        int offsetX = 50;
        int offsetY = 20;
        BoundingBox offsetBox = new BoundingBox(Traversals.modify(bothCoords, coord -> coord + offsetX, box.topLeft()), Traversals.modify(bothCoords, coord -> coord + offsetX, box.bottomRight()));
        System.out.println("\nOffset by (50, 50):");
        System.out.println("  " + offsetBox);
        // Clamp coordinates to positive values
        Tuple2<Integer, Integer> negativeCoords = new Tuple2<>(-5, -10);
        Tuple2<Integer, Integer> clamped = Traversals.modify(bothCoords, coord -> Math.max(0, coord), negativeCoords);
    ...
    private static void demonstrateTimePeriods() {
        ...
        TimePeriod shiftedWork = new TimePeriod("Shifted Work Hours", Traversals.modify(bothHours, hour -> hour + 2, workHours.hourRange()));
        System.out.println("\nShifted forward 2 hours:");
        System.out.println("  " + shiftedWork);
        // Extend both periods by 1 hour
        TimePeriod extendedWork = new TimePeriod("Extended Work", Traversals.modify(bothHours, hour -> {
        ...
        // 26 should wrap to 2
        Tuple2<Integer, Integer> late = new Tuple2<>(22, 26);
