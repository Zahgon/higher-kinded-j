...
public class StateMachineExample {
    ...
    @GeneratePrisms
    public sealed interface OrderState permits Draft, Submitted, Processing, Shipped, Delivered, Cancelled {
    }
    ...
    @GenerateLenses
    public record Submitted(String orderId, List<String> items, Instant submittedAt) implements OrderState {
    }

    @GenerateLenses
    public record Processing(String orderId, List<String> items, String warehouseId, Instant startedAt) implements OrderState {
    }

    public record Shipped(String orderId, List<String> items, String trackingNumber, String carrier, Instant shippedAt) implements OrderState {
    }

    public record Delivered(String orderId, Instant deliveredAt, String signature) implements OrderState {
    }

    public record Cancelled(String orderId, String reason, Instant cancelledAt) implements OrderState {
    }
    ...
    private static void demonstrateConditionalTransitions() {
        ...
        OrderState processing = new Processing("ORD-003", List.of("Monitor"), "WH-UNAVAILABLE", Instant.now());
        System.out.println("\nProcessing order at unavailable warehouse: " + processing);
        OrderState maybeCancelled = PROCESSING.getOptional(processing).filter(proc -> proc.warehouseId().equals("WH-UNAVAILABLE")).map(proc -> (OrderState) new Cancelled("ORD-003", "Warehouse unavailable", Instant.now())).orElse(processing);
        if (CANCELLED.matches(maybeCancelled)) {
            System.out.println("Order was cancelled due to warehouse issue");
            System.out.println("  Reason: " + CANCELLED.mapOptional(Cancelled::reason, maybeCancelled).orElse("Unknown"));
        }
        System.out.println();
    }
    ...
    private static void demonstrateStateQueries() {
        ...
        List<OrderState> orders = List.of(new Draft("ORD-001", List.of("Item1")), new Submitted("ORD-002", List.of("Item2"), Instant.now()), new Processing("ORD-003", List.of("Item3"), "WH-001", Instant.now()), new Shipped("ORD-004", List.of("Item4"), "TRACK-001", "DHL", Instant.now()), new Delivered("ORD-005", Instant.now(), "Customer"), new Cancelled("ORD-006", "Out of stock", Instant.now()));
        ...
        List<String> trackingNumbers = orders.stream().flatMap(order -> SHIPPED.mapOptional(Shipped::trackingNumber, order).stream()).toList();
        System.out.println("\nTracking numbers: " + trackingNumbers);
        // Check if any orders are awaiting action (Draft or Submitted)
        List<String> awaitingOrderIds = orders.stream().flatMap(order -> DRAFT.mapOptional(Draft::orderId, order).or(() -> SUBMITTED.mapOptional(Submitted::orderId, order)).stream()).toList();
    ...
    private static void demonstrateComplexWorkflow() {
        ...
        boolean isHighValue = processingDetails.map(proc -> proc.items().stream().anyMatch(item -> item.contains("Premium"))).orElse(false);
        if (isHighValue) {
    ...
    private static OrderState transitionToSubmitted(OrderState state) {
        return DRAFT.getOptional(state).filter(draft -> !draft.items().isEmpty()).map(draft -> (OrderState) new Submitted(draft.orderId(), draft.items(), Instant.now())).orElse(state);
    }

    private static OrderState transitionToProcessing(OrderState state, String warehouseId) {
        return SUBMITTED.getOptional(state).map(submitted -> (OrderState) PROCESSING.build(new Processing(submitted.orderId(), submitted.items(), warehouseId, Instant.now()))).orElse(state);
    }

    private static OrderState transitionToShipped(OrderState state, String trackingNumber, String carrier) {
        return PROCESSING.getOptional(state).map(processing -> (OrderState) SHIPPED.build(new Shipped(processing.orderId(), processing.items(), trackingNumber, carrier, Instant.now()))).orElse(state);
    }

    private static OrderState transitionToDelivered(OrderState state, String signature) {
        return SHIPPED.getOptional(state).map(shipped -> (OrderState) DELIVERED.build(new Delivered(shipped.orderId(), Instant.now(), signature))).orElse(state);
    }

    private static OrderState cancelOrder(OrderState state, String reason) {
        // Can cancel from Draft, Submitted, or Processing states
        return DRAFT.mapOptional(Draft::orderId, state).or(() -> SUBMITTED.mapOptional(Submitted::orderId, state)).or(() -> PROCESSING.mapOptional(Processing::orderId, state)).map(orderId -> (OrderState) new Cancelled(orderId, reason, Instant.now())).orElse(state);
    }
}
