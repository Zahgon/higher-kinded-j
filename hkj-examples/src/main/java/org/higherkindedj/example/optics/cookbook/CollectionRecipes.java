...
public class CollectionRecipes {
    ...
    private static void recipeModifyAllElements() {
        ...
        List<Player> players = List.of(new Player("Alice", 100, true), new Player("Bob", 85, true), new Player("Charlie", 92, false));
        // Traversal for list elements
        Traversal<List<Player>, Player> allPlayers = Traversals.forList();
        // Lens to player's score
        Lens<Player, Integer> scoreLens = Lens.of(Player::score, (p, s) -> new Player(p.name(), s, p.active()));
    ...
    private static void recipeGetAllValues() {
        ...
        List<Player> players = List.of(new Player("Alice", 100, true), new Player("Bob", 85, true), new Player("Charlie", 92, false));
        Traversal<List<Player>, Player> allPlayers = Traversals.forList();
        Lens<Player, String> nameLens = Lens.of(Player::name, (p, n) -> new Player(n, p.score(), p.active()));
        Lens<Player, Integer> scoreLens = Lens.of(Player::score, (p, s) -> new Player(p.name(), s, p.active()));
        // Get all names
    ...
    private static void recipeNestedCollections() {
        ...
        Team teamA = new Team("Team A", List.of(new Player("Alice", 100, true), new Player("Bob", 85, true)));
        Team teamB = new Team("Team B", List.of(new Player("Charlie", 92, true), new Player("Diana", 88, false)));
        League league = new League("Premier League", List.of(teamA, teamB));
        // Build path: League -> teams -> players -> score
        Lens<League, List<Team>> teamsLens = Lens.of(League::teams, (l, teams) -> new League(l.name(), teams));
        Lens<Team, List<Player>> playersLens = Lens.of(Team::players, (t, players) -> new Team(t.name(), players));
        Lens<Player, Integer> scoreLens = Lens.of(Player::score, (p, s) -> new Player(p.name(), s, p.active()));
        // Compose lenses and traversals directly
        Traversal<League, Integer> allScores = teamsLens.andThen(Traversals.<Team>forList()).andThen(playersLens).andThen(Traversals.<Player>forList()).andThen(scoreLens);
    ...
    private static void recipeConditionalCollectionUpdate() {
        ...
        List<Player> players = List.of(new Player("Alice", 100, true), new Player("Bob", 85, false), new Player("Charlie", 92, true), new Player("Diana", 78, false));
        Traversal<List<Player>, Player> allPlayers = Traversals.forList();
        Lens<Player, Integer> scoreLens = Lens.of(Player::score, (p, s) -> new Player(p.name(), s, p.active()));
        // Only boost active players
        Traversal<List<Player>, Integer> allScores = allPlayers.andThen(scoreLens);
        // Using modify with conditional logic
        List<Player> updated = Traversals.modify(allPlayers, player -> {
            if (player.active()) {
