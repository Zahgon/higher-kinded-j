...
public class ConditionalUpdateRecipes {
    ...
    private static void recipeUpdateSpecificVariant() {
        ...
        Prism<OrderStatus, Processing> processingPrism = Prism.of(status -> status instanceof Processing p ? Optional.of(p) : Optional.empty(), p -> p);
        ...
        OrderStatus updatedPending = processingPrism.modify(p -> new Processing(p.orderId(), "bob"), pending);
        OrderStatus updatedProcessing = processingPrism.modify(p -> new Processing(p.orderId(), "bob"), processing);
        OrderStatus updatedShipped = processingPrism.modify(p -> new Processing(p.orderId(), "bob"), shipped);
    ...
    private static void recipeFilterByType() {
        ...
        Prism<OrderStatus, Pending> pendingPrism = Prism.of(status -> status instanceof Pending p ? Optional.of(p) : Optional.empty(), p -> p);
        List<OrderStatus> orders = List.of(new Pending("ORD-001"), new Processing("ORD-002", "alice"), new Pending("ORD-003"), new Shipped("ORD-004", "TRACK-456"), new Cancelled("ORD-005", "Out of stock"));
        // Get only pending orders
        List<OrderStatus> pendingOrders = orders.stream().filter(pendingPrism::matches).collect(Collectors.toList());
    ...
    private static void recipeTypeChecking() {
        ...
        Prism<OrderStatus, Shipped> shippedPrism = Prism.of(status -> status instanceof Shipped s ? Optional.of(s) : Optional.empty(), s -> s);
    ...
    private static void recipeExclusionFiltering() {
        ...
        Prism<OrderStatus, Cancelled> cancelledPrism = Prism.of(status -> status instanceof Cancelled c ? Optional.of(c) : Optional.empty(), c -> c);
        List<OrderStatus> orders = List.of(new Pending("ORD-001"), new Cancelled("ORD-002", "Customer request"), new Processing("ORD-003", "bob"), new Cancelled("ORD-004", "Payment failed"), new Shipped("ORD-005", "TRACK-111"));
        // Get all non-cancelled orders
        List<OrderStatus> activeOrders = orders.stream().filter(cancelledPrism::doesNotMatch).collect(Collectors.toList());
