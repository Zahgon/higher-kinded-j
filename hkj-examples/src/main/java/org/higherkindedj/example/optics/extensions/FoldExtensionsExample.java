...
public class FoldExtensionsExample {
    ...
    private static void demonstrateFindMaybe(Store store) {
        ...
        // Chain with map
        Maybe<String> vipEmail = findMaybe(customersFold, c -> c.loyaltyPoints() > 2000, store).map(Customer::email);
    ...
    private static void demonstrateGetAllMaybe(Store store) {
        ...
        Maybe<Double> orderTotal = getAllMaybe(itemsFold, order1).map(items -> items.stream().mapToDouble(Product::price).sum());
        if (orderTotal.isJust()) {
            Double total = orderTotal.get();
            System.out.println("Order total: $" + String.format("%.2f", total));
        }
        // Count items by category
        Maybe<Long> furnitureCount = getAllMaybe(itemsFold, store.orders().get(2)).map(items -> items.stream().filter(p -> "Furniture".equals(p.category())).count());
    ...
    private static void demonstrateChaining(Store store) {
        ...
        Maybe<String> expensiveElectronicsName = findMaybe(inventoryFold, p -> "Electronics".equals(p.category()) && p.price() > 400, store).map(Product::name).map(String::toUpperCase);
        ...
        Maybe<String> stockAlert = previewMaybe(inventoryFold, store).map(p -> {
            if (p.stock() == 0) {
        ...
        Fold<Order, Product> itemsFold = Fold.of(Order::items);
        Maybe<String> mostExpensiveFurnitureMessage = getAllMaybe(inventoryFold, store).flatMap(products -> products.stream().filter(p -> "Furniture".equals(p.category())).max((p1, p2) -> Double.compare(p1.price(), p2.price())).map(Maybe::just).orElse(Maybe.nothing())).map(p -> "Most expensive furniture: " + p.name() + " ($" + p.price() + ")");
    ...
    private static void demonstrateRealWorldScenarios(Store store) {
        ...
        store.inventory().forEach(product -> {
            Maybe<Product> restockItem = findMaybe(inventoryFold, p -> p.id().equals(product.id()) && p.stock() < 5, store);
        ...
        Fold<Store, Customer> customersFold = Fold.of(Store::customers);
        Maybe<String> vipRewardMessage = findMaybe(customersFold, c -> c.loyaltyPoints() > 2500, store).map(c -> "Congratulations " + c.name() + "! You've earned a $50 reward (" + c.loyaltyPoints() + " points)");
        ...
        store.orders().forEach(order -> {
            ...
            if (unavailableItem.isNothing()) {
                ...
                System.out.println("  Order " + order.orderId() + ": BLOCKED - " + p.name() + " out of stock");
            }
        });
        System.out.println();
        ...
        for (String category : categories) {
            Maybe<Long> count = getAllMaybe(inventoryFold, store).map(products -> products.stream().filter(p -> category.equals(p.category())).count());
            if (count.isJust()) {
                Long c = count.get();
        ...
        Maybe<List<Product>> affordableProducts = getAllMaybe(inventoryFold, store).map(products -> products.stream().filter(p -> p.price() >= 100 && p.price() <= 500).toList());
