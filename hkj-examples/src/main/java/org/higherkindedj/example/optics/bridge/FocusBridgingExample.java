...
public class FocusBridgingExample {
    ...
    private static void demonstrateSimpleBridging(Company company) {
        ...
        String hqCityInline = CompanyFocus.headquarters().toLens().andThen(AddressOptics.city()).get(company);
    ...
    private static void demonstrateModifications(Company company) {
        ...
        UnaryOperator<String> formatPhone = phone -> phone.replaceAll("[^0-9]", "").replaceAll("(.{3})(.{3})(.{4})", "($1) $2-$3");
        Company withFormattedPhones = CompanyBridge.allCompanyPhones().modifyAll(formatPhone, company);
        System.out.println("Phone after formatting: " + CompanyBridge.allCompanyPhones().getAll(withFormattedPhones).getFirst());
        // Update all headquarters fields at once
        Company withNewHq = CompanyFocus.headquarters().toLens().modify(addr -> addr.withStreet("456 Corporate Ave").withCity("San Francisco").withPostcode("94105"), company);
    ...
    private static void demonstrateDomainService(Company company) {
        ...
        System.out.println("After relocation to Seattle: " + CompanyBridge.HEADQUARTERS_CITY.get(relocated));
    ...
    private static Company createSampleCompany() {
        ...
        Employee alice = new Employee("E001", "Alice Smith", ContactInfo.builder().email("alice@acme.com").phone("617-555-0101").build(), new BigDecimal("95000"));
        Employee bob = new Employee("E002", "Bob Jones", ContactInfo.builder().email("bob@acme.com").phone("617-555-0102").fax("617-555-9999").build(), new BigDecimal("85000"));
        Department engineering = new Department("Engineering", alice, List.of(alice, bob), Address.builder().street("100 Tech Drive").city("Boston").postcode("02101").country("USA").build());
        // Sales department in Chicago
        Employee carol = new Employee("E003", "Carol White", ContactInfo.builder().email("carol@acme.com").phone("312-555-0201").build(), new BigDecimal("75000"));
        Department sales = new Department("Sales", carol, List.of(carol), Address.builder().street("200 Commerce St").city("Chicago").postcode("60601").country("USA").build());
        // Company with headquarters in New York
        return new Company("Acme Corp", Address.builder().street("1 Corporate Plaza").city("New York").postcode("10001").country("USA").build(), List.of(engineering, sales));
    }
}
