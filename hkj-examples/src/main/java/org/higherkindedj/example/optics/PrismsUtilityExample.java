...
public class PrismsUtilityExample {
    ...
    private static void demonstrateLeftAndRight() {
        ...
        Either<ValidationError, String> validationResult = Either.left(new ValidationError("ERR001", "Invalid email format"));
        Prism<Either<ValidationError, String>, ValidationError> validationErrorPrism = Prisms.left();
        Optional<String> errorCode = validationErrorPrism.mapOptional(ValidationError::code, validationResult);
    ...
    private static void demonstrateNotNull() {
        ...
        System.out.println("Extract from 'hello': " + notNullPrism.getOptional(value).orElse("was null"));
        System.out.println("Extract from null: " + notNullPrism.getOptional(nullValue).orElse("was null"));
        // Practical use: Filter null values in a list
        List<String> mixedList = Arrays.asList("hello", null, "world", null, "test");
        Traversal<List<String>, String> nonNullStrings = Traversals.<String>forList().andThen(Prisms.<String>notNull().asTraversal());
    ...
    private static void demonstrateInstanceOf() {
        ...
        List<Animal> animals = List.of(new Dog("Rex", "German Shepherd"), new Cat("Whiskers", 9), new Dog("Max", "Beagle"), new Bird("Tweety", true));
        // Extract all dogs
        ...
        List<String> breeds = Traversals.getAll(Traversals.<Animal>forList().andThen(dogBreed), animals);
        System.out.println("All dog breeds: " + breeds);
        // Modify all dogs
        List<Animal> renamedDogs = animals.stream().map(a -> dogPrism.modify(dog -> new Dog("Sir " + dog.name(), dog.breed()), a)).toList();
    ...
    private static void demonstrateListHead() {
        ...
        Optional<Integer> firstEven = headPrism.mapOptional(name -> name.length() % 2 == 0 ? 1 : 0, names);
    ...
    private static void demonstrateListLast() {
        ...
        List<String> logEntries = List.of("2025-01-01 10:00:00 INFO Starting", "2025-01-01 10:01:00 INFO Processing", "2025-01-01 10:02:00 ERROR Failed");
    ...
    private static void demonstrateComposition() {
        ...
        Prism<Optional<Either<String, DatabaseSettings>>, Either<String, DatabaseSettings>> somePrism = Prisms.some();
        Prism<Either<String, DatabaseSettings>, DatabaseSettings> rightPrism = Prisms.right();
        // Valid configuration
        Config validConfig = new Config(Optional.of(Either.right(new DatabaseSettings("localhost", 5432))));
        // Extract database settings
        Optional<DatabaseSettings> settings = somePrism.getOptional(validConfig.database()).flatMap(rightPrism::getOptional);
        ...
        Optional<String> errorMsg = somePrism.getOptional(errorConfig.database()).flatMap(leftPrism::getOptional);
        System.out.println("Error message: " + errorMsg.orElse("N/A"));
        // Missing configuration
        Config missingConfig = new Config(Optional.empty());
        Optional<DatabaseSettings> missingSettings = somePrism.getOptional(missingConfig.database()).flatMap(rightPrism::getOptional);
        System.out.println("Missing config host: " + missingSettings.map(DatabaseSettings::host).orElse("N/A"));
        System.out.println();
    }
}
