...
public class FluentOpticOpsExample {
    ...
    private static void staticMethodExamples(Order order) {
        ...
        Lens<Order, String> orderToPostCode = OrderLenses.address().andThen(ShippingAddressLenses.postCode());
        ...
        Traversal<Order, BigDecimal> orderToAllPrices = OrderTraversals.items().andThen(OrderItemLenses.price().asTraversal());
        ...
        Order discountedOrder = OpticOps.modifyAll(order, orderToAllPrices, price -> price.multiply(discountMultiplier).setScale(2, RoundingMode.HALF_UP));
    ...
    private static void fluentBuilderExamples(Order order) {
        ...
        Order updatedCity = OpticOps.setting(order).through(OrderLenses.address().andThen(ShippingAddressLenses.city()), "Manchester");
        String newCity = OpticOps.getting(updatedCity).through(OrderLenses.address().andThen(ShippingAddressLenses.city()));
        System.out.println("  New city: " + newCity);
        System.out.println();
        // Example 3: Modifying with fluent API
        System.out.println("Example 3: Double all quantities");
        Traversal<Order, Integer> orderToQuantities = OrderTraversals.items().andThen(OrderItemLenses.quantity().asTraversal());
        Order doubledQuantities = OpticOps.modifying(order).allThrough(orderToQuantities, quantity -> quantity * 2);
        System.out.println("  Original quantities: " + OpticOps.getting(order).allThrough(orderToQuantities));
        System.out.println("  Doubled quantities: " + OpticOps.getting(doubledQuantities).allThrough(orderToQuantities));
        System.out.println();
        // Example 4: Chaining multiple operations
        System.out.println("Example 4: Complex update chain");
        Order statusUpdated = OpticOps.setting(order).through(OrderLenses.status(), OrderStatus.PROCESSING);
        Order complexUpdate = OpticOps.modifying(statusUpdated).through(OrderLenses.address().andThen(ShippingAddressLenses.postCode()), String::toUpperCase);
        System.out.println("  Status: " + OpticOps.getting(complexUpdate).through(OrderLenses.status()));
        System.out.println("  Postcode: " + OpticOps.getting(complexUpdate).through(OrderLenses.address().andThen(ShippingAddressLenses.postCode())));
        System.out.println();
    }
    ...
    private static void realWorldWorkflow(Order order) {
        ...
        int totalItems = OpticOps.getAll(order, OrderTraversals.items().andThen(OrderItemLenses.quantity().asTraversal())).stream().mapToInt(Integer::intValue).sum();
        ...
        if (totalItems >= 5) {
            ...
            processedOrder = OpticOps.modifyAll(processedOrder, OrderTraversals.items().andThen(OrderItemLenses.price().asTraversal()), price -> price.multiply(bulkDiscount).setScale(2, RoundingMode.HALF_UP));
        } else {
        ...
        processedOrder = OpticOps.modifying(processedOrder).through(OrderLenses.address().andThen(ShippingAddressLenses.postCode()), String::toUpperCase);
        processedOrder = OpticOps.modifying(processedOrder).through(OrderLenses.address().andThen(ShippingAddressLenses.city()), city -> capitalise(city));
        ...
        BigDecimal finalTotal = OpticOps.getAll(processedOrder, OrderTraversals.items().andThen(OrderItemLenses.price().asTraversal())).stream().reduce(BigDecimal.ZERO, BigDecimal::add);
    ...
    private static void queryingExamples(Order order) {
        ...
        Traversal<Order, BigDecimal> orderToPrices = OrderTraversals.items().andThen(OrderItemLenses.price().asTraversal());
        // Example 1: Check if any item is expensive
        System.out.println("Example 1: Check for expensive items (> £100)");
        boolean hasExpensiveItems = OpticOps.exists(order, orderToPrices, price -> price.compareTo(new BigDecimal("100")) > 0);
        ...
        boolean allPositive = OpticOps.all(order, orderToPrices, price -> price.compareTo(BigDecimal.ZERO) > 0);
        ...
        Optional<OrderItem> bulkItem = OpticOps.find(order, OrderTraversals.items(), item -> item.quantity() > 2);
        bulkItem.ifPresentOrElse(item -> System.out.println("  Found: " + item.productName() + " (qty: " + item.quantity() + ")"), () -> System.out.println("  No bulk items found"));
        System.out.println();
    ...
    private static void validationExamples(Order order) {
        ...
        System.out.println("OpticOps provides validation-aware modification methods that integrate with");
        ...
        Either<String, Order> validatedOrder = OpticOps.modifyAllEither(order, orderToPrices, price -> {
            if (price.compareTo(BigDecimal.ZERO) <= 0) {
                return Either.left("Price must be positive: " + price);
            }
            return Either.right(price);
        });
        validatedOrder.fold(error -> {
        ...
        Order testOrder = new Order("TEST-001", OrderStatus.PENDING, List.of(new OrderItem("P1", "Item 1", 1, new BigDecimal("50.00")), new OrderItem("P2", "Item 2", 2, new BigDecimal("100.00"))), order.address());
        Validated<List<String>, Order> validatedWithAccumulation = OpticOps.modifyAllValidated(testOrder, orderToPrices, price -> {
        ...
        validatedWithAccumulation.fold(errors -> {
        ...
        Either<String, Order> fluentValidation = OpticOps.modifyingWithValidation(order).allThroughEither(orderToPrices, price -> price.compareTo(BigDecimal.ZERO) > 0 ? Either.right(price) : Either.left("Invalid price: " + price));
        fluentValidation.fold(error -> {
        ...
        System.out.println("  • modifyAllValidated / allThroughValidated: Multi-field, accumulate all errors");
        System.out.println("  • modifyAllEither / allThroughEither: Multi-field, short-circuit on first error");
        System.out.println();
    ...
    private static Order createSampleOrder() {
        return new Order("ORD-2025-001", OrderStatus.PENDING, List.of(new OrderItem("PROD-101", "Laptop", 1, new BigDecimal("899.99")), new OrderItem("PROD-202", "Wireless Mouse", 2, new BigDecimal("29.99")), new OrderItem("PROD-303", "USB-C Cable", 3, new BigDecimal("12.50")), new OrderItem("PROD-404", "Laptop Stand", 1, new BigDecimal("45.00"))), new ShippingAddress("123 High Street", "london", "sw1a 1aa", "United Kingdom"));
    }
    ...
    private static void printOrder(Order order) {
        ...
        order.items().forEach(item -> System.out.println("    - " + item.productName() + " x" + item.quantity() + " @ £" + item.price()));
