...
public class PaginationExample {
    ...
    // Domain models
    public record Product(String sku, String name, double price, int stock, boolean featured, String badge) {
    ...
    private static void demonstrateFeaturedProducts(List<Product> catalogue) {
        ...
        List<Product> withHeroSection = Traversals.modify(heroProducts, p -> p.markFeatured().withBadge("HOT"), catalogue);
        System.out.println("Hero section products:");
        Traversals.getAll(heroProducts, withHeroSection).forEach(p -> System.out.printf("  ⭐ %s [%s] - Featured: %s%n", p.name(), p.badge(), p.featured()));
        // Apply special 15% discount to hero products
        Lens<Product, Double> priceLens = Lens.of(Product::price, (prod, newPrice) -> new Product(prod.sku(), prod.name(), newPrice, prod.stock(), prod.featured(), prod.badge()));
        ...
        for (int i = 0; i < 5; i++) {
            ...
            String marker = i < 3 ? "★" : " ";
            System.out.printf("  %s %s: £%.2f → £%.2f%n", marker, original.name(), original.price(), discounted.price());
        }
        System.out.println();
    }
    ...
    private static void demonstrateClearanceSection(List<Product> catalogue) {
        ...
        List<Product> withClearance = Traversals.modify(clearanceItems, p -> p.applyDiscount(0.3).withBadge("CLEARANCE"), catalogue);
        System.out.println("\nAfter 30% clearance discount:");
        Traversals.getAll(clearanceItems, withClearance).forEach(p -> System.out.printf("  🏷️ %s [%s] - £%.2f%n", p.name(), p.badge(), p.price()));
    ...
    private static void demonstrateBulkPageUpdate(List<Product> catalogue) {
        ...
        System.out.printf("  Product 1 (Page 1): %s [%s]%n", finalCatalogue.get(0).name(), finalCatalogue.get(0).badge());
        System.out.printf("  Product 6 (Page 2): %s [%s]%n", finalCatalogue.get(5).name(), finalCatalogue.get(5).badge());
        System.out.printf("  Product 11 (Page 3): %s [%s]%n", finalCatalogue.get(10).name(), finalCatalogue.get(10).badge());
        System.out.printf("  Product 16 (Page 4): %s [%s]%n", finalCatalogue.get(15).name(), finalCatalogue.get(15).badge());
        System.out.println();
    }
    ...
    private static PagedResponse getPage(List<Product> catalogue, int pageNumber, int pageSize) {
        Traversal<List<Product>, Product> pageTraversal = ListTraversals.slicing(pageNumber * pageSize, (pageNumber + 1) * pageSize);
