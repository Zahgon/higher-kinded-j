...
public class OptionalMapTraversalsExample {
    ...
    record ApplicationConfig(String appName, Optional<ServerConfig> server, Map<String, String> properties) {
    ...
    private static void demonstrateOptionalTraversals() {
        ...
        ServerConfig server = new ServerConfig("localhost", Optional.of(8080), Optional.of("/etc/ssl/cert.pem"));
        System.out.println("\n--- Server Configuration ---");
        System.out.println("Original:  " + server);
        // Modify port if present
        ServerConfig offsetServer = new ServerConfig(server.hostname(), Traversals.modify(optTraversal, p -> p + 1000, server.port()), server.sslCertPath());
        System.out.println("Offset:    " + offsetServer);
        // Handle server with no port
        ServerConfig serverNoPort = new ServerConfig("api.example.com", Optional.empty(), Optional.empty());
        System.out.println("\nServer without port: " + serverNoPort);
        ServerConfig attemptOffset = new ServerConfig(serverNoPort.hostname(), Traversals.modify(optTraversal, p -> p + 1000, serverNoPort.port()), serverNoPort.sslCertPath());
    ...
    private static void demonstrateMapValueTraversals() {
        ...
        Traversal<Map<String, Double>, Double> expensiveItems = priceTraversal.filtered(price -> price > 20.0);
        Map<String, Double> discounted = Traversals.modify(expensiveItems, price -> price * 0.9, prices);
    ...
    private static void demonstrateComposedTraversals() {
        ...
        services.put("db", new ServerConfig("db.example.com", Optional.of(5432), Optional.of("/etc/ssl/db.pem")));
        services.put("cache", new ServerConfig("cache.example.com", Optional.empty(), Optional.empty()));
        ...
        services.forEach((name, config) -> {
            ...
            offsetServices.put(name, new ServerConfig(config.hostname(), newPort, config.sslCertPath()));
        });
        System.out.println("\nAfter port offset (+1000):");
        offsetServices.forEach((name, config) -> System.out.println("  " + name + ": " + config));
        // Extract all configured ports
        List<Integer> configuredPorts = services.values().stream().flatMap(config -> config.port().stream()).toList();
    ...
    private static void demonstrateFeatureFlagManagement() {
        ...
        // Not yet configured
        flags.put("beta-features", Optional.empty());
        flags.put("analytics", Optional.of(true));
        FeatureFlags config = new FeatureFlags(flags);
        System.out.println("Original flags:");
        System.out.println("  " + config);
        flags.forEach((name, value) -> System.out.println("    " + name + ": " + value.map(String::valueOf).orElse("unset")));
        // Enable all flags that are currently set (respect unset flags)
        Traversal<Map<String, Optional<Boolean>>, Optional<Boolean>> allFlagValues = Traversals.forMapValues();
        ...
        enabled.forEach((name, value) -> System.out.println("    " + name + ": " + value.map(String::valueOf).orElse("unset")));
        // Disable specific flags
        Map<String, Optional<Boolean>> selective = new HashMap<>(flags);
        // Explicitly disable
        selective.put("beta-features", Optional.of(false));
        System.out.println("\nWith beta-features explicitly disabled:");
        selective.forEach((name, value) -> System.out.println("    " + name + ": " + value.map(String::valueOf).orElse("unset")));
        // Extract enabled feature names
        List<String> enabledFeatures = flags.entrySet().stream().filter(e -> e.getValue().orElse(false)).map(Map.Entry::getKey).toList();
