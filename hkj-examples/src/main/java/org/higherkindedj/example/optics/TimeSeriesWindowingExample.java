...
public class TimeSeriesWindowingExample {
    ...
    public record DailyMetric(LocalDate date, double revenue, int transactions, double avgOrderValue, boolean adjusted) {
    ...
    private static void demonstrateRecentPeriodAnalysis(List<DailyMetric> metrics) {
        ...
        DailyMetric bestDay = recentMetrics.stream().max((a, b) -> Double.compare(a.revenue(), b.revenue())).orElseThrow();
        DailyMetric worstDay = recentMetrics.stream().min((a, b) -> Double.compare(a.revenue(), b.revenue())).orElseThrow();
    ...
    private static void demonstrateHistoricalComparison(List<DailyMetric> metrics) {
        ...
        double historicalAvg = historicalData.stream().mapToDouble(DailyMetric::revenue).average().orElse(0.0);
        ...
        String trend = growthPercent > 5 ? "📈 Strong growth" : growthPercent > 0 ? "📊 Moderate growth" : "📉 Decline";
    ...
    private static void demonstrateSeasonalAdjustment(List<DailyMetric> metrics) {
        ...
        List<DailyMetric> step1 = Traversals.modify(firstWeek, m -> m.applySeasonalAdjustment(0.9), metrics);
        List<DailyMetric> step2 = Traversals.modify(middleWeeks, m -> m.applySeasonalAdjustment(1.0), step1);
        List<DailyMetric> adjusted = Traversals.modify(lastWeek, m -> m.applySeasonalAdjustment(1.15), step2);
        ...
        System.out.printf("Adjustment impact: %+.1f%%%n", ((adjustedTotal - originalTotal) / originalTotal) * 100);
    ...
    private static void demonstrateOutlierHandling(List<DailyMetric> metrics) {
        ...
        Traversal<List<DailyMetric>, DailyMetric> middlePortion = ListTraversals.slicing(5, metrics.size() - 5);
        ...
        double variance = baseline.stream().mapToDouble(m -> Math.pow(m.revenue() - mean, 2)).average().orElse(0.0);
        ...
        long outlierCount = metrics.stream().filter(m -> m.revenue() < lowerBound || m.revenue() > upperBound).count();
        System.out.printf("%nOutliers detected: %d days%n", outlierCount);
        // Smooth outliers by capping to bounds
        Lens<DailyMetric, Double> revenueLens = Lens.of(DailyMetric::revenue, (m, newRev) -> new DailyMetric(m.date(), newRev, m.transactions(), m.avgOrderValue(), m.adjusted()));
        Traversal<List<DailyMetric>, Double> allRevenues = Traversals.<DailyMetric>forList().andThen(revenueLens.asTraversal());
        List<DailyMetric> smoothed = Traversals.modify(allRevenues, rev -> Math.max(lowerBound, Math.min(upperBound, rev)), metrics);
        double originalVariance = metrics.stream().mapToDouble(m -> Math.pow(m.revenue() - mean, 2)).average().orElse(0.0);
        double smoothedVariance = smoothed.stream().mapToDouble(m -> Math.pow(m.revenue() - mean, 2)).average().orElse(0.0);
        ...
        System.out.printf("Variance reduction: %.1f%%%n", ((originalVariance - smoothedVariance) / originalVariance) * 100);
        System.out.println();
