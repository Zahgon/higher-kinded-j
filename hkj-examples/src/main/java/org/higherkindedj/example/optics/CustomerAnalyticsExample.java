...
public class CustomerAnalyticsExample {
    ...
    public record AnalyticsReport(int totalCustomers, int vipCustomers, double totalRevenue, double averageSpending, List<String> highValueCustomers) {
    }
    ...
    private static List<Customer> createSampleCustomers() {
        return List.of(new Customer("Alice", List.of(new Purchase("Laptop", 1200.0, "Electronics", true), new Purchase("Headphones", 150.0, "Electronics", false)), false, 1350.0), new Customer("Bob", List.of(new Purchase("Book", 25.0, "Books", false), new Purchase("Coffee Maker", 80.0, "Appliances", false)), false, 105.0), new Customer("Charlie", List.of(new Purchase("Phone", 900.0, "Electronics", true), new Purchase("Tablet", 600.0, "Electronics", true), new Purchase("Case", 30.0, "Accessories", false)), true, 1530.0), new Customer("Diana", List.of(new Purchase("Desk", 400.0, "Furniture", false)), false, 400.0), new Customer("Eve", List.of(new Purchase("Monitor", 500.0, "Electronics", true), new Purchase("Keyboard", 120.0, "Electronics", false), new Purchase("Mouse", 60.0, "Electronics", false)), false, 680.0));
    }
    ...
    private static void demonstrateVipSegmentation(List<Customer> customers) {
        ...
        Traversal<List<Customer>, Customer> highSpenders = allCustomers.filtered(c -> c.totalSpending() > 1000);
        ...
        for (Customer customer : result) {
            System.out.println("  " + customer.name() + ": £" + customer.totalSpending() + " (VIP=" + customer.vip() + ")");
        }
    ...
    private static void demonstratePremiumBuyerAnalysis(List<Customer> customers) {
        ...
        Traversal<List<Customer>, Customer> premiumBuyers = allCustomers.filterBy(customerPurchases, Purchase::premium);
        ...
        for (Customer customer : premiumCustomers) {
            List<String> premiumItems = customer.purchases().stream().filter(Purchase::premium).map(Purchase::productName).toList();
    ...
    private static void demonstrateComplexFiltering(List<Customer> customers) {
        ...
        Traversal<List<Customer>, Customer> targetCustomers = allCustomers.filtered(c -> c.totalSpending() > 500).filtered(c -> !c.vip()).filtered(c -> c.purchases().size() > 1);
        ...
        for (Customer customer : targets) {
            System.out.println("  " + customer.name() + ": £" + customer.totalSpending() + " (" + customer.purchases().size() + " purchases)");
        }
    ...
    private static void demonstrateFilterByWithNestedQueries(List<Customer> customers) {
        ...
        Traversal<List<Customer>, Customer> bigElectronicsBuyers = allCustomers.filterBy(customerPurchases, p -> p.category().equals("Electronics") && p.amount() > 500);
        ...
        for (Customer customer : result) {
            List<String> expensiveElectronics = customer.purchases().stream().filter(p -> p.category().equals("Electronics") && p.amount() > 500).map(p -> p.productName() + " (£" + p.amount() + ")").toList();
    ...
    private static void demonstrateAggregationWithFilters(List<Customer> customers) {
        ...
        double vipRevenue = vipCustomersFold.foldMap(Monoids.doubleAddition(), Customer::totalSpending, customers);
        ...
        Fold<List<Customer>, Customer> frequentBuyers = allCustomersFold.filtered(c -> c.purchases().size() >= 3);
        double frequentBuyerRevenue = frequentBuyers.foldMap(Monoids.doubleAddition(), Customer::totalSpending, customers);
        ...
        Fold<Customer, Purchase> electronicsPurchases = purchasesFold.filtered(p -> p.category().equals("Electronics"));
        double totalElectronicsSpending = 0;
        for (Customer customer : customers) {
            totalElectronicsSpending += electronicsPurchases.foldMap(Monoids.doubleAddition(), Purchase::amount, customer);
        }
    ...
    private static void demonstrateCombinedFoldExtraction(List<Customer> customers) {
        ...
        Fold<List<Customer>, String> namesFold = allCustomersFold.andThen(Fold.of(c -> List.of(c.name())));
        // Extract all product names from all customers
        Fold<List<Customer>, String> productNamesFold = allCustomersFold.andThen(Fold.of(Customer::purchases)).andThen(Fold.of(p -> List.of(p.productName())));
        ...
        Fold<List<Customer>, Double> spendingFold = allCustomersFold.andThen(Fold.of(c -> List.of(c.totalSpending())));
        Fold<List<Customer>, Double> purchaseAmountsFold = allCustomersFold.andThen(Fold.of(Customer::purchases)).andThen(Fold.of(p -> List.of(p.amount())));
        ...
        System.out.println("Total monetary values (spending + purchase amounts): £" + String.format("%.2f", totalMonetary));
        System.out.println();
    }
    ...
    private static void demonstrateBulkUpdates(List<Customer> customers) {
        ...
        Traversal<List<Customer>, Customer> allCustomers = Traversals.forList();
        Lens<Customer, Double> spendingLens = Lens.of(Customer::totalSpending, (c, s) -> new Customer(c.name(), c.purchases(), c.vip(), s));
        // Apply 10% discount to high spenders (>£1000)
        Traversal<List<Customer>, Double> highSpenderSpending = allCustomers.filtered(c -> c.totalSpending() > 1000).andThen(spendingLens.asTraversal());
        List<Customer> afterDiscount = Traversals.modify(highSpenderSpending, spending -> spending * 0.9, customers);
        ...
        for (int i = 0; i < customers.size(); i++) {
            ...
            if (before.totalSpending() != after.totalSpending()) {
                System.out.println("  " + after.name() + ": £" + before.totalSpending() + " -> £" + after.totalSpending() + " (DISCOUNTED)");
            } else {
                System.out.println("  " + after.name() + ": £" + after.totalSpending() + " (unchanged)");
            }
        }
        // Apply loyalty bonus to frequent buyers (3+ purchases)
        Traversal<List<Customer>, Customer> frequentBuyers = allCustomers.filtered(c -> c.purchases().size() >= 3);
        List<Customer> afterBonus = Traversals.modify(frequentBuyers, c -> c.applyDiscount(5), customers);
        System.out.println("\nAfter applying 5% loyalty bonus to frequent buyers (3+ purchases):");
        for (int i = 0; i < customers.size(); i++) {
            Customer before = customers.get(i);
            Customer after = afterBonus.get(i);
            if (before.totalSpending() != after.totalSpending()) {
                System.out.println("  " + after.name() + ": £" + before.totalSpending() + " -> £" + after.totalSpending() + " (BONUS APPLIED)");
            } else {
                System.out.println("  " + after.name() + ": £" + after.totalSpending() + " (unchanged)");
            }
        }
