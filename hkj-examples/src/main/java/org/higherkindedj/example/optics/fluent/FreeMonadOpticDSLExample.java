...
public class FreeMonadOpticDSLExample {
    ...
    private static void basicProgramExamples(UserProfile profile) {
        ...
        Free<OpticOpKind.Witness, String> getProgram = OpticPrograms.get(profile, UserProfileLenses.username());
        ...
        Free<OpticOpKind.Witness, UserProfile> setProgram = OpticPrograms.set(profile, UserProfileLenses.email(), "new.email@example.com");
        ...
        Free<OpticOpKind.Witness, UserProfile> modifyProgram = OpticPrograms.modify(profile, UserProfileLenses.settings().andThen(ProfileSettingsLenses.emailNotifications()), notifications -> !notifications);
    ...
    private static void composedProgramExamples(UserProfile profile) {
        ...
        Free<OpticOpKind.Witness, UserProfile> conditionalProgram = OpticPrograms.get(profile, UserProfileLenses.settings().andThen(ProfileSettingsLenses.emailNotifications())).flatMap(emailEnabled -> {
            if (emailEnabled) {
                System.out.println("  Email notifications enabled, enabling SMS too");
                return OpticPrograms.set(profile, UserProfileLenses.settings().andThen(ProfileSettingsLenses.smsNotifications()), true);
            } else {
        ...
        System.out.println("  Result - Email: " + result.settings().emailNotifications() + ", SMS: " + result.settings().smsNotifications());
        System.out.println();
        System.out.println("Example 2: Multi-step transformation");
        // Program: Update theme and language together
        Free<OpticOpKind.Witness, UserProfile> multiStepProgram = updateThemeAndLanguage(profile, "dark", "en-GB");
    ...
    private static void interpreterComparison(UserProfile profile) {
        ...
        Free<OpticOpKind.Witness, UserProfile> complexProgram = OpticPrograms.get(profile, UserProfileLenses.email()).flatMap(currentEmail -> {
            String normalised = currentEmail.toLowerCase().trim();
            return OpticPrograms.set(profile, UserProfileLenses.email(), normalised);
        }).flatMap(updated -> OpticPrograms.modify(updated, UserProfileLenses.settings().andThen(ProfileSettingsLenses.emailNotifications()), notifications -> true));
     ...
     */
    private static Free<OpticOpKind.Witness, UserProfile> updateThemeAndLanguage(UserProfile profile, String theme, String language) {
        return OpticPrograms.set(profile, UserProfileLenses.settings().andThen(ProfileSettingsLenses.theme()), theme).flatMap(updated -> OpticPrograms.set(updated, UserProfileLenses.settings().andThen(ProfileSettingsLenses.preferredLanguage()), language));
    }
    ...
    private static Free<OpticOpKind.Witness, UserProfile> buildMigrationProgram(UserProfile profile) {
        ...
        return OpticPrograms.modify(profile, UserProfileLenses.email(), String::toLowerCase).flatMap(p1 -> normaliseAddresses(p1)).flatMap(p2 -> OpticPrograms.set(p2, UserProfileLenses.settings().andThen(ProfileSettingsLenses.preferredLanguage()), "en-GB")).flatMap(p3 -> OpticPrograms.set(p3, UserProfileLenses.settings().andThen(ProfileSettingsLenses.emailNotifications()), true));
    }
    ...
    private static Free<OpticOpKind.Witness, UserProfile> normaliseAddresses(UserProfile profile) {
        Traversal<UserProfile, String> allPostCodes = UserProfileTraversals.addresses().andThen(UserAddressLenses.postCode().asTraversal());
    ...
    private static UserProfile createSampleProfile() {
        ...
        return new UserProfile("USR-001", "alice_smith", "Alice.Smith@Example.COM", settings, addresses);
    }
    ...
    private static void printProfile(UserProfile profile) {
        ...
        profile.addresses().forEach(addr -> System.out.println("    " + addr.type() + ": " + addr.street() + ", " + addr.city() + " " + addr.postCode()));
    }
}
