...
public class PredicateListTraversalsExample {
    ...
    private static void demonstrateTakingWhile() {
        ...
        List<Product> products = List.of(new Product("SKU001", "Widget", 10.0, 100), new Product("SKU002", "Gadget", 15.0, 50), // Stops here
        new Product("SKU003", "Gizmo", 25.0, 75), // Not included
        new Product("SKU004", "Thing", 12.0, 25));
        Traversal<List<Product>, Product> affordablePrefix = ListTraversals.takingWhile(p -> p.price() < 20.0);
        System.out.println("Original products:");
        products.forEach(p -> System.out.println("  " + p));
        // Discount affordable prefix
        List<Product> discounted = Traversals.modify(affordablePrefix, p -> new Product(p.sku(), p.name(), p.price() * 0.9, p.stock()), products);
    ...
    private static void demonstrateDroppingWhile() {
        ...
        List<Product> products = List.of(new Product("SKU001", "Widget", 10.0, 20), new Product("SKU002", "Gadget", 25.0, 30), // First well-stocked
        new Product("SKU003", "Gizmo", 15.0, 75), // Included
        new Product("SKU004", "Thing", 12.0, 25));
        Traversal<List<Product>, Product> wellStocked = ListTraversals.droppingWhile(p -> p.stock() < 50);
        System.out.println("Original products:");
        products.forEach(p -> System.out.println("  " + p));
        // Restock well-stocked items (and everything after first well-stocked)
        List<Product> restocked = Traversals.modify(wellStocked, p -> new Product(p.sku(), p.name(), p.price(), p.stock() + 50), products);
        ...
        List<String> logLines = List.of("[CONFIG] Loading application.properties", "[CONFIG] Initialising database pool", "[INFO] Application started", "[WARN] High memory usage", "[ERROR] Connection timeout");
        Traversal<List<String>, String> runtimeLogs = ListTraversals.droppingWhile(line -> line.startsWith("[CONFIG]"));
    ...
    private static void demonstrateElement() {
        ...
        List<Product> products = List.of(new Product("SKU001", "Widget", 10.0, 100), new Product("SKU002", "Gadget", 25.0, 50), new Product("SKU003", "Gizmo", 15.0, 75));
        ...
        List<Product> updated = Traversals.modify(thirdProduct, p -> new Product(p.sku(), p.name(), p.price() * 0.8, p.stock()), products);
        ...
        List<List<Integer>> nested = List.of(List.of(1, 2, 3), List.of(10, 20, 30), List.of(100, 200, 300));
        Traversal<List<List<Integer>>, List<Integer>> secondList = ListTraversals.element(1);
        Traversal<List<List<Integer>>, Integer> secondListFirstElement = secondList.andThen(ListTraversals.element(0));
    ...
    private static void demonstrateComposedPredicateTraversals() {
        ...
        List<Product> products = List.of(// No stock
        new Product("SKU001", "Widget", 10.0, 0), new Product("SKU002", "Gadget", 15.0, 50), new Product("SKU003", "Gizmo", 25.0, 75), new Product("SKU004", "Thing", 12.0, 100), new Product("SKU005", "Doodad", 30.0, 25), new Product("SKU006", "Thingamajig", 8.0, 80));
        // Combine taking with filtering - take first 4, then filter by stock and price
        Traversal<List<Product>, Product> topAffordableInStock = ListTraversals.<Product>taking(4).filtered(p -> p.stock() > 0).filtered(p -> p.price() < 20.0);
        System.out.println("All products:");
        for (int i = 0; i < products.size(); i++) {
        ...
        List<Product> discounted = Traversals.modify(topAffordableInStock, p -> new Product(p.sku(), p.name(), p.price() * 0.85, p.stock()), products);
    ...
    private static void demonstrateTimeSeriesProcessing() {
        ...
        List<Transaction> transactions = List.of(new Transaction("TX001", now.minusHours(48), 100.0, "COMPLETED"), new Transaction("TX002", now.minusHours(36), 200.0, "COMPLETED"), new Transaction("TX003", now.minusHours(12), 150.0, "PENDING"), new Transaction("TX004", now.minusHours(6), 300.0, "PENDING"), new Transaction("TX005", now.minusHours(1), 250.0, "PENDING"));
        Traversal<List<Transaction>, Transaction> beforeCutoff = ListTraversals.takingWhile(t -> t.timestamp().isBefore(cutoff));
        System.out.println("All transactions:");
        transactions.forEach(t -> System.out.println("  " + t));
        // Process old transactions
        List<Transaction> processed = Traversals.modify(beforeCutoff, t -> new Transaction(t.id(), t.timestamp(), t.amount(), "ARCHIVED"), transactions);
    ...
    private static void demonstratePriorityQueueHandling() {
        ...
        System.out.println("--------------------------------------");
        List<Task> tasks = List.of(new Task("T1", 1, "PENDING"), new Task("T2", 1, "PENDING"), // Priority changes here
        new Task("T3", 2, "PENDING"), new Task("T4", 2, "PENDING"), new Task("T5", 3, "PENDING"));
        ...
        List<Task> processed = Traversals.modify(highPriority, t -> new Task(t.id(), t.priority(), "COMPLETED"), tasks);
    ...
    private static void demonstrateBatchProcessing() {
        ...
        System.out.println("----------------------------------------------");
        LocalDateTime now = LocalDateTime.now();
        List<LogEntry> logs = List.of(new LogEntry(now.minusMinutes(10), "INFO", "WARMUP: Initialising cache"), new LogEntry(now.minusMinutes(9), "INFO", "WARMUP: Loading data"), new LogEntry(now.minusMinutes(8), "INFO", "INIT: System ready"), new LogEntry(now.minusMinutes(7), "INFO", "Processing request 1"), new LogEntry(now.minusMinutes(6), "WARN", "High latency detected"), new LogEntry(now.minusMinutes(5), "ERROR", "Connection timeout"));
        Traversal<List<LogEntry>, LogEntry> steadyState = ListTraversals.droppingWhile(LogEntry::isWarmup);
