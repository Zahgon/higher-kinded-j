...
public class VStreamOpticsExample {
    ...
    // Lenses
    static final Lens<Config, VStream<Rule>> RULES_LENS = Lens.of(Config::rules, (config, rules) -> new Config(config.env(), rules));

    static final Lens<Rule, String> RULE_NAME_LENS = Lens.of(Rule::name, (rule, name) -> new Rule(name, rule.priority()));

    static final Lens<Rule, Integer> RULE_PRIORITY_LENS = Lens.of(Rule::priority, (rule, priority) -> new Rule(rule.name(), priority));
    ...
    private static void demonstrateFocusDSLIntegration() {
        ...
        Config config = new Config("production", VStream.fromList(List.of(new Rule("auth-check", 1), new Rule("rate-limit", 2), new Rule("cors-allow", 3))));
        // Get all rule names
    ...
    private static void demonstrateToVStreamPath() {
        ...
        List<Integer> result = vstreamPath.filter(n -> n % 2 == 0).map(n -> n * 10).take(3).toList().unsafeRun();
