...
public class NearlyPrismExample {
    ...
    private static void demonstrateValidationPatterns() {
        ...
        Prism<String, Unit> validEmail = Prisms.nearly("user@example.com", s -> s.contains("@") && s.contains("."));
        // Age validation
        Prism<Integer, Unit> adultAge = Prisms.nearly(18, age -> age >= 18 && age <= 120);
        // Password strength validation
        Prism<String, Unit> strongPassword = Prisms.nearly("SecureP@ss1", pwd -> pwd.length() >= 8 && pwd.chars().anyMatch(Character::isUpperCase) && pwd.chars().anyMatch(Character::isDigit));
        // Test emails
    ...
    private static void demonstrateComposition() {
        ...
        List<Optional<String>> testCases = List.of(Optional.of("hello"), Optional.of(""), Optional.empty());
