...
public class StringTraversalsExample {
    ...
    private static void demonstrateCharacterTraversals() {
        ...
        System.out.println("Character 'a' frequency: " + allChars.stream().filter(c -> c == 'a').count() + " times");
        // ROT13 cipher demonstration
        Traversal<String, Character> letters = chars.filtered(c -> Character.isLetter(c));
        String secret = "hello";
        String encoded = Traversals.modify(letters, c -> {
    ...
    private static void demonstrateWordTraversals() {
        ...
        String titleCase = Traversals.modify(words, word -> word.substring(0, 1).toUpperCase() + word.substring(1).toLowerCase(), rawTitle);
    ...
    private static void demonstrateLineTraversals() {
        ...
        String warningsAndErrors = Traversals.modify(lines.filtered(line -> line.contains("ERROR") || line.contains("WARN")), String::toUpperCase, logContent);
    ...
    private static void demonstrateEmailNormalisation() {
        ...
        System.out.println("----------------------------------------");
        String[] emails = { "  User.Name@EXAMPLE.COM  ", "admin@TEST-DOMAIN.org", "contact@My-Company.CO.UK  " };
        Traversal<String, String> words = StringTraversals.worded();
    ...
    private static void demonstrateCsvProcessing() {
        ...
        String transformed = Traversals.modify(dataLines, line -> {
    ...
    private static void demonstrateConfigurationSanitisation() {
        ...
        Traversal<String, String> lines = StringTraversals.lined();
        // Remove comments
        Traversal<String, String> nonCommentLines = lines.filtered(line -> !line.trim().startsWith("#") && !line.trim().isEmpty());
        String withoutComments = Traversals.getAll(nonCommentLines, configFile).stream().collect(Collectors.joining("\n"));
        System.out.println("Without comments:");
        System.out.println(withoutComments);
        // Redact sensitive values
        String redacted = Traversals.modify(lines, line -> {
        ...
        String trimmed = Traversals.modify(nonCommentLines, line -> {
