...
public class CoreTypePrismsExample {
    ...
    private static void demonstrateMaybePrisms() {
        ...
        ApiResponse response = new ApiResponse("req-123", Maybe.just(new User("u1", "Alice", "alice@example.com", 30)), List.of());
        Prism<Maybe<User>, User> userPrism = Prisms.just();
        Optional<User> user = userPrism.getOptional(response.data());
        System.out.println("User from response: " + user.map(u -> u.name() + " (" + u.email() + ")").orElse("No user"));
        System.out.println();
    }
    ...
    private static void demonstrateEitherPrisms() {
        ...
        List<Either<String, User>> results = List.of(Either.right(new User("u1", "Alice", "alice@example.com", 30)), Either.left("Invalid email format"), Either.right(new User("u2", "Bob", "bob@example.com", 25)));
        Prism<Either<String, User>, User> userRightPrism = Prisms.right();
        long successCount = results.stream().filter(r -> userRightPrism.getOptional(r).isPresent()).count();
    ...
    private static void demonstrateValidatedPrisms() {
        ...
        List<Validated<String, User>> validationResults = List.of(Validated.valid(new User("u1", "Alice", "alice@example.com", 30)), Validated.invalid("Email already exists"), Validated.invalid("Age must be at least 18"), Validated.valid(new User("u2", "Bob", "bob@example.com", 25)));
        Prism<Validated<String, User>, User> userValidPrism = Prisms.valid();
        Prism<Validated<String, User>, String> errorPrism = Prisms.invalid();
        List<User> validUsers = validationResults.stream().flatMap(v -> userValidPrism.getOptional(v).stream()).toList();
        List<String> errors = validationResults.stream().flatMap(v -> errorPrism.getOptional(v).stream()).toList();
    ...
    private static void demonstrateTryPrisms() {
        ...
        System.out.println("Exception: " + failurePrism.getOptional(successResult).map(Throwable::getMessage).orElse("None"));
        // Failure case
        ...
        System.out.println("Exception: " + failurePrism.getOptional(failureResult).map(Throwable::getMessage).orElse("None"));
        ...
        System.out.println("User loaded: " + userSuccessPrism.getOptional(dbResult).map(u -> u.name()).orElse("Failed to load"));
        System.out.println();
    }
    ...
    private static void demonstrateMaybeTraversals() {
        ...
        ApiResponse response = new ApiResponse("req-456", Maybe.just(new User("u1", "Alice", "alice@example.com", 30)), List.of());
    ...
    private static void demonstrateEitherTraversals() {
        ...
        Either<String, Integer> enriched = Traversals.modify(leftTraversal, msg -> "[ERROR] " + msg, error);
    ...
    private static void demonstrateComposition() {
        ...
        ApiResponse response = new ApiResponse("req-789", Maybe.just(new User("u1", "Alice", "alice@example.com", 30)), List.of("Deprecated API"));
    ...
    private static void demonstrateBeforeAfterComparison() {
        ...
        if (tryUser.isSuccess()) {
            System.out.println("  User: " + tryUser.fold(u -> u.name(), ex -> "Failed: " + ex.getMessage()));
        } else {
        ...
        System.out.println("  User: " + successPrism.getOptional(tryUser).map(User::name).orElse("Failed to load"));
        System.out.println("  Error: " + failurePrism.getOptional(tryUser).map(Throwable::getMessage).orElse("None"));
        System.out.println();
    }
