...
public class DataValidationPipelineExample {
    ...
    @GenerateLenses
    public record Invalid(RawCustomer originalValue, List<String> errors) implements ValidationResult {
    }
    ...
    public record RawCustomer(String id, String name, String email, String dateOfBirth, String status) {
    }

    public record ValidatedCustomer(String id, String name, String email, LocalDate dateOfBirth, String status) {
    }
    ...
    private static void demonstrateBasicValidation() {
        ...
        RawCustomer validCustomer = new RawCustomer("C001", "Alice Smith", "alice@example.com", "1990-05-15", "active");
        RawCustomer invalidCustomer = new RawCustomer("C002", "", "not-an-email", "invalid-date", "active");
    ...
    private static void demonstrateDataCleansing() {
        ...
        RawCustomer messyCustomer = new RawCustomer(" c003 ", "  bob jones  ", " BOB@EXAMPLE.COM ", "1985-03-20", " ACTIVE ");
        ValidationResult result = new Valid(messyCustomer);
        // Clean up whitespace and normalise case
        ValidationResult cleaned = VALID.modify(valid -> new Valid(new RawCustomer(valid.value().id().trim().toLowerCase(), normaliseName(valid.value().name()), valid.value().email().trim().toLowerCase(), valid.value().dateOfBirth().trim(), valid.value().status().trim().toLowerCase())), result);
        System.out.println("Original: " + messyCustomer);
        System.out.println("Cleaned:  " + VALID.mapOptional(Valid::value, cleaned).orElse(messyCustomer));
        // Conditionally fix common email typos
        RawCustomer typoCustomer = new RawCustomer("C004", "Charlie", "charlie@gmial.com", "1992-07-10", "active");
        ValidationResult typoResult = new Valid(typoCustomer);
        ValidationResult fixed = VALID.modifyWhen(valid -> valid.value().email().contains("@gmial."), valid -> new Valid(new RawCustomer(valid.value().id(), valid.value().name(), valid.value().email().replace("@gmial.", "@gmail."), valid.value().dateOfBirth(), valid.value().status())), typoResult);
        System.out.println("Email with typo: " + typoCustomer.email());
        System.out.println("Fixed email:     " + VALID.mapOptional(v -> v.value().email(), fixed).orElse("N/A"));
        System.out.println();
    }

    private static void demonstrateValidationPipeline() {
        System.out.println("--- Multi-Stage Validation Pipeline ---");
        List<RawCustomer> rawData = List.of(new RawCustomer("C001", "Alice", "alice@example.com", "1990-05-15", "active"), new RawCustomer("C002", "", "bob@example.com", "1985-03-20", "active"), new RawCustomer("C003", "Charlie", "charlie@example.com", "not-a-date", "inactive"), new RawCustomer("", "Dave", "dave@example.com", "1992-07-10", "active"));
    ...
    private static void demonstrateErrorReporting() {
        ...
        RawCustomer customer = new RawCustomer("C005", "Eve", "eve@example.com", "invalid-date", "active");
        ValidationResult result = validateCustomer(customer);
        // Extract first error or warning using mapOptional
        String errorMessage = INVALID.mapOptional(inv -> inv.errors().isEmpty() ? null : inv.errors().get(0), result).or(() -> WARNING.mapOptional(warn -> warn.warnings().isEmpty() ? null : warn.warnings().get(0), result)).orElse("No errors or warnings");
        ...
        RawCustomer original = VALID.mapOptional(Valid::value, result).or(() -> INVALID.mapOptional(Invalid::originalValue, result)).or(() -> WARNING.mapOptional(Warning::value, result)).or(() -> SKIPPED.mapOptional(Skipped::value, result)).orElse(new RawCustomer("UNKNOWN", "", "", "", ""));
    ...
    private static void demonstrateBatchValidation() {
        ...
        List<RawCustomer> batch = List.of(new RawCustomer("C001", "Alice", "alice@example.com", "1990-05-15", "active"), new RawCustomer("C002", "", "bob@example.com", "1985-03-20", "active"), new RawCustomer("C003", "Charlie", "charlie@example.com", "2020-01-01", "active"), new RawCustomer("C004", "Dave", "dave@example.com", "invalid", "inactive"), new RawCustomer("C005", "Eve", "eve@example.com", "1995-12-25", "active"));
        List<ValidationResult> results = batch.stream().map(DataValidationPipelineExample::validateCustomer).toList();
    ...
    private static ValidationResult validateCustomer(RawCustomer customer) {
        ...
        // Validate email
        if (customer.email() == null || !customer.email().matches("^[A-Za-z0-9+_.-]+@[A-Za-z0-9.-]+$")) {
    ...
    private static String normaliseName(String name) {
        ...
        return Arrays.stream(name.trim().split("\\s+")).map(word -> word.isEmpty() ? word : word.substring(0, 1).toUpperCase() + word.substring(1).toLowerCase()).collect(Collectors.joining(" "));
    }
}
