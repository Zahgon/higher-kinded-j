...
public class FluentValidationExample {
    ...
    // ============================================================================
    // UserProfile lenses
    static final Lens<UserProfile, String> USERNAME = Lens.of(UserProfile::username, (u, v) -> new UserProfile(v, u.email(), u.age(), u.bio()));

    static final Lens<UserProfile, String> EMAIL = Lens.of(UserProfile::email, (u, v) -> new UserProfile(u.username(), v, u.age(), u.bio()));

    static final Lens<UserProfile, Integer> AGE = Lens.of(UserProfile::age, (u, v) -> new UserProfile(u.username(), u.email(), v, u.bio()));

    static final Lens<UserProfile, String> BIO = Lens.of(UserProfile::bio, (u, v) -> new UserProfile(u.username(), u.email(), u.age(), v));

    // OrderForm lenses and traversals
    static final Lens<OrderForm, String> ORDER_ID = Lens.of(OrderForm::orderId, (o, v) -> new OrderForm(v, o.itemPrices(), o.customerEmail()));

    static final Lens<OrderForm, List<BigDecimal>> ITEM_PRICES = Lens.of(OrderForm::itemPrices, (o, v) -> new OrderForm(o.orderId(), v, o.customerEmail()));

    static final Lens<OrderForm, String> CUSTOMER_EMAIL = Lens.of(OrderForm::customerEmail, (o, v) -> new OrderForm(o.orderId(), o.itemPrices(), v));

    static final Traversal<OrderForm, BigDecimal> ALL_ITEM_PRICES = new Traversal<OrderForm, BigDecimal>() {

        @Override
        public <F extends WitnessArity<TypeArity.Unary>> Kind<F, OrderForm> modifyF(Function<BigDecimal, Kind<F, BigDecimal>> f, OrderForm source, Applicative<F> app) {
            throw new UnsupportedOperationException("STUB: not implemented");
        }
    };

    // AppConfig lenses
    static final Lens<AppConfig, String> API_ENDPOINT = Lens.of(AppConfig::apiEndpoint, (c, v) -> new AppConfig(v, c.maxConnections(), c.timeoutSeconds(), c.logLevel()));

    static final Lens<AppConfig, Integer> MAX_CONNECTIONS = Lens.of(AppConfig::maxConnections, (c, v) -> new AppConfig(c.apiEndpoint(), v, c.timeoutSeconds(), c.logLevel()));

    static final Lens<AppConfig, Integer> TIMEOUT_SECONDS = Lens.of(AppConfig::timeoutSeconds, (c, v) -> new AppConfig(c.apiEndpoint(), c.maxConnections(), v, c.logLevel()));

    static final Lens<AppConfig, String> LOG_LEVEL = Lens.of(AppConfig::logLevel, (c, v) -> new AppConfig(c.apiEndpoint(), c.maxConnections(), c.timeoutSeconds(), v));

    // DataImport traversal
    static final Traversal<DataImport, String> ALL_EMAILS = new Traversal<DataImport, String>() {

        @Override
        public <F extends WitnessArity<TypeArity.Unary>> Kind<F, DataImport> modifyF(Function<String, Kind<F, String>> f, DataImport source, Applicative<F> app) {
            throw new UnsupportedOperationException("STUB: not implemented");
        }
    };
    ...
    private static void staticMethodExamples() {
        ...
        UserProfile validUser = new UserProfile("alice123", "alice@example.com", 25, "Software engineer");
        Either<String, UserProfile> emailResult = OpticOps.modifyEither(validUser, EMAIL, FluentValidationExample::validateEmail);
        emailResult.fold(error -> {
        ...
        Either<String, UserProfile> invalidEmailResult = OpticOps.modifyEither(invalidUser, EMAIL, FluentValidationExample::validateEmail);
        invalidEmailResult.fold(error -> {
            System.out.println("  ✗ Validation failed: " + error);
            return null;
        }, user -> {
            System.out.println("  ✓ Email validated: " + user.email());
        ...
        UserProfile userWithBio = new UserProfile("charlie789", "charlie@example.com", 28, "  Product manager  ");
        Maybe<UserProfile> bioResult = OpticOps.modifyMaybe(userWithBio, BIO, FluentValidationExample::validateBioOptional);
        ...
        Maybe<UserProfile> emptyBioResult = OpticOps.modifyMaybe(userWithEmptyBio, BIO, FluentValidationExample::validateBioOptional);
        ...
        OrderForm order = new OrderForm("ORD-001", List.of(new BigDecimal("50.00"), // Invalid: negative
        new BigDecimal("-10.00"), // Invalid: exceeds max
        new BigDecimal("15000.00"), new BigDecimal("25.50")), "customer@example.com");
        Validated<List<String>, OrderForm> pricesResult = OpticOps.modifyAllValidated(order, ALL_ITEM_PRICES, FluentValidationExample::validatePrice);
        pricesResult.fold(errors -> {
        ...
        Either<String, OrderForm> pricesEitherResult = OpticOps.modifyAllEither(order, ALL_ITEM_PRICES, FluentValidationExample::validatePriceEither);
        pricesEitherResult.fold(error -> {
    ...
    private static void fluentBuilderExamples() {
        ...
        Either<String, UserProfile> usernameResult = OpticOps.modifyingWithValidation(user).throughEither(USERNAME, FluentValidationExample::validateUsername);
        usernameResult.fold(error -> {
        ...
        UserProfile userWithLongBio = new UserProfile("alice123", "alice@example.com", 25, "a".repeat(600));
        Maybe<UserProfile> longBioResult = OpticOps.modifyingWithValidation(userWithLongBio).throughMaybe(BIO, FluentValidationExample::validateBioOptional);
        ...
        DataImport importData = new DataImport(List.of("valid@example.com", "invalid-email", "another@valid.com", "bad@"), "admin");
        Validated<List<String>, DataImport> emailsResult = OpticOps.modifyingWithValidation(importData).allThroughValidated(ALL_EMAILS, FluentValidationExample::validateEmailForImport);
        emailsResult.fold(errors -> {
        ...
        OrderForm invalidOrder = new OrderForm("ORD-002", List.of(new BigDecimal("100.00"), new BigDecimal("-50.00"), new BigDecimal("200.00")), "buyer@example.com");
        Either<String, OrderForm> orderResult = OpticOps.modifyingWithValidation(invalidOrder).allThroughEither(ALL_ITEM_PRICES, FluentValidationExample::validatePriceEither);
        orderResult.fold(error -> {
    ...
    private static void userRegistrationScenario() {
        ...
        Either<String, UserProfile> step1 = OpticOps.modifyEither(registrationForm, USERNAME, FluentValidationExample::validateUsername);
        Either<String, UserProfile> step2 = step1.flatMap(profile -> OpticOps.modifyEither(profile, EMAIL, FluentValidationExample::validateEmail));
        Either<String, UserProfile> step3 = step2.flatMap(profile -> OpticOps.modifyEither(profile, AGE, FluentValidationExample::validateAge));
        step3.fold(error -> {
    ...
    private static void orderProcessingScenario() {
        ...
        OrderForm order = new OrderForm("ORD-12345", List.of(new BigDecimal("99.99"), new BigDecimal("149.50"), new BigDecimal("25.00"), new BigDecimal("299.99")), "customer@store.com");
        ...
        Validated<List<String>, OrderForm> validatedOrder = OpticOps.modifyingWithValidation(order).allThroughValidated(ALL_ITEM_PRICES, FluentValidationExample::validatePrice);
        validatedOrder.fold(errors -> {
            System.out.println("✗ Order validation failed:");
            errors.forEach(error -> System.out.println("  - " + error));
            return null;
        }, validOrder -> {
            System.out.println("✓ Order validated successfully!");
            BigDecimal total = validOrder.itemPrices().stream().reduce(BigDecimal.ZERO, BigDecimal::add);
    ...
    private static void configurationValidationScenario() {
        ...
        Either<String, AppConfig> validatedConfig = OpticOps.modifyEither(config, MAX_CONNECTIONS, FluentValidationExample::validateMaxConnections).flatMap(c -> OpticOps.modifyEither(c, TIMEOUT_SECONDS, FluentValidationExample::validateTimeout));
        validatedConfig.fold(error -> {
    ...
    private static void dataImportScenario() {
        ...
        DataImport importBatch = new DataImport(List.of("alice@example.com", "BOB@EXAMPLE.COM", "charlie.brown@test.org", "invalid.email", "diana@company.com", "@no-local-part.com"), "import-job-001");
        System.out.println("Importing " + importBatch.emails().size() + " emails...\n");
        Validated<List<String>, DataImport> result = OpticOps.modifyingWithValidation(importBatch).allThroughValidated(ALL_EMAILS, FluentValidationExample::validateEmailForImport);
        result.fold(errors -> {
            ...
            System.out.println("  Imported and normalized " + validImport.emails().size() + " email(s):");
            validImport.emails().forEach(email -> System.out.println("    - " + email));
            return null;
        });
    }
    ...
    private static void comparisonExamples() {
        ...
        OrderForm testOrder = new OrderForm("TEST-001", List.of(// Error 1: negative
        new BigDecimal("-100.00"), // Error 2: exceeds max
        new BigDecimal("20000.00"), // Valid
        new BigDecimal("50.00")), "test@example.com");
        ...
        Either<String, OrderForm> eitherResult = OpticOps.modifyAllEither(testOrder, ALL_ITEM_PRICES, FluentValidationExample::validatePriceEither);
        ...
        Validated<List<String>, OrderForm> validatedResult = OpticOps.modifyAllValidated(testOrder, ALL_ITEM_PRICES, FluentValidationExample::validatePrice);
        validatedResult.fold(errors -> {
