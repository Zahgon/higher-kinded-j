...
public class PartsOfTraversalExample {
    ...
    private static void basicPartsOfUsage() {
        ...
        List<Product> products = List.of(new Product("Widget", 25.99, 100), new Product("Gadget", 49.99, 50), new Product("Gizmo", 15.50, 200));
        // Create a traversal for all product prices
        Traversal<List<Product>, Double> priceTraversal = Traversals.<Product>forList().andThen(ProductLenses.price().asTraversal());
    ...
    private static void sortingWithPartsOf() {
        ...
        List<Product> products = List.of(new Product("Expensive Widget", 199.99, 10), new Product("Budget Gadget", 9.99, 500), new Product("Mid-range Gizmo", 59.99, 100), new Product("Premium Device", 299.99, 25));
    ...
    private static void reversingWithPartsOf() {
        ...
        Project project = new Project("Website Redesign", List.of(new Task("Design mockups", 1, false), new Task("Implement frontend", 2, false), new Task("Backend API", 3, false), new Task("Testing", 4, false), new Task("Deployment", 5, false)));
        // Create a traversal for task priorities
        Traversal<Project, Integer> priorityTraversal = ProjectTraversals.tasks().andThen(TaskLenses.priority().asTraversal());
        ...
        project.tasks().forEach(t -> System.out.println("  " + t.description() + " -> P" + t.priority()));
        System.out.println("\nAfter reversing priorities:");
        reversedProject.tasks().forEach(t -> System.out.println("  " + t.description() + " -> P" + t.priority()));
    ...
    private static void distinctWithPartsOf() {
        ...
        List<Product> products = List.of(new Product("Widget", 25.99, 100), new Product("Gadget", 49.99, 50), // Duplicate name
        new Product("Widget", 30.00, 75), new Product("Gizmo", 15.50, 200), // Duplicate name
        new Product("Gadget", 55.00, 30));
        // Create a traversal for product names
        Traversal<List<Product>, String> nameTraversal = Traversals.<Product>forList().andThen(ProductLenses.name().asTraversal());
        ...
        products.forEach(p -> System.out.println("  " + p.name() + " (£" + p.price() + ")"));
        System.out.println("\nAfter deduplication:");
        deduplicatedProducts.forEach(p -> System.out.println("  " + p.name() + " (£" + p.price() + ")"));
    ...
    private static void customComparatorSorting() {
        ...
        List<Product> products = List.of(new Product("zebra Widget", 25.99, 100), new Product("Alpha Gadget", 49.99, 50), new Product("BETA Device", 35.00, 75), new Product("gamma Tool", 15.50, 200));
        // Create a traversal for product names
        Traversal<List<Product>, String> nameTraversal = Traversals.<Product>forList().andThen(ProductLenses.name().asTraversal());
        // Sort names case-insensitively
        List<Product> sortedProducts = Traversals.sorted(nameTraversal, String.CASE_INSENSITIVE_ORDER, products);
        ...
        // Sort by name length
        List<Product> sortedByLength = Traversals.sorted(nameTraversal, Comparator.comparingInt(String::length), products);
        System.out.println("\nAfter sorting by name length:");
        sortedByLength.forEach(p -> System.out.println("  " + p.name() + " (length: " + p.name().length() + ")"));
        System.out.println();
    }
    ...
    private static void combiningWithFilteredTraversals() {
        ...
        List<Product> products = List.of(// Out of stock
        new Product("Widget A", 99.99, 0), new Product("Widget B", 25.99, 100), // Out of stock
        new Product("Widget C", 149.99, 0), new Product("Widget D", 49.99, 50), new Product("Widget E", 75.00, 200));
        // Create a traversal for prices of in-stock products only
        Traversal<List<Product>, Double> inStockPrices = Traversals.<Product>forList().filtered(p -> p.stockLevel() > 0).andThen(ProductLenses.price().asTraversal());
        // Sort only the in-stock prices
        List<Product> result = Traversals.sorted(inStockPrices, products);
        System.out.println("Original products:");
        products.forEach(p -> System.out.println("  " + p.name() + " -> £" + p.price() + " (stock: " + p.stockLevel() + ")"));
        System.out.println("\nAfter sorting only in-stock product prices:");
        result.forEach(p -> System.out.println("  " + p.name() + " -> £" + p.price() + " (stock: " + p.stockLevel() + ")"));
    ...
    private static void realWorldCatalogueNormalisation() {
        ...
        Catalogue catalogue = new Catalogue("Tech Store", List.of(new Category("Electronics", List.of(new Product("Laptop", 999.99, 20), new Product("Tablet", 499.99, 50), new Product("Phone", 799.99, 100))), new Category("Accessories", List.of(new Product("Charger", 29.99, 500), new Product("Case", 49.99, 300), new Product("Cable", 19.99, 1000)))));
        // Create a deep traversal to all product prices in the catalogue
        Traversal<Catalogue, Double> allPrices = CatalogueTraversals.categories().andThen(CategoryTraversals.products()).andThen(ProductLenses.price().asTraversal());
        ...
        // Only 3 values for 6 positions
        List<Double> partialPrices = List.of(100.00, 200.00, 300.00);
    ...
    private static void printCataloguePrices(Catalogue catalogue) {
        catalogue.categories().forEach(category -> {
            ...
            category.products().forEach(product -> System.out.println("    " + product.name() + ": £" + product.price()));
        });
        System.out.println();
    }
}
