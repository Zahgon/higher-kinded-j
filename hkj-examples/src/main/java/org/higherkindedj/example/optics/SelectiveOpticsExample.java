...
public class SelectiveOpticsExample {
    ...
    private static void conditionalLensExample() {
        ...
        Kind<IdKind.Witness, UserLogin> result = attemptsLens.modifyWhen(belowThreshold, attempts -> attempts + 1, userLogin, IdSelective.instance());
        ...
        Kind<IdKind.Witness, UserLogin> noChange = attemptsLens.modifyWhen(belowThreshold, attempts -> attempts + 1, maxedUserLogin, IdSelective.instance());
    ...
    private static void branchingLensExample() {
        ...
        Kind<IdKind.Witness, Account> positive = balanceLens.modifyBranch(isPositive, // 5% interest
        balance -> balance * 1.05, // $25 overdraft fee
        balance -> balance - 25.0, positiveAccount, IdSelective.instance());
        Kind<IdKind.Witness, Account> negative = balanceLens.modifyBranch(isPositive, balance -> balance * 1.05, balance -> balance - 25.0, negativeAccount, IdSelective.instance());
        System.out.println("Positive account: " + positiveAccount + " -> " + ID.narrow(positive).value());
        System.out.println("Negative account: " + negativeAccount + " -> " + ID.narrow(negative).value());
        System.out.println();
    }
    ...
    private static void conditionalTraversalExample() {
        ...
        Bank bank = new Bank("Premium Bank", List.of(new Account("ACC-001", 5000.0, true), new Account("ACC-002", 100.0, false), new Account("ACC-003", 10000.0, true), new Account("ACC-004", 500.0, false)));
        ...
        Kind<IdKind.Witness, Bank> updated = accountsTraversal.modifyWhen(isPremium, account -> Id.of(AccountLenses.balance().modify(// 10% bonus
        balance -> balance * 1.10, account)), bank, IdSelective.instance());
    ...
    private static void speculativeTraversalExample() {
        ...
        List<Account> accounts = List.of(new Account("ACC-001", 5000.0, true), new Account("ACC-002", 100.0, false), new Account("ACC-003", 10000.0, true));
        // Simulate expensive operations
        Function<Account, Kind<IdKind.Witness, Double>> premiumCalculation = account -> {
            System.out.println("  Calculating premium rate for " + account.id());
            // Simulate expensive operation
            return Id.of(account.balance() * 1.10);
        };
        Function<Account, Kind<IdKind.Witness, Double>> standardCalculation = account -> {
        ...
        Kind<IdKind.Witness, List<Double>> results = Traversals.speculativeTraverseList(accounts, Account::isPremium, premiumCalculation, standardCalculation, IdSelective.instance());
    ...
    private static void shortCircuitExample() {
        ...
        List<Account> accounts = List.of(new Account("ACC-001", 1000.0, false), // This will trigger stop
        new Account("ACC-002", -50.0, false), new Account("ACC-003", 2000.0, false), new Account("ACC-004", 3000.0, false));
        Predicate<Account> isOverdrawn = account -> account.balance() < 0;
        System.out.println("Processing accounts until overdraft found:");
        Kind<IdKind.Witness, List<Account>> results = Traversals.traverseListUntil(accounts, isOverdrawn, account -> {
    ...
    private static void comparativeExample() {
        ...
        Bank bank = new Bank("Test Bank", List.of(new Account("ACC-001", 1000.0, false), new Account("ACC-002", -50.0, false), new Account("ACC-003", -100.0, false)));
        Traversal<Bank, Account> accountsTraversal = BankTraversals.accounts();
        // Validation function
        Function<Account, Kind<ValidatedKind.Witness<String>, Account>> validator = account -> {
        ...
        Validated<String, Bank> applicativeResult = VALIDATED.narrow(accountsTraversal.modifyF(validator, bank, Instances.validated(Semigroups.string("; "))));
        ...
        Validated<String, Bank> selectiveResult = VALIDATED.narrow(accountsTraversal.modifyWhen(// Only validate if basic check passes
        isValid, // Only validate if basic check passes
        validator, bank, selective));
