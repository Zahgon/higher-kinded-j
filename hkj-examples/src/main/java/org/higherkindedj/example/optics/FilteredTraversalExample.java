...
public class FilteredTraversalExample {
    ...
    private static void demonstrateBasicFiltering() {
        ...
        List<User> users = List.of(new User("Alice", true, 100, SubscriptionTier.PREMIUM), new User("Bob", false, 200, SubscriptionTier.FREE), new User("Charlie", true, 150, SubscriptionTier.BASIC));
        ...
        for (User user : result) {
            System.out.println("  " + user.name() + ": score=" + user.score() + " (active=" + user.active() + ")");
        }
    ...
    private static void demonstrateFilteredComposition() {
        ...
        Traversal<List<User>, User> allUsers = Traversals.forList();
        Lens<User, String> nameLens = Lens.of(User::name, (u, n) -> new User(n, u.active(), u.score(), u.tier()));
        // Compose: list -> filtered active users -> name
        Traversal<List<User>, String> activeUserNames = allUsers.filtered(User::active).andThen(nameLens.asTraversal());
        List<User> users = List.of(new User("alice", true, 100, SubscriptionTier.PREMIUM), new User("bob", false, 200, SubscriptionTier.FREE), new User("charlie", true, 150, SubscriptionTier.BASIC));
    ...
    private static void demonstrateChainedFilters() {
        ...
        Traversal<List<User>, User> targetUsers = allUsers.filtered(User::active).filtered(user -> user.score() > 120).filtered(user -> user.tier() == SubscriptionTier.PREMIUM || user.tier() == SubscriptionTier.ENTERPRISE);
        List<User> users = List.of(// active but low score
        new User("Alice", true, 100, SubscriptionTier.PREMIUM), // high score but inactive
        new User("Bob", false, 200, SubscriptionTier.ENTERPRISE), new User("Charlie", true, 150, // active, high score, wrong tier
        SubscriptionTier.BASIC), // matches all criteria
        new User("Diana", true, 180, SubscriptionTier.PREMIUM), // matches all criteria
        new User("Eve", true, 200, SubscriptionTier.ENTERPRISE));
        ...
        for (User user : result) {
            System.out.println("  " + user.name() + " (score=" + user.score() + ", tier=" + user.tier() + ")");
        }
        System.out.println();
    }
    ...
    private static void demonstrateStaticCombinator() {
        ...
        Lens<User, String> nameLens = Lens.of(User::name, (u, n) -> new User(n, u.active(), u.score(), u.tier()));
        Traversal<List<User>, String> activeUserNames = Traversals.<User>forList().andThen(// Static combinator
        Traversals.filtered(User::active)).andThen(nameLens.asTraversal());
        List<User> users = List.of(new User("alice", true, 100, SubscriptionTier.PREMIUM), new User("bob", false, 200, SubscriptionTier.FREE));
        List<String> names = Traversals.getAll(activeUserNames, users);
    ...
    private static void demonstrateFilterByNested() {
        ...
        Traversal<List<Customer>, Customer> customersWithOverdue = allCustomers.filterBy(customerInvoices, Invoice::overdue);
        List<Customer> customers = List.of(new Customer("Alice", List.of(new Invoice("INV-001", 100.0, false), new Invoice("INV-002", 200.0, false))), new Customer("Bob", List.of(// OVERDUE!
        new Invoice("INV-003", 150.0, true), new Invoice("INV-004", 50.0, false))), new Customer("Charlie", List.of(new Invoice("INV-005", 300.0, false))));
        ...
        Traversal<List<Order>, Order> ordersWithExpensive = allOrders.filterBy(orderItems, item -> item.price() > 500);
        List<Order> orders = List.of(new Order(List.of(new Item("Apple", 50, "Food"), new Item("Banana", 30, "Food"))), new Order(List.of(new Item("Laptop", 1000, "Electronics"))), new Order(List.of(new Item("Book", 20, "Books"))));
    ...
    private static void demonstrateFoldFiltering() {
        ...
        Order order = new Order(List.of(new Item("Apple", 50, "Food"), new Item("Laptop", 1000, "Electronics"), new Item("Book", 25, "Books"), new Item("Phone", 800, "Electronics")));
        ...
        boolean allElectronics = expensiveItems.all(item -> item.category().equals("Electronics"), order);
    ...
    private static void demonstrateFilteredAsFold() {
        ...
        Fold<List<User>, User> premiumFold = allUsers.filtered(u -> u.tier() == SubscriptionTier.PREMIUM).asFold();
        List<User> users = List.of(new User("Alice", true, 300, SubscriptionTier.PREMIUM), new User("Bob", true, 150, SubscriptionTier.FREE), new User("Charlie", false, 250, SubscriptionTier.PREMIUM), new User("Diana", true, 180, SubscriptionTier.BASIC));
        // Use the full Fold API on the filtered result
        System.out.println("Premium users: " + premiumFold.getAll(users).stream().map(User::name).toList());
        ...
        Lens<User, String> nameLens = Lens.of(User::name, (u, n) -> new User(n, u.active(), u.score(), u.tier()));
        Fold<List<User>, String> activePremiumNames = allUsers.filtered(User::active).filtered(u -> u.tier() == SubscriptionTier.PREMIUM).asFold().andThen(nameLens.asFold());
