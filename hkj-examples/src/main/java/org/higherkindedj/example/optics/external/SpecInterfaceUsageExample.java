...
public class SpecInterfaceUsageExample {
    ...
    private static void demonstrateLocalDateOptics() {
        ...
        // December = 12
        LocalDate decemberDate = LocalDateOptics.monthValue().set(12, date);
        ...
        LocalDate endOfMonth = LocalDateOptics.dayOfMonth().set(date.getMonth().length(date.isLeapYear()), date);
        ...
        LocalDate newYearsDay = LocalDateOptics.monthValue().set(1, LocalDateOptics.dayOfMonth().set(1, LocalDateOptics.year().modify(y -> y + 1, date)));
    ...
    private static void demonstrateJsonNodeOptics() throws Exception {
        ...
        System.out.println("  name (text):     " + maybeName.map(StringNode::stringValue).orElse("not found"));
        ...
        System.out.println("  age (numeric):   " + maybeAge.map(NumericNode::intValue).map(String::valueOf).orElse("not found"));
        Optional<NumericNode> maybeBalance = JsonNodeOptics.numeric().getOptional(root.get("balance"));
        System.out.println("  balance (numeric):" + maybeBalance.map(NumericNode::doubleValue).map(String::valueOf).orElse("not found"));
        // Boolean prism - returns BooleanNode
        Optional<BooleanNode> maybeActive = JsonNodeOptics.bool().getOptional(root.get("active"));
        System.out.println("  active (bool):   " + maybeActive.map(BooleanNode::booleanValue).map(String::valueOf).orElse("not found"));
        System.out.println();
        ...
        System.out.println("  tags (array):    " + maybeTags.map(a -> a.size() + " elements").orElse("not found"));
        Optional<ObjectNode> maybeAddress = JsonNodeOptics.object().getOptional(root.get("address"));
        System.out.println("  address (object):" + maybeAddress.map(o -> o.size() + " fields").orElse("not found"));
        System.out.println();
        ...
        System.out.println("  name as numeric: " + nameAsNumeric.map(n -> String.valueOf(n.intValue())).orElse("empty (correct!)"));
        Optional<ArrayNode> ageAsArray = JsonNodeOptics.array().getOptional(root.get("age"));
        System.out.println("  age as array:    " + ageAsArray.map(ArrayNode::toString).orElse("empty (correct!)"));
        System.out.println();
        ...
        if (maybeAddress.isPresent()) {
            ...
            System.out.println("  address.city:    " + city.map(StringNode::stringValue).orElse("not found"));
        }
        System.out.println();
    }
    ...
    private static void demonstrateJooqOptics() {
        ...
        System.out.println();
        CustomerRecord customer = CustomerRecord.builder().id(1L).name("Alice Smith").email("alice@example.com").creditLimit(new BigDecimal("5000.00")).build();
        ...
        CustomerRecord newEmail = JooqStyleRecordOptics.email().set("alice.johnson@example.com", customer);
        System.out.println("  New email:    " + newEmail);
        System.out.println();
        // --- Using modify for transformations ---
        System.out.println("Modifying values:");
        CustomerRecord increased = JooqStyleRecordOptics.creditLimit().modify(limit -> limit.multiply(new BigDecimal("1.20")), customer);
        ...
        result.add(DSL.using(SQLDialect.DEFAULT).newRecord(DSL.field("id", Long.class), DSL.field("name", String.class), DSL.field("balance", BigDecimal.class)));
