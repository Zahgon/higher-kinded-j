...
public class FoldPlusCombinationExample {
    ...
    static final Lens<Department, List<Team>> deptTeamsLens = Lens.of(Department::teams, (d, t) -> new Department(d.name(), t));

    static final Lens<Team, Employee> teamLeadLens = Lens.of(Team::lead, (t, l) -> new Team(t.name(), l, t.members()));

    static final Lens<Employee, String> employeeEmailLens = Lens.of(Employee::email, (e, em) -> new Employee(e.name(), em));

    static final Lens<Employee, String> employeeNameLens = Lens.of(Employee::name, (e, n) -> new Employee(n, e.email()));
    ...
    private static void demonstrateLensPlusFold() {
        ...
        Fold<Team, String> memberNamesFold = Fold.<Team, Employee>of(Team::members).andThen(employeeNameLens.asFold());
        // Combine: get lead name AND all member names
        Fold<Team, String> allTeamNames = leadNameFold.plus(memberNamesFold);
        Team team = new Team("Platform", new Employee("Alice", "alice@co"), List.of(new Employee("Bob", "bob@co"), new Employee("Carol", "carol@co")));
    ...
    private static void demonstrateTraversalAsFoldPlus() {
        ...
        Lens<Team, List<Employee>> teamMembersLens = Lens.of(Team::members, (t, m) -> new Team(t.name(), t.lead(), m));
        Traversal<Team, Employee> membersTraversal = teamMembersLens.asTraversal().andThen(Traversals.forList());
        Fold<Team, String> memberEmailsFold = membersTraversal.asFold().andThen(employeeEmailLens.asFold());
    ...
    private static void demonstrateSumMultiplePaths() {
        ...
        Department dept = new Department("Engineering", List.of(new Team("Backend", new Employee("Alice", "alice@co"), List.of(new Employee("Bob", "bob@co"))), new Team("Frontend", new Employee("Carol", "carol@co"), List.of(new Employee("Dave", "dave@co")))));
        // Build folds to extract emails from different paths
        Fold<Department, Team> teamsFold = Fold.of(Department::teams);
        Fold<Department, String> leadEmails = teamsFold.andThen(teamLeadLens.asFold()).andThen(employeeEmailLens.asFold());
        Fold<Department, String> memberEmails = teamsFold.andThen(Fold.<Team, Employee>of(Team::members)).andThen(employeeEmailLens.asFold());
        // Combine using sum
    ...
    private static void demonstrateSealedInterfaceBranches() {
        ...
        Prism<ContactInfo, PhoneContact> phonePrism = Prism.of(c -> c instanceof PhoneContact p ? Optional.of(p) : Optional.empty(), c -> c);
        Prism<ContactInfo, EmailContact> emailPrism = Prism.of(c -> c instanceof EmailContact e ? Optional.of(e) : Optional.empty(), c -> c);
        // Extract the "value" from either branch
        Fold<ContactInfo, String> phoneFold = phonePrism.asFold().andThen(Fold.of(p -> List.of(p.number())));
        Fold<ContactInfo, String> emailFold = emailPrism.asFold().andThen(Fold.of(e -> List.of(e.address())));
    ...
    private static void demonstrateMonoidAggregation() {
        ...
        Lens<Product, Double> priceLens = Lens.of(Product::price, (p, v) -> new Product(p.name(), v, p.shippingCost()));
        Lens<Product, Double> shippingLens = Lens.of(Product::shippingCost, (p, v) -> new Product(p.name(), p.price(), v));
