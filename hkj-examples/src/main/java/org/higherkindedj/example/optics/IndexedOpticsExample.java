...
public class IndexedOpticsExample {
    ...
    private static void demonstrateListIndexing() {
        ...
        List<String> numbered = IndexedTraversals.imodify(indexed, (index, task) -> (index + 1) + ". " + task, tasks);
        System.out.println("\nNumbered tasks:");
    ...
    private static void demonstrateMapIndexing() {
        ...
        Map<String, Integer> withBonus = IndexedTraversals.imodify(indexed, (key, score) -> {
    ...
    private static void demonstratePositionBasedLogic() {
        ...
        List<LineItem> items = List.of(new LineItem("Laptop", 1, 999.99), new LineItem("Mouse", 2, 24.99), new LineItem("Keyboard", 1, 79.99), new LineItem("Monitor", 1, 299.99), new LineItem("Webcam", 1, 59.99), new LineItem("Headphones", 1, 149.99));
        System.out.println("Original items:");
        for (int i = 0; i < items.size(); i++) {
            System.out.printf("  Position %d: %s - £%.2f%n", i, items.get(i).productName(), items.get(i).price());
        }
        // Apply 10% discount to items at even positions (0, 2, 4...)
        List<LineItem> discounted = IndexedTraversals.imodify(itemsIndexed, (index, item) -> {
            if (index % 2 == 0) {
        ...
        List<LineItem> marked = IndexedTraversals.imodify(itemsIndexed, (index, item) -> {
            String prefix = "";
            if (index == 0)
                prefix = "[FIRST] ";
            if (index == lastIndex)
    ...
    private static void demonstrateIndexedFiltering() {
        ...
        IndexedTraversal<Integer, List<String>, String> oddPositions = indexed.filterIndex(i -> i % 2 == 1);
        ...
        List<LineItem> items = List.of(// Index 0, expensive
        new LineItem("Laptop", 1, 999.99), // Index 1, cheap
        new LineItem("Pen", 1, 2.99), // Index 2, mid-range
        new LineItem("Keyboard", 1, 79.99), // Index 3, cheap
        new LineItem("Mouse", 1, 24.99), // Index 4, expensive
        new LineItem("Monitor", 1, 299.99));
        IndexedTraversal<Integer, List<LineItem>, LineItem> itemsIndexed = IndexedTraversals.forList();
        // Filter: even positions AND price > £50
        IndexedTraversal<Integer, List<LineItem>, LineItem> evenAndExpensive = itemsIndexed.filterIndex(i -> i % 2 == 0).filtered(item -> item.price() > 50);
        List<Pair<Integer, LineItem>> filtered = IndexedTraversals.toIndexedList(evenAndExpensive, items);
        System.out.println("\nEven positions AND price > £50:");
        for (Pair<Integer, LineItem> pair : filtered) {
            System.out.printf("  Position %d: %s (£%.2f)%n", pair.first(), pair.second().productName(), pair.second().price());
        }
        System.out.println();
    }
    ...
    private static void demonstrateLensIndexing() {
        ...
        IndexedLens<String, Customer, String> emailLens = IndexedLens.of(// The index: field name
        "email", // Getter
        Customer::email, // Setter
        (customer, newEmail) -> new Customer(customer.name(), newEmail));
        ...
        Customer updated = emailLens.imodify((fieldName, oldValue) -> {
    ...
    private static void demonstrateAuditTrail() {
        ...
        IndexedLens<String, Customer, String> emailLens = IndexedLens.of("email", Customer::email, (c, email) -> new Customer(c.name(), email));
        ...
        for (FieldChange change : auditLog) {
            System.out.printf("  Field '%s' changed from '%s' to '%s' at %s%n", change.fieldName(), change.oldValue(), change.newValue(), change.timestamp());
        }
        System.out.println();
    }
    ...
    private static void demonstrateComposition() {
        ...
        IndexedTraversal<Integer, List<OrderWithItems>, OrderWithItems> ordersIndexed = IndexedTraversals.forList();
        // Second level: lens to items field
        Lens<OrderWithItems, List<LineItem>> itemsLens = Lens.of(OrderWithItems::items, (order, items) -> new OrderWithItems(order.id(), items));
        // Third level: indexed traversal for items
        IndexedTraversal<Integer, List<LineItem>, LineItem> itemsIndexed = IndexedTraversals.forList();
        // Compose: orders → items field → each item with PAIRED indices
        IndexedTraversal<Pair<Integer, Integer>, List<OrderWithItems>, LineItem> composed = ordersIndexed.andThen(itemsLens.asTraversal()).iandThen(itemsIndexed);
        List<OrderWithItems> orders = List.of(new OrderWithItems("ORD-1", List.of(new LineItem("Laptop", 1, 999.99), new LineItem("Mouse", 1, 24.99))), new OrderWithItems("ORD-2", List.of(new LineItem("Keyboard", 1, 79.99), new LineItem("Monitor", 1, 299.99))));
        // Access with paired indices: (order index, item index)
        List<Pair<Pair<Integer, Integer>, LineItem>> all = IndexedTraversals.toIndexedList(composed, orders);
        ...
        for (Pair<Pair<Integer, Integer>, LineItem> entry : all) {
            ...
            System.out.printf("  Order %d, Item %d: %s%n", indices.first(), indices.second(), item.productName());
        }
        // Modify with full path visibility
        List<OrderWithItems> updated = IndexedTraversals.imodify(composed, (indices, item) -> {
            int orderIdx = indices.first();
            int itemIdx = indices.second();
            System.out.printf("Processing [order=%d, item=%d]: %s%n", orderIdx, itemIdx, item.productName());
            return item;
        }, orders);
        System.out.println();
    }
    ...
    private static void demonstrateRealWorldScenario() {
        ...
        Order order = new Order("ORD-12345", List.of(new LineItem("Laptop", 1, 999.99), new LineItem("Mouse", 2, 24.99), new LineItem("Keyboard", 1, 79.99), new LineItem("Monitor", 1, 299.99)), new LinkedHashMap<>(Map.of("priority", "express", "gift-wrap", "true", "delivery-note", "Leave at door")));
        // Task 1: Generate packing slip
        System.out.println("Packing Slip for " + order.orderId() + ":");
        IndexedTraversal<Integer, List<LineItem>, LineItem> itemsIndexed = IndexedTraversals.forList();
        List<Pair<Integer, LineItem>> indexedItems = IndexedTraversals.toIndexedList(itemsIndexed, order.items());
        for (Pair<Integer, LineItem> pair : indexedItems) {
            // 1-based for display
            int position = pair.first() + 1;
            LineItem item = pair.second();
            System.out.printf("  Item %d: %s (Qty: %d) - £%.2f%n", position, item.productName(), item.quantity(), item.price() * item.quantity());
        }
        ...
        List<LineItem> discounted = IndexedTraversals.imodify(itemsIndexed, (index, item) -> {
            if ((index + 1) % 3 == 0) {
                ...
                System.out.printf("  Position %d (%s): £%.2f → £%.2f (15%% off)%n", index + 1, item.productName(), item.price(), newPrice);
                return new LineItem(item.productName(), item.quantity(), newPrice);
            }
            return item;
        }, order.items());
        double originalTotal = order.items().stream().mapToDouble(item -> item.price() * item.quantity()).sum();
        double discountedTotal = discounted.stream().mapToDouble(item -> item.price() * item.quantity()).sum();
        ...
        System.out.println("\nProcessing metadata:");
        IndexedTraversal<String, Map<String, String>, String> metadataIndexed = IndexedTraversals.forMap();
        List<Pair<String, String>> metadataEntries = IndexedTraversals.toIndexedList(metadataIndexed, order.metadata());
        for (Pair<String, String> entry : metadataEntries) {
            ...
            switch(key) {
                ...
                case "gift-wrap" ->
                    System.out.println("  Gift wrapping: " + (value.equals("true") ? "Required" : "Not required"));
                case "delivery-note" ->
        ...
        IndexedTraversal<Integer, List<LineItem>, LineItem> highValue = itemsIndexed.filteredWithIndex((index, item) -> item.price() > 100);
        List<Pair<Integer, LineItem>> expensive = IndexedTraversals.toIndexedList(highValue, order.items());
        for (Pair<Integer, LineItem> pair : expensive) {
            System.out.printf("  Position %d: %s (£%.2f)%n", pair.first() + 1, pair.second().productName(), pair.second().price());
        }
    ...
    private static void demonstrateForIndexedComprehension() {
        ...
        Lens<LineItem, Double> priceLens = Lens.of(LineItem::price, (item, price) -> new LineItem(item.productName(), item.quantity(), price));
        List<LineItem> items = List.of(new LineItem("Laptop", 1, 999.99), new LineItem("Mouse", 2, 24.99), new LineItem("Keyboard", 1, 79.99), new LineItem("Monitor", 1, 299.99), new LineItem("Webcam", 1, 59.99));
        System.out.println("Original items:");
        for (int i = 0; i < items.size(); i++) {
            System.out.printf("  Position %d: %s - £%.2f%n", i, items.get(i).productName(), items.get(i).price());
        }
        ...
        Kind<IdKind.Witness, List<LineItem>> discountedResult = ForIndexed.overIndexed(itemsIndexed, items, Instances.monad(id())).filterIndex(i -> i % 2 == 0).modify(priceLens, (index, price) -> price * 0.9).run();
        ...
        Kind<IdKind.Witness, List<LineItem>> combinedResult = ForIndexed.overIndexed(itemsIndexed, items, Instances.monad(id())).filter((index, item) -> index < 3 && item.price() > 50.0).modify(priceLens, // 15% off
        (index, price) -> price * 0.85).run();
        ...
        Kind<IdKind.Witness, List<Pair<Integer, LineItem>>> indexedListResult = ForIndexed.overIndexed(itemsIndexed, items, Instances.monad(id())).filterIndex(// Odd positions only
        i -> i % 2 == 1).toIndexedList();
        ...
        for (Pair<Integer, LineItem> pair : indexedList) {
            System.out.printf("  Index %d: %s (£%.2f)%n", pair.first(), pair.second().productName(), pair.second().price());
        }
