...
public class SagaOrderExample {
    ...
    private static void successfulOrderSaga() {
        ...
        Saga<String> orderSaga = // Step 1: Charge payment -> returns a payment ID
        Saga.of(VTask.of(() -> {
            System.out.println("  [Step 1] Charging payment...");
            return "PAY-12345";
        }), // Compensation: refund the payment
        (Consumer<String>) paymentId -> System.out.println("  [Compensate] Refunding " + paymentId)).// Step 2: Reserve inventory -> returns a reservation ID
        andThen(paymentId -> Saga.of(VTask.of(() -> {
            System.out.println("  [Step 2] Reserving inventory (payment: " + paymentId + ")...");
            return "RES-67890";
        }), (Consumer<String>) reservationId -> System.out.println("  [Compensate] Releasing reservation " + reservationId))).// Step 3: Schedule shipping -> returns a tracking ID
        andThen(reservationId -> Saga.of(VTask.of(() -> {
            System.out.println("  [Step 3] Scheduling shipping (reservation: " + reservationId + ")...");
            return "TRACK-11111";
        }), (Consumer<String>) trackingId -> System.out.println("  [Compensate] Cancelling shipment " + trackingId)));
    ...
    private static void failedOrderSagaWithCompensation() {
        ...
        Saga<String> failingOrderSaga = Saga.of(VTask.of(() -> {
            System.out.println("  [Step 1] Charging payment...");
            return "PAY-99999";
        }), (Consumer<String>) paymentId -> System.out.println("  [Compensate] Refunding " + paymentId)).andThen(paymentId -> Saga.of(VTask.of(() -> {
            System.out.println("  [Step 2] Reserving inventory...");
            return "RES-88888";
        }), (Consumer<String>) reservationId -> System.out.println("  [Compensate] Releasing reservation " + reservationId))).andThen(reservationId -> Saga.of(VTask.<String>of(() -> {
            System.out.println("  [Step 3] Scheduling shipping...");
            // Simulate a failure in the shipping step
            throw new RuntimeException("Shipping service unavailable: no carriers in region");
        }), (Consumer<String>) trackingId -> System.out.println("  [Compensate] Cancelling shipment " + trackingId)));
        ...
        try {
            ...
            System.out.println("  Order failed: " + e.getMessage());
            System.out.println("  All compensations ran successfully (payment refunded, inventory released).");
        }
        System.out.println();
    }
    ...
    private static void sagaWithSagaBuilder() {
        ...
        Saga<String> orderSaga = SagaBuilder.<Unit>start().// Step 1: Charge payment (standalone action, does not depend on previous result)
        step("charge-payment", VTask.of(() -> {
            System.out.println("  [charge-payment] Processing $49.99...");
            return "PAY-55555";
        }), paymentId -> System.out.println("  [charge-payment] Refunding " + paymentId)).// Step 2: Reserve inventory (depends on payment ID from previous step)
        step("reserve-inventory", paymentId -> VTask.of(() -> {
            System.out.println("  [reserve-inventory] Reserving items for " + paymentId + "...");
            return "RES-44444";
        }), reservationId -> System.out.println("  [reserve-inventory] Releasing " + reservationId)).// Step 3: Schedule shipping (depends on reservation ID)
        step("schedule-shipping", reservationId -> VTask.of(() -> {
            System.out.println("  [schedule-shipping] Scheduling for " + reservationId + "...");
            return "TRACK-33333";
        }), trackingId -> System.out.println("  [schedule-shipping] Cancelling " + trackingId)).build();
        try {
            String trackingId = orderSaga.run().run();
    ...
    private static void sagaRunSafeExample() {
        ...
        Saga<String> saga = SagaBuilder.<Unit>start().step("validate-order", VTask.of(() -> {
            System.out.println("  [validate-order] Validating...");
            return "ORDER-77777";
        }), orderId -> System.out.println("  [validate-order] Rolling back " + orderId)).step("charge-payment", orderId -> VTask.<String>of(() -> {
            System.out.println("  [charge-payment] Charging for " + orderId + "...");
            // Simulate payment failure
            throw new RuntimeException("Insufficient funds");
        }), paymentId -> System.out.println("  [charge-payment] Refunding " + paymentId)).build();
