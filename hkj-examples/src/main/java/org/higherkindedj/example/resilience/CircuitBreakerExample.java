...
public class CircuitBreakerExample {
    ...
    private static void basicCircuitBreakerExample() {
        ...
        CircuitBreakerConfig config = CircuitBreakerConfig.builder().failureThreshold(3).openDuration(Duration.ofSeconds(1)).callTimeout(Duration.ofSeconds(5)).build();
        ...
        for (int i = 1; i <= 8; i++) {
            try {
                ...
                System.out.println("  Call " + i + ": " + result + " (circuit: " + breaker.currentStatus() + ")");
            } catch (CircuitOpenException e) {
                // The circuit is open -- calls are rejected without hitting the service
                System.out.println("  Call " + i + ": REJECTED - " + e.getMessage() + " (circuit: " + breaker.currentStatus() + ")");
            } catch (RuntimeException e) {
                System.out.println("  Call " + i + ": FAILED - " + e.getMessage() + " (circuit: " + breaker.currentStatus() + ")");
            }
        }
    ...
    private static void sharedCircuitBreakerExample() {
        ...
        CircuitBreaker serviceBreaker = CircuitBreaker.create(CircuitBreakerConfig.builder().failureThreshold(2).openDuration(Duration.ofMillis(500)).callTimeout(Duration.ofSeconds(5)).build());
        // Endpoint 1: GET /users -> returns a String (JSON)
        VTask<String> getUsers = serviceBreaker.protect(VTask.of(() -> {
            System.out.println("    [GET /users] called");
            return "{\"users\": [\"Alice\", \"Bob\"]}";
        }));
        // Endpoint 2: GET /users/count -> returns an Integer
        VTask<Integer> getUserCount = serviceBreaker.protect(VTask.of(() -> {
            System.out.println("    [GET /users/count] called");
            return 42;
        }));
        // Endpoint 3: POST /health -> returns a Boolean
        VTask<Boolean> healthCheck = serviceBreaker.protect(VTask.of(() -> {
    ...
    private static void fallbackExample() {
        ...
        CircuitBreaker breaker = CircuitBreaker.create(CircuitBreakerConfig.builder().failureThreshold(2).openDuration(Duration.ofSeconds(1)).callTimeout(Duration.ofSeconds(5)).build());
        AtomicInteger callCount = new AtomicInteger(0);
        // A service that always fails (simulating a persistent outage)
        VTask<String> unreliableService = VTask.of(() -> {
        ...
        VTask<String> withFallback = breaker.protectWithFallback(unreliableService, ex -> {
        ...
        System.out.println("  Note: After the circuit opens, fallback is returned without calling the service.");
        System.out.println();
    }
    ...
    private static void metricsMonitoringExample() {
        ...
        CircuitBreaker breaker = CircuitBreaker.create(CircuitBreakerConfig.builder().failureThreshold(3).openDuration(Duration.ofSeconds(2)).callTimeout(Duration.ofSeconds(5)).build());
        AtomicInteger callNumber = new AtomicInteger(0);
        // Simulate a mix of successes and failures
        VTask<String> service = VTask.of(() -> {
