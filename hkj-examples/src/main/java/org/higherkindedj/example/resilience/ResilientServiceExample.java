...
public class ResilientServiceExample {
    ...
    private static void resilienceBuilderExample() {
        ...
        CircuitBreaker circuitBreaker = CircuitBreaker.create(CircuitBreakerConfig.builder().failureThreshold(5).openDuration(Duration.ofSeconds(10)).callTimeout(Duration.ofSeconds(5)).build());
        RetryPolicy retryPolicy = RetryPolicy.exponentialBackoffWithJitter(3, Duration.ofMillis(100)).withMaxDelay(Duration.ofSeconds(2));
        ...
        VTask<String> flakyService = VTask.of(() -> {
        ...
        VTask<String> resilientCall = Resilience.<String>builder(flakyService).withCircuitBreaker(circuitBreaker).withRetry(retryPolicy).withBulkhead(bulkhead).withTimeout(Duration.ofSeconds(30)).withFallback(ex -> "Fallback: service unavailable (" + ex.getMessage() + ")").build();
    ...
    private static void perElementRetryExample() {
        ...
        Function<String, VTask<String>> processElement = item -> VTask.of(() -> {
            int attempt = totalAttempts.incrementAndGet();
            // Fail roughly 50% of the time
            if (attempt % 2 == 0) {
                throw new RuntimeException("Processing failed for '" + item + "' (attempt " + attempt + ")");
            }
        ...
        Function<String, VTask<String>> resilientProcess = Resilience.withRetryPerElement(processElement, perElementPolicy);
    ...
    private static void retryEventMonitoringExample() {
        ...
        RetryPolicy monitoredPolicy = RetryPolicy.exponentialBackoff(5, Duration.ofMillis(50)).onRetry(event -> {
            retryCount.incrementAndGet();
            System.out.println("    [RetryEvent] Attempt " + event.attemptNumber() + " failed: " + event.lastException().getMessage() + " | Next delay: " + event.nextDelay().toMillis() + "ms" + " | Time: " + event.timestamp());
        });
        // Operation that fails 3 times then succeeds
        VTask<String> flakyOperation = VTask.of(() -> {
            int call = callCount.incrementAndGet();
            if (call <= 3) {
        ...
        CircuitBreaker breaker = CircuitBreaker.create(CircuitBreakerConfig.builder().failureThreshold(10).openDuration(Duration.ofSeconds(30)).callTimeout(Duration.ofSeconds(5)).build());
        VTask<String> resilientOp = Resilience.<String>builder(flakyOperation).withCircuitBreaker(breaker).withRetry(monitoredPolicy).build();
    ...
    private static void fullMetricsSummaryExample() {
        ...
        CircuitBreaker breaker = CircuitBreaker.create(CircuitBreakerConfig.builder().failureThreshold(5).openDuration(Duration.ofSeconds(30)).callTimeout(Duration.ofSeconds(5)).build());
        AtomicInteger retryEvents = new AtomicInteger(0);
        RetryPolicy retryPolicy = RetryPolicy.exponentialBackoffWithJitter(3, Duration.ofMillis(50)).onRetry(event -> retryEvents.incrementAndGet());
        ...
        for (int i = 1; i <= 10; i++) {
            ...
            VTask<String> serviceCall = VTask.of(() -> {
            ...
            VTask<String> resilientCall = Resilience.<String>builder(serviceCall).withCircuitBreaker(breaker).withRetry(retryPolicy).withBulkhead(bulkhead).withTimeout(Duration.ofSeconds(10)).withFallback(ex -> "FALLBACK").build();
            String result = resilientCall.run();
