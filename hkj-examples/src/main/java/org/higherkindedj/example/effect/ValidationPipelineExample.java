...
public class ValidationPipelineExample {
    ...
    private static void maybeValidation() {
        ...
        // Too short
        MaybePath<String> invalidName = validateNameM("A");
        ...
        // Nothing
        System.out.println("Invalid user (no error info): " + invalidUser.run());
        System.out.println();
    }
    ...
    private static void complexValidation() {
        ...
        // Validate user
        EitherPath<String, User> userValidation = validateNameE("Bob").zipWith3(validateEmailE("bob@company.com"), validateAgeE(30), User::new);
        // Validate address
        EitherPath<String, Address> addressValidation = validateStreetE("123 Main St").zipWith3(validateCityE("New York"), validateZipCodeE("10001"), Address::new);
        // Combine user and address validations
        EitherPath<String, Registration> registrationValidation = userValidation.zipWith(addressValidation, Registration::new);
        ...
        if (regResult.isRight()) {
            ...
            System.out.println("  Address: " + reg.address().street() + ", " + reg.address().city() + " " + reg.address().zipCode());
        } else {
            System.out.println("Registration failed: " + regResult.getLeft());
        }
        // Invalid registration
        System.out.println("\nInvalid registration attempt:");
        EitherPath<String, Registration> invalidReg = // Invalid - too short
        validateNameE("X").zipWith3(validateEmailE("bob@company.com"), validateAgeE(30), User::new).zipWith(validateStreetE("123 Main St").zipWith3(validateCityE("New York"), validateZipCodeE("10001"), Address::new), Registration::new);
