...
public class VStreamPathExample {
    ...
    private static void factoryMethods() {
        ...
        // Effectful unfold
        VStreamPath<Integer> unfolded = Path.vstreamUnfold(1, state -> VTask.succeed(state > 3 ? Optional.empty() : Optional.of(new VStream.Seed<>(state, state + 1))));
    ...
    private static void lazyComposition() {
        ...
        List<String> pipeline = Path.vstreamRange(1, 100).filter(n -> n % 2 == 0).map(n -> "Even: " + n).take(5).toList().unsafeRun();
    ...
    private static void terminalOperations() {
        ...
        String csv = stream.foldLeft("", (acc, n) -> acc.isEmpty() ? String.valueOf(n) : acc + "," + n).unsafeRun();
        ...
        Integer doubleSum = Path.vstreamOf("ab", "cde", "f").foldMap(sumMonoid, String::length).unsafeRun();
    ...
    private static void focusBridge() {
        ...
        VStreamPath<Person> people = Path.vstreamOf(new Person("Alice", 30), new Person("Bob", 25), new Person("Charlie", 35));
    ...
    private static void sequenceAndTraverse() {
        ...
        List<VStreamPath<Integer>> streams = List.of(Path.vstreamOf(1, 2), Path.vstreamOf(10, 20), Path.vstreamOf(100, 200));
        ...
        VStreamPath<String> traversed = PathOps.traverseVStream(inputs, n -> Path.vstreamOf("v" + n, "w" + n));
        System.out.println("traverseVStream: " + traversed.toList().unsafeRun());
        // Flatten: VStreamPath<VStreamPath<A>> -> VStreamPath<A>
        VStreamPath<VStreamPath<Integer>> nested = Path.vstreamOf(Path.vstreamOf(10, 20), Path.vstreamOf(30));
    ...
    private static void realWorldPipeline() {
        ...
        VStreamPath<LogEntry> logs = Path.vstreamOf(new LogEntry("INFO", "Application started", 1000), new LogEntry("DEBUG", "Connecting to database", 1001), new LogEntry("INFO", "User login: alice", 1002), new LogEntry("WARN", "Slow query detected", 1003), new LogEntry("ERROR", "Connection timeout", 1004), new LogEntry("INFO", "User login: bob", 1005), new LogEntry("DEBUG", "Cache hit ratio: 0.85", 1006), new LogEntry("ERROR", "Disk space low", 1007), new LogEntry("INFO", "Batch job completed", 1008));
        // Pipeline: extract error messages
        List<String> errorMessages = logs.filter(log -> "ERROR".equals(log.level())).map(LogEntry::message).toList().unsafeRun();
        System.out.println("Errors: " + errorMessages);
        // Pipeline: count entries by level
        Long warnAndAbove = logs.filter(log -> "WARN".equals(log.level()) || "ERROR".equals(log.level())).count().unsafeRun();
        ...
        Optional<String> firstError = logs.filter(log -> "ERROR".equals(log.level())).map(LogEntry::message).headOption().unsafeRun();
        System.out.println("First error: " + firstError);
        // Pipeline: build summary using fold
        String summary = logs.filter(log -> "INFO".equals(log.level())).map(LogEntry::message).foldLeft("", (acc, msg) -> acc.isEmpty() ? msg : acc + "; " + msg).unsafeRun();
