...
public class AccumulatingValidationExample {
    ...
    private static void basicAccumulation() {
        ...
        ValidationPath<List<String>, User> validUser = validName.zipWith3Accum(validEmail, validAge, User::new);
        ...
        // No @
        ValidationPath<List<String>, String> invalidEmail = validateEmail("not-an-email");
        // Negative
        ValidationPath<List<String>, Integer> invalidAge = validateAge(-5);
        ValidationPath<List<String>, User> invalidUser = invalidName.zipWith3Accum(invalidEmail, invalidAge, User::new);
    ...
    private static void shortCircuitVsAccumulating() {
        ...
        ValidationPath<List<String>, String> shortCircuit = invalidName.via(n -> invalidEmail.via(e -> invalidAge.map(a -> String.format("User: %s, %s, %d", n, e, a))));
        ...
        ValidationPath<List<String>, String> accumulated = invalidName.zipWith3Accum(invalidEmail, invalidAge, (n, e, a) -> String.format("User: %s, %s, %d", n, e, a));
    ...
    private static void complexAccumulatingValidation() {
        ...
        ValidationPath<List<String>, User> userValidation = // Invalid
        validateName("X").zipWith3Accum(validateEmail("bob@company.com"), validateAge(200), // Age also invalid
        User::new);
        // Validate address with some invalid fields
        ValidationPath<List<String>, Address> addressValidation = // Invalid
        validateStreet("").zipWith3Accum(validateCity("New York"), validateZipCode("ABCDE"), // Zip also invalid
        Address::new);
        // Combine all validations - ALL errors collected
        ValidationPath<List<String>, Registration> registrationValidation = userValidation.zipWithAccum(addressValidation, Registration::new);
