...
public class LazyPathExample {
    ...
    private static void memoisationDemo() {
        ...
        LazyPath<BigInteger> lazyFib = LazyPath.defer(() -> {
    ...
    private static void lazyChaining() {
        ...
        LazyPath<String> chain = LazyPath.defer(() -> {
        ...
        // 0
        System.out.println("Chain created. Steps executed: " + step.get());
        System.out.println("Forcing evaluation...");
        String result = chain.get();
        System.out.println("Final result: " + result);
        // 3
        System.out.println("Total steps: " + step.get());
    ...
    private static void expensiveComputationPatterns() {
        ...
        LazyPath<Config> config = LazyPath.defer(() -> {
        ...
        if (needsConfig) {
            ...
            // Config already cached
            System.out.println("Port: " + config.map(Config::port).get());
        }
        // Pattern 2: Conditional expensive computation
        System.out.println("\nConditional Computation:");
        LazyPath<String> expensiveResult = LazyPath.defer(() -> {
            System.out.println("  [Running expensive analysis...]");
            sleep(100);
            return "Analysis complete: 42 issues found";
        });
        // Simulate user choice
        boolean userWantsAnalysis = false;
