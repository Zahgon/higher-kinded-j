...
public class ResilienceExample {
    ...
    private static void basicRetryExample() {
        ...
        IOPath<String> flakyOperation = Path.io(() -> {
    ...
    private static void selectiveRetryExample() {
        ...
        RetryPolicy policy = RetryPolicy.fixed(5, Duration.ofMillis(50)).retryOn(UncheckedIOException.class);
        // Operation that throws UncheckedIOException (should retry)
        IOPath<String> ioErrorOp = Path.io(() -> {
            int attempt = attempts.incrementAndGet();
        ...
        attempts.set(0);
        IOPath<String> illegalArgOp = Path.io(() -> {
            int attempt = attempts.incrementAndGet();
        ...
        attempts.set(0);
        RetryPolicy customPolicy = RetryPolicy.fixed(5, Duration.ofMillis(50)).retryIf(ex -> ex.getMessage() != null && ex.getMessage().contains("transient"));
        IOPath<String> transientOp = Path.io(() -> {
            int attempt = attempts.incrementAndGet();
            System.out.println("  Attempt " + attempt);
            if (attempt < 3) {
                throw new RuntimeException("transient failure");
    ...
    private static void exhaustedRetriesExample() {
        ...
        IOPath<String> alwaysFails = Path.io(() -> {
    ...
    private static void flakyServiceSimulation() {
        ...
        IOPath<String> flakyService = Path.io(() -> {
        ...
        RetryPolicy resilientPolicy = RetryPolicy.exponentialBackoffWithJitter(5, Duration.ofMillis(50)).withMaxDelay(Duration.ofMillis(500));
        ...
        for (int i = 0; i < 10; i++) {
            try {
                ...
                System.out.println("  Request " + (i + 1) + " failed after " + e.getAttempts() + " attempts");
            }
        }
        System.out.println("\nResults:");
    ...
    private static void completableFutureRetryExample() {
        ...
        CompletableFuturePath<String> asyncOp = CompletableFuturePath.supplyAsync(() -> {
            int attempt = attempts.incrementAndGet();
    ...
    private static void combinedPatternsExample() {
        ...
        IOPath<String> primaryService = Path.io(() -> {
        ...
        IOPath<String> fallbackService = Path.io(() -> {
        ...
        IOPath<String> resilientCall = Path.io(() -> {
            try {
