...
public class AdvancedEffectsExample {
    ...
    private static void readerPathExamples() {
        ...
        ReaderPath<AppConfig, String> connectionInfo = getDbUrl.via(url -> getMaxConn.map(maxConn -> String.format("Connecting to %s with pool size %d", url, maxConn)));
        ...
        ReaderPath<AppConfig, String> combined = getDbUrl.zipWith(getMaxConn, (url, max) -> String.format("URL=%s, Pool=%d", url, max));
    ...
    private static void statePathExamples() {
        ...
        WithStatePath<Integer, Integer> nextId = WithStatePath.<Integer>modify(n -> n + 1).then(WithStatePath::get);
        // Generate 3 IDs sequentially
        WithStatePath<Integer, List<Integer>> threeIds = nextId.via(id1 -> nextId.via(id2 -> nextId.map(id3 -> List.of(id1, id2, id3))));
        StateTuple<Integer, List<Integer>> result = threeIds.run(0);
        // [1, 2, 3]
        System.out.println("Generated IDs: " + result.value());
        // 3
        System.out.println("Final counter: " + result.state());
        ...
        WithStatePath<Cart, String> shoppingSession = addToCart(new CartItem("Book", 29.99)).then(() -> addToCart(new CartItem("Coffee", 4.99))).then(() -> addToCart(new CartItem("Pen", 1.99))).then(() -> getTotal.via(total -> getItemCount.map(count -> String.format("Cart has %d items, total: £%.2f", count, total))));
    ...
    private static void writerPathExamples() {
        ...
        WriterPath<List<String>, Integer> computation = WriterPath.tell(List.of("Starting computation"), logMonoid).then(() -> WriterPath.pure(42, logMonoid)).via(n -> WriterPath.writer(n * 2, List.of("Doubled value to " + (n * 2)), logMonoid));
        // 84
        System.out.println("Result: " + computation.value());
        ...
        WriterPath<List<AuditEntry>, String> auditedOperation = WriterPath.tell(List.of(new AuditEntry("User login", "2025-01-15T10:00:00")), auditMonoid).then(() -> WriterPath.writer("data-123", List.of(new AuditEntry("Data accessed", "2025-01-15T10:00:01")), auditMonoid)).via(data -> WriterPath.writer("processed-" + data, List.of(new AuditEntry("Data processed", "2025-01-15T10:00:02")), auditMonoid));
    ...
    private static void combinedPatterns() {
        ...
        // 20% tax, 10% discount over £100
        OrderConfig config = new OrderConfig(0.20, 100.0, 0.10);
        // Calculate order with configuration
        double subtotal = 150.0;
        ReaderPath<OrderConfig, String> orderCalculation = applyDiscount(subtotal).via(discounted -> calculateTax(discounted).map(tax -> String.format("Subtotal: £%.2f, After discount: £%.2f, Tax: £%.2f, Total: £%.2f", subtotal, discounted, tax, discounted + tax)));
        System.out.println("Order calculation:");
        System.out.println(orderCalculation.run(config));
        // State for tracking order count
        WithStatePath<Integer, List<String>> processOrders = trackOrder("ORD-001").via(msg1 -> trackOrder("ORD-002").via(msg2 -> trackOrder("ORD-003").map(msg3 -> List.of(msg1, msg2, msg3))));
    ...
    private static ReaderPath<OrderConfig, Double> applyDiscount(double subtotal) {
        return ReaderPath.asks(cfg -> subtotal >= cfg.discountThreshold() ? subtotal * (1 - cfg.discountRate()) : subtotal);
    }

    private static WithStatePath<Integer, String> trackOrder(String orderId) {
        return WithStatePath.<Integer>modify(count -> count + 1).then(WithStatePath::get).map(count -> String.format("Order %s is #%d today", orderId, count));
    }
