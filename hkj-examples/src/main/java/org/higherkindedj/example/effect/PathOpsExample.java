...
public class PathOpsExample {
    ...
    private static void traverseMaybeExamples() {
        ...
        MaybePath<List<Integer>> parsed = PathOps.traverseMaybe(validNumbers, PathOpsExample::parseIntMaybe);
        System.out.println("Valid numbers: " + parsed.run());
        // Just[[1, 2, 3, 4, 5]]
        // One invalid fails all
        List<String> withInvalid = List.of("1", "two", "3");
        MaybePath<List<Integer>> parsedInvalid = PathOps.traverseMaybe(withInvalid, PathOpsExample::parseIntMaybe);
    ...
    private static void sequenceEitherExamples() {
        ...
        // All successful
        List<EitherPath<String, Integer>> allRight = List.of(Path.right(1), Path.right(2), Path.right(3));
        ...
        List<EitherPath<String, Integer>> withLeft = List.of(Path.right(1), Path.left("Error at position 2"), Path.right(3));
    ...
    private static void traverseEitherExamples() {
        ...
        EitherPath<String, List<String>> userNames = PathOps.traverseEither(userIds, PathOpsExample::lookupUser);
        ...
        EitherPath<String, List<String>> invalidLookup = PathOps.traverseEither(withInvalid, PathOpsExample::lookupUser);
    ...
    private static void sequenceValidatedExamples() {
        ...
        // All valid
        List<ValidationPath<List<String>, Integer>> allValid = List.of(Path.valid(1, LIST_SEMIGROUP), Path.valid(2, LIST_SEMIGROUP), Path.valid(3, LIST_SEMIGROUP));
        ValidationPath<List<String>, List<Integer>> sequenced = PathOps.sequenceValidated(allValid, LIST_SEMIGROUP);
        System.out.println("All valid: " + sequenced.run());
        // Valid[[1, 2, 3]]
        // Multiple invalids - ALL errors accumulated
        List<ValidationPath<List<String>, Integer>> withInvalids = List.of(Path.invalid(List.of("Error 1"), LIST_SEMIGROUP), Path.valid(2, LIST_SEMIGROUP), Path.invalid(List.of("Error 3"), LIST_SEMIGROUP));
        ValidationPath<List<String>, List<Integer>> sequencedInvalids = PathOps.sequenceValidated(withInvalids, LIST_SEMIGROUP);
    ...
    private static void traverseValidatedExamples() {
        ...
        ValidationPath<List<String>, List<String>> validated = PathOps.traverseValidated(emails, PathOpsExample::validateEmail, LIST_SEMIGROUP);
        ...
        List<String> mixedEmails = List.of("alice@example.com", "invalid-email", "bob.com", "charlie@domain.net");
        ValidationPath<List<String>, List<String>> mixedValidation = PathOps.traverseValidated(mixedEmails, PathOpsExample::validateEmail, LIST_SEMIGROUP);
    ...
    private static void firstSuccessExamples() {
        ...
        List<TryPath<String>> dataSources = List.of(Path.failure(new RuntimeException("Primary DB unavailable")), Path.failure(new RuntimeException("Cache miss")), Path.success("Data from backup source"));
        ...
        List<TryPath<String>> allFail = List.of(Path.failure(new RuntimeException("Error 1")), Path.failure(new RuntimeException("Error 2")), Path.failure(new RuntimeException("Final error")));
        TryPath<String> allFailResult = PathOps.firstSuccess(allFail);
        System.out.println("All fail: " + allFailResult.run().fold(s -> "Success: " + s, t -> "Failure: " + t.getMessage()));
        // Failure: Final error
        // First succeeds - others not tried
        List<TryPath<String>> firstSucceeds = List.of(Path.success("Got it on first try!"), Path.failure(new RuntimeException("This won't be tried")));
