...
public class CollectionPathsExample {
    ...
    private static void listPathBasics() {
        ...
        // [2, 4, 6, 8, 10]
        System.out.println("Doubled: " + doubled.run());
        // Filtering
        ListPath<Integer> evens = fromList.filter(n -> n % 2 == 0);
        // [2, 4]
        System.out.println("Evens: " + evens.run());
    ...
    private static void listPathZipping() {
        ...
        // [1a, 2b, 3c]
        System.out.println("Zipped: " + zipped.run());
        // Shortest list wins
        ...
        // [1a, 2b]
        System.out.println("Truncated: " + truncated.run());
        // Three-way zip
    ...
    private static void listPathFlatMap() {
        ...
        // [1, 10, 2, 20, 3, 30]
        System.out.println("Expanded: " + expanded.run());
        // Practical: Processing nested data
        record Order(String id, List<String> items) {
        }
        List<Order> orders = List.of(new Order("ord-1", List.of("Widget", "Gadget")), new Order("ord-2", List.of("Gizmo")), new Order("ord-3", List.of("Thingamajig", "Doohickey", "Whatsit")));
    ...
    private static void streamPathBasics() {
        ...
        // [9, 16, 25]
        System.out.println("Squares > 5: " + processed.toList());
        ...
        // Can call again!
        System.out.println("First: " + nums.headOption());
        System.out.println();
    }
    ...
    private static void streamPathLazyEvaluation() {
        ...
        StreamPath<Integer> pipeline = StreamPath.of(Stream.of(1, 2, 3, 4, 5)).map(n -> {
    ...
    private static void streamPathInfiniteSequences() {
        ...
        StreamPath<Integer> fibonacci = StreamPath.iterate(new FibPair(0, 1), p -> new FibPair(p.b(), p.a() + p.b())).map(FibPair::a);
        ...
        System.out.println("5 random numbers: " + randoms.take(5).toList().stream().map(d -> String.format("%.3f", d)).toList());
        System.out.println();
    }
    ...
    private static void practicalPatterns() {
        ...
        ListPath<Task> tasks = ListPath.of(new Task("T1", "Setup"), new Task("T2", "Build"), new Task("T3", "Test"), new Task("T4", "Deploy"));
        ListPath<String> processed = tasks.peek(t -> System.out.println("  Processing: " + t.id())).map(t -> t.id() + " completed");
        ...
        ListPath<RawRecord> rawData = ListPath.of(new RawRecord("alpha", "10"), new RawRecord("beta", "invalid"), new RawRecord("gamma", "30"), new RawRecord("delta", "40"));
        ListPath<CleanRecord> cleanData = rawData.filter(// Filter parseable
        r -> r.value().matches("\\d+")).map(r -> new CleanRecord(r.name(), Integer.parseInt(r.value())));
