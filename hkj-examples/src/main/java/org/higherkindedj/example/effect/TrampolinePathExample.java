...
public class TrampolinePathExample {
    ...
    private static void basicTrampolineExamples() {
        ...
        TrampolinePath<Integer> nested = TrampolinePath.defer(() -> TrampolinePath.defer(() -> TrampolinePath.defer(() -> TrampolinePath.done(999))));
    ...
    private static void chainedOperationsExample() {
        ...
        TrampolinePath<Integer> computation = TrampolinePath.done(10).map(// 20
        x -> x * 2).map(// 25
        x -> x + 5).map(// 5
        x -> x / 5);
        System.out.println("Chained map: 10 -> *2 -> +5 -> /5 = " + computation.run());
        // Using via for dependent computations
        TrampolinePath<String> viaChain = TrampolinePath.done(100).via(n -> TrampolinePath.done("Number: " + n)).via(s -> TrampolinePath.done(s.toUpperCase()));
        ...
        TrampolinePath<String> sequence = TrampolinePath.done("first").peek(s -> System.out.println("  Computed: " + s)).then(() -> TrampolinePath.done("second"));
