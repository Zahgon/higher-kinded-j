...
public class ForPathExample {
    ...
    static final Lens<User, Address> addressLens = Lens.of(User::address, (u, a) -> new User(u.name(), a, u.email()));

    static final Lens<Address, String> cityLens = Lens.of(Address::city, (a, c) -> new Address(c, a.postcode()));
    ...
    static final AffinePath<User, String> emailPath = AffinePath.of(Affine.of(u -> Optional.ofNullable(u.email()), (u, e) -> new User(u.name(), u.address(), e)));
    ...
    private static void maybePathBasics() {
        ...
        MaybePath<Integer> result = ForPath.from(Path.just(10)).from(a -> Path.just(20)).from(// t is Tuple2<Integer, Integer>
        t -> Path.just(5)).yield((a, b, c) -> a + b + c);
        // 35
        System.out.println("Sum of 10 + 20 + 5 = " + result.getOrElse(-1));
        // Using let() for pure computations
        MaybePath<String> withLet = ForPath.from(Path.just(10)).let(// b = 20 (pure calculation)
        a -> a * 2).let(// c = 30 (sum via tuple)
        t -> t._1() + t._2()).yield((a, b, c) -> "Original: " + a + ", Doubled: " + b + ", Sum: " + c);
        System.out.println(withLet.getOrElse("no value"));
        // Original: 10, Doubled: 20, Sum: 30
        // Short-circuiting with Nothing
        MaybePath<Integer> shortCircuit = ForPath.from(Path.just(5)).<Integer>from(// stops here
        a -> Path.nothing()).from(// never executed
        t -> Path.just(100)).yield((a, b, c) -> a + b + c);
        // -1
        System.out.println("Short-circuited result: " + shortCircuit.getOrElse(-1));
        System.out.println();
    }
    ...
    private static void maybePathWithGuards() {
        ...
        // Guard that passes
        MaybePath<Integer> evenOnly = ForPath.from(Path.just(4)).when(n -> n % 2 == 0).yield(n -> n * 10);
        // 40
        System.out.println("4 is even, result: " + evenOnly.getOrElse(-1));
        // Guard that fails
        MaybePath<Integer> oddFiltered = ForPath.from(Path.just(3)).when(n -> n % 2 == 0).yield(n -> n * 10);
        // -1
        System.out.println("3 is odd, filtered out: " + oddFiltered.getOrElse(-1));
        // Guard in multi-step comprehension
        MaybePath<String> multiStepGuard = ForPath.from(Path.just(5)).from(a -> Path.just(10)).when(// 5 + 10 = 15 > 10, passes
        t -> t._1() + t._2() > 10).yield((a, b) -> "Sum is " + (a + b));
        // Sum is 15
        System.out.println(multiStepGuard.getOrElse("filtered"));
        System.out.println();
    }
    ...
    private static void eitherPathWorkflow() {
        ...
        EitherPath<String, String> successResult = ForPath.from(Path.<String, String>right("Alice")).from(name -> Path.right("order-123")).yield((name, orderId) -> "Created " + orderId + " for " + name);
        System.out.println("Success: " + successResult.run().getRight());
        // Failed workflow - starts with Left
        EitherPath<String, String> failedResult = ForPath.from(Path.<String, String>left("User not found")).from(name -> Path.right("order-456")).yield((name, orderId) -> "Created " + orderId);
    ...
    private static void ioPathComposition() {
        ...
        IOPath<String> serverInfo = ForPath.from(readConfig).from(env -> readPort).let(t -> t._1().toUpperCase()).yield((env, port, upperEnv) -> upperEnv + " server on port " + port);
        ...
        IOPath<String> multiStep = ForPath.from(Path.io(() -> "Step 1")).from(s1 -> Path.io(() -> s1 + " -> Step 2")).from(t -> Path.io(() -> t._2() + " -> Step 3")).yield((s1, s2, s3) -> "Final: " + s3);
    ...
    private static void nonDetPathCombinations() {
        ...
        NonDetPath<String> allCombinations = ForPath.from(Path.list("red", "blue", "green")).from(c -> Path.list("S", "M", "L")).yield((colour, size) -> colour + "-" + size);
        ...
        NonDetPath<String> filteredCombinations = ForPath.from(Path.list("red", "blue", "green")).from(c -> Path.list("S", "M", "L")).when(// filter out blue-S
        t -> !t._1().equals("blue") || !t._2().equals("S")).yield((colour, size) -> colour + "-" + size);
        ...
        NonDetPath<String> pairs = ForPath.from(Path.list(1, 2, 3, 4, 5)).from(a -> Path.list(1, 2, 3, 4, 5)).when(// only pairs where first < second
        t -> t._1() < t._2()).yield((a, b) -> "(" + a + "," + b + ")");
    ...
    private static void opticsIntegration() {
        ...
        MaybePath<String> cityResult = ForPath.from(Path.just(user)).focus(userCityPath).yield((u, city) -> u.name() + " lives in " + city);
        ...
        MaybePath<String> cityResult2 = ForPath.from(Path.just(user)).focus(addressPath).focus(// t is Tuple2<User, Address>
        t -> t._2().city()).yield((u, address, city) -> u.name() + " lives in " + city);
        ...
        MaybePath<String> matchSuccess = ForPath.from(Path.just(userWithEmail)).match(emailPath).yield((u, email) -> u.name() + "'s email: " + email);
        ...
        MaybePath<String> matchFailure = ForPath.from(Path.just(userWithoutEmail)).match(emailPath).yield((u, email) -> u.name() + "'s email: " + email);
    ...
    private static void highArityExample() {
        ...
        MaybePath<String> sixStep = ForPath.from(Path.just("Alice")).let(name -> name.length()).from(t -> Path.just(t._1().toUpperCase())).let(t -> t._2() * 10).let(t -> t._3() + "!").let(t -> t._1() + " has " + t._2() + " letters").yield((name, len, upper, score, exclaimed, summary) -> summary + " (score: " + score + ")");
        ...
        NonDetPath<String> combos = ForPath.from(Path.list("A", "B")).from(letter -> Path.list(1, 2, 3)).let(t -> t._1() + t._2()).let(t -> t._3().length()).when(t -> t._4() == 2).let(t -> t._3() + " (len=" + t._4() + ")").yield((letter, num, combined, len, formatted) -> formatted);
        System.out.println("6-step NonDetPath: " + combos.run());
        // 8-binding MaybePath comprehension — the maximum arity
        MaybePath<String> eightStep = ForPath.from(Path.just("Bob")).let(// b = 3
        name -> name.length()).from(// c = "bob"
        t -> Path.just(t._1().toLowerCase())).let(// d = 300
        t -> t._2() * 100).let(// e = "bob!"
        t -> t._3() + "!").let(// f = "Bob#300"
        t -> t._1() + "#" + t._4()).let(// g = 7
        t -> t._6().length()).let(// h
        t -> "result=" + t._6() + " (len " + t._7() + ")").yield((name, len, lower, score, excl, tag, tagLen, summary) -> summary);
    ...
    private static void parallelComposition() {
        ...
        MaybePath<String> maybePar = ForPath.par(Path.just("Alice"), Path.just(42)).yield((name, age) -> name + " is " + age);
        System.out.println("MaybePath par(2): " + maybePar.getOrElse("nothing"));
        // Alice is 42
        // 2. Static par() — short-circuits on Nothing
        MaybePath<String> maybeParShort = ForPath.par(Path.just("Bob"), Path.<Integer>nothing()).yield((name, age) -> name + " is " + age);
        ...
        EitherPath<String, String> eitherPar = ForPath.par(Path.<String, String>right("Alice"), Path.<String, Integer>right(42), Path.<String, String>right("admin")).yield((name, age, role) -> name + " (" + age + ") [" + role + "]");
        System.out.println("EitherPath par(3): " + eitherPar.run().getRight());
        // Alice (42) [admin]
        // 4. Static par() with IdPath — pure value combination
        IdPath<Integer> idPar = ForPath.par(Path.idPath(Id.of(10)), Path.idPath(Id.of(20)), Path.idPath(Id.of(30))).yield((a, b, c) -> a + b + c);
        System.out.println("IdPath par(3): " + idPar.run().value());
        // 60
        // 5. VTaskPath par() — true parallel execution on virtual threads
        VTaskPath<String> vtaskPar = ForPath.par(Path.vtaskPath(() -> {
            Thread.sleep(50);
            return "user-data";
        }), Path.vtaskPath(() -> {
            Thread.sleep(50);
        ...
        IOPath<String> ioPar = ForPath.par(Path.io(() -> "hello"), Path.io(() -> "world")).yield((a, b) -> a + " " + b);
        System.out.println("IOPath par(2): " + ioPar.unsafeRun());
        // hello world
        // 7. Sequential from() chaining — each step depends on the previous
        MaybePath<String> sequential = ForPath.from(Path.just("Alice")).from(name -> Path.just(name.length())).yield((name, len) -> name + " has " + len + " letters");
    ...
    private static void traverseWithinForPath() {
        ...
        MaybePath<List<Integer>> traverseResult = ForPath.from(Path.just(Arrays.asList(1, 2, 3))).traverse(ListTraverse.INSTANCE, list -> LIST.widen(list), (Integer i) -> MAYBE.<Integer>just(i * 2)).yield((original, traversed) -> LIST.narrow(traversed));
        ...
        MaybePath<List<Integer>> traverseWithFailure = ForPath.from(Path.just(Arrays.asList(1, 2, 3))).traverse(ListTraverse.INSTANCE, list -> LIST.widen(list), (Integer i) -> i > 2 ? MAYBE.<Integer>nothing() : MAYBE.just(i * 10)).yield((original, traversed) -> LIST.narrow(traversed));
