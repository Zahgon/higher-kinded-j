...
public class VTaskPathExample {
    ...
    private static void basicCreationAndExecution() {
        ...
        String message = tryResult.fold(value -> "   Success: " + value, error -> "   Failure: " + error.getMessage());
    ...
    private static void transformationAndChaining() {
        ...
        VTask<String> enriched = greeting.flatMap(s -> VTask.of(() -> s + ", World!")).flatMap(s -> VTask.of(() -> s.toUpperCase()));
        ...
        VTask<Integer> debugged = VTask.succeed(10).peek(v -> System.out.println("peek - Before transform: " + v)).map(v -> v * 2).peek(v -> System.out.println("peek - After transform: " + v));
        debugged.runSafe();
    ...
    private static void errorHandlingPatterns() {
        ...
        VTask<String> recoveredWith = failingTask.recoverWith(error -> VTask.of(() -> "Fallback from secondary source"));
        System.out.println("recoverWith - Result: " + recoveredWith.runSafe().orElse("error"));
        // mapError: Transform the exception type
        VTask<String> mappedError = failingTask.mapError(error -> new IllegalStateException("Wrapped: " + error.getMessage()));
        Try<String> errorResult = mappedError.runSafe();
        String errorMessage = errorResult.fold(value -> "mapError - Value: " + value, error -> "mapError - Error type: " + error.getClass().getSimpleName());
        System.out.println(errorMessage);
        // Fallback chain: Try multiple sources
        System.out.println("\nFallback chain:");
        VTask<String> resilientConfig = VTask.<String>fail(new RuntimeException("Primary failed")).recoverWith(e -> {
    ...
    private static void timeoutHandling() {
        ...
        VTask<String> slowOp = VTask.of(() -> {
            sleep(2000);
            return "Slow result";
        });
        VTask<String> slowWithTimeout = slowOp.timeout(Duration.ofMillis(500));
        Try<String> slowResult = slowWithTimeout.runSafe();
        String slowMessage = slowResult.fold(value -> "Slow operation: " + value, error -> "Slow operation timed out: " + error.getClass().getSimpleName());
        System.out.println(slowMessage);
        // Timeout with recovery
        VTask<String> withRecovery = slowOp.timeout(Duration.ofMillis(500)).recover(e -> "Default after timeout");
    ...
    private static void parallelExecutionWithPar() {
        ...
        VTask<String> combined = Par.map2(userTask, profileTask, (user, profile) -> user + " | " + profile);
        System.out.println("map2 - Combined: " + combined.runSafe().orElse("error"));
        // all: Execute a list of tasks in parallel
        List<VTask<Integer>> tasks = List.of(VTask.of(() -> {
            sleep(100);
            return 1;
        }), VTask.of(() -> {
            sleep(100);
        ...
        VTask<String> server1 = VTask.of(() -> {
            sleep(200);
            return "Response from Server 1";
        });
        VTask<String> server2 = VTask.of(() -> {
            sleep(50);
    ...
    private static void realWorldServiceAggregation() {
        ...
        long elapsed = System.currentTimeMillis() - start;
        String dashboardMessage = result.fold(d -> "Dashboard loaded successfully:\n" + "  User: " + d.userName + "\n" + "  Orders: " + d.recentOrders + "\n" + "  Analytics: " + d.analyticsData, error -> "Dashboard failed: " + error.getMessage());
        ...
        VTask<Dashboard> dashboardWithTimeout = loadDashboard(userId).timeout(// Too short for the 150ms services
        Duration.ofMillis(100)).recover(e -> Dashboard.empty());
        Try<Dashboard> timeoutResult = dashboardWithTimeout.runSafe();
        String timeoutMessage = timeoutResult.fold(d -> "Result: " + (d.isEmpty() ? "Empty (fallback)" : "Loaded"), error -> "Error: " + error.getMessage());
    ...
    private static VTask<Dashboard> loadDashboard(String userId) {
        ...
        VTask<String> analyticsTask = VTask.of(() -> {
            sleep(150);
