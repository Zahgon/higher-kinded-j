...
public class ServiceLayerExample {
    ...
    private static void basicServiceOperations() {
        ...
        // alice@example.com
        System.out.println("User email: " + userEmail.unsafeRun());
        System.out.println();
    }
    ...
    private static void composingServices() {
        ...
        IOPath<OrderSummary> getOrderSummary = findOrderById("o1").via(order -> findUserById(order.userId()).map(user -> new OrderSummary(user, order, "Processing")));
        System.out.println("Order summary computation created");
        System.out.println("Executing...");
        OrderSummary summary = getOrderSummary.unsafeRun();
        System.out.println("Summary: " + summary.user().name() + " ordered " + summary.order().product() + " (status: " + summary.status() + ")");
        // Combine independent lookups with zipWith
        System.out.println("\n--- Parallel-style Composition (zipWith) ---");
        IOPath<String> combinedInfo = findUserById("u1").zipWith(findOrderById("o1"), (user, order) -> user.name() + " has order for " + order.product());
    ...
    private static void errorHandlingInServices() {
        ...
        IOPath<User> withFallback = findUserById("unknown").handleError(ex -> new User("default", "Guest", "guest@example.com"));
        // Guest
        System.out.println("With fallback: " + withFallback.unsafeRun().name());
        // Option 3: handleErrorWith for recovery computation
        IOPath<User> withRecovery = findUserById("unknown").handleErrorWith(ex -> {
            System.out.println("  Primary lookup failed, trying backup...");
            // Fallback to known user
            return findUserById("u1");
        });
        // Alice
        System.out.println("With recovery: " + withRecovery.unsafeRun().name());
        System.out.println();
    }
    ...
    private static void transactionPattern() {
        ...
        IOPath<String> transaction = Path.io(() -> {
            System.out.println("  [TX] Starting transaction");
            return "tx-123";
        }).via(txId -> findUserById("u1").map(user -> {
            operationCount.incrementAndGet();
            System.out.println("  [TX] Validated user: " + user.name());
            return user;
        })).via(user -> findOrderById("o1").map(order -> {
            operationCount.incrementAndGet();
