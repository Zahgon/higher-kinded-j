...
public class VStreamParallelExample {
    ...
    private static void parallelApiCalls() {
        ...
        VStream<String> profiles = VStreamPar.parEvalMap(userIds, 2, userId -> VTask.of(() -> {
    ...
    private static void unorderedParallel() {
        ...
        VStream<String> result = VStreamPar.parEvalMapUnordered(numbers, 3, n -> VTask.of(() -> {
            Thread.sleep(n);
            return "done-" + n;
        }));
    ...
    private static void batchDatabaseInsert() {
        ...
        VStream<String> insertResults = records.mapChunked(3, batch -> {
    ...
    private static void parallelBatchPipeline() {
        ...
        VStream<Integer> batchSums = VStreamPar.parEvalMap(data.chunk(5), 2, chunk -> VTask.of(() -> {
    ...
    private static void vstreamPathParallel() {
        ...
        List<String> result = numbers.filter(n -> n % 2 == 0).parEvalMap(3, n -> VTask.of(() -> {
            Thread.sleep(10);
            return "processed-" + n;
        })).toList().unsafeRun();
