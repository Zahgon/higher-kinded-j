...
public class CompletableFuturePathExample {
    ...
    private static void basicAsyncExamples() {
        ...
        // Async computation
        CompletableFuturePath<String> async = CompletableFuturePath.supplyAsync(() -> {
        ...
        // Check status
        // true
        System.out.println("Is done? " + completed.isDone());
        System.out.println();
    }
    ...
    private static void chainingAsyncOperations() {
        ...
        CompletableFuturePath<User> userFetch = CompletableFuturePath.supplyAsync(() -> {
            System.out.println("  [Fetching user...]");
            sleep(100);
            return new User("user-123", "Alice", "alice@example.com");
        });
        // Chain: fetch user -> fetch their orders -> get first order
        CompletableFuturePath<String> orderSummary = userFetch.map(// Extract name
        user -> user.name()).via(name -> CompletableFuturePath.supplyAsync(() -> {
        ...
        CompletableFuturePath<Integer> withLogging = CompletableFuturePath.completed(100).peek(v -> System.out.println("  [Log] Value is: " + v)).map(v -> v * 2).peek(v -> System.out.println("  [Log] After doubling: " + v));
    ...
    private static void parallelComposition() {
        ...
        CompletableFuturePath<String> fetchUserData = CompletableFuturePath.supplyAsync(() -> {
            System.out.println("  [Fetching user data...]");
            sleep(200);
            return "UserData";
        });
        CompletableFuturePath<String> fetchProductData = CompletableFuturePath.supplyAsync(() -> {
            System.out.println("  [Fetching product data...]");
            sleep(200);
            return "ProductData";
        });
        // Combine them (runs in parallel!)
        CompletableFuturePath<String> combined = fetchUserData.zipWith(fetchProductData, (user, product) -> "Combined: " + user + " + " + product);
        ...
        CompletableFuturePath<Integer> a = CompletableFuturePath.supplyAsync(() -> {
            sleep(50);
            return 10;
        });
        CompletableFuturePath<Integer> b = CompletableFuturePath.supplyAsync(() -> {
            sleep(50);
            return 20;
        });
        CompletableFuturePath<Integer> c = CompletableFuturePath.supplyAsync(() -> {
            sleep(50);
            return 30;
        });
    ...
    private static void errorHandling() {
        ...
        // Failed future
        CompletableFuturePath<String> failed = CompletableFuturePath.failed(new RuntimeException("Network error"));
        ...
        CompletableFuturePath<String> primaryService = CompletableFuturePath.supplyAsync(() -> {
            System.out.println("  [Primary service failing...]");
            throw new RuntimeException("Primary unavailable");
        });
        CompletableFuturePath<String> withFallbackService = primaryService.recoverWith(ex -> {
        ...
        CompletableFuturePath<String> pipeline = CompletableFuturePath.completed("input").via(s -> CompletableFuturePath.supplyAsync(() -> {
            if (s.length() < 10) {
    ...
    private static void timeoutHandling() {
        ...
        CompletableFuturePath<String> fast = CompletableFuturePath.supplyAsync(() -> {
            sleep(50);
            return "Fast result";
        });
        try {
        ...
        CompletableFuturePath<String> slow = CompletableFuturePath.supplyAsync(() -> {
            sleep(500);
            return "Slow result";
        });
        try {
        ...
        CompletableFuturePath<String> withTimeoutRecovery = CompletableFuturePath.supplyAsync(() -> {
            sleep(500);
