...
public class ScopeExample {
    ...
    private static void allSucceedExample() {
        ...
        VTask<String> profileTask = VTask.of(() -> {
            sleep(100);
            return "Profile: Developer";
        });
        VTask<String> prefsTask = VTask.of(() -> {
            sleep(100);
            return "Theme: Dark";
        });
        long start = System.currentTimeMillis();
        VTask<List<String>> allResults = Scope.<String>allSucceed().fork(userTask).fork(profileTask).fork(prefsTask).join();
    ...
    private static void anySucceedExample() {
        ...
        VTask<String> mirror1 = VTask.of(() -> {
            sleep(200);
            return "Response from Mirror 1";
        });
        VTask<String> mirror2 = VTask.of(() -> {
            // Faster mirror
            sleep(50);
            return "Response from Mirror 2";
        });
        VTask<String> mirror3 = VTask.of(() -> {
            sleep(150);
            return "Response from Mirror 3";
        });
        long start = System.currentTimeMillis();
        VTask<String> fastest = Scope.<String>anySucceed().fork(mirror1).fork(mirror2).fork(mirror3).join();
    ...
    private static void accumulatingExample() {
        ...
        VTask<String> validateUsername = VTask.of(() -> {
            sleep(50);
            throw new IllegalArgumentException("Username too short");
        });
        VTask<String> validateEmail = VTask.of(() -> {
            sleep(50);
            throw new IllegalArgumentException("Invalid email format");
        });
        VTask<String> validateAge = VTask.of(() -> {
            sleep(50);
            return "Age: 25 (valid)";
        });
        // Error mapper: convert exceptions to strings
        VTask<Validated<List<String>, List<String>>> validation = Scope.<String, String>accumulating(Throwable::getMessage).fork(validateUsername).fork(validateEmail).fork(validateAge).join();
        Validated<List<String>, List<String>> result = validation.runSafe().orElse(Validated.invalid(List.of("Unknown error")));
        result.fold(errors -> {
    ...
    private static void timeoutExample() {
        ...
        VTask<String> slowTask1 = VTask.of(() -> {
            sleep(500);
            return "Slow result 1";
        });
        VTask<String> slowTask2 = VTask.of(() -> {
            sleep(500);
            return "Slow result 2";
        });
        // This will timeout before tasks complete
        VTask<List<String>> withTimeout = Scope.<String>allSucceed().timeout(Duration.ofMillis(200)).fork(slowTask1).fork(slowTask2).join();
        Try<List<String>> result = withTimeout.runSafe();
        String message = result.fold(value -> "Success: " + value, error -> "Timed out: " + error.getClass().getSimpleName());
        System.out.println(message);
        // Timeout with recovery
        VTask<List<String>> withRecovery = Scope.<String>allSucceed().timeout(Duration.ofMillis(200)).fork(slowTask1).fork(slowTask2).join().recover(e -> List.of("Default value 1", "Default value 2"));
    ...
    private static void safeResultHandling() {
        ...
        VTask<Try<List<String>>> trySafe = Scope.<String>allSucceed().fork(task1).fork(task2).joinSafe();
        ...
        VTask<Either<Throwable, List<String>>> eitherResult = Scope.<String>allSucceed().fork(task1).fork(task2).joinEither();
        Either<Throwable, List<String>> either = eitherResult.runSafe().orElse(Either.left(new RuntimeException()));
        System.out.println("joinEither: " + either);
        // joinMaybe: Returns VTask<Maybe<R>> (Nothing on failure)
        VTask<Maybe<List<String>>> maybeResult = Scope.<String>allSucceed().fork(task1).fork(task2).joinMaybe();
    ...
    private static void scopeJoinerExample() {
        ...
        VTask<List<String>> result = Scope.withJoiner(joiner).fork(VTask.succeed("A")).fork(VTask.succeed("B")).fork(VTask.succeed("C")).join();
    ...
    private static void realWorldDashboardExample() {
        ...
        long elapsed = System.currentTimeMillis() - start;
        String message = result.fold(d -> String.format("Dashboard loaded:%n  User: %s%n  Orders: %s%n  Notifications: %d", d.userName, d.recentOrders, d.notificationCount), error -> "Dashboard failed: " + error.getMessage());
        System.out.println(message);
        ...
        String failMessage = failResult.fold(d -> "Dashboard: " + d.userName, error -> "Expected failure: " + error.getMessage());
    ...
    private static VTask<Dashboard> loadDashboard(String userId) {
        ...
        VTask<List<String>> ordersTask = VTask.of(() -> {
            sleep(150);
            return List.of("ORD-001", "ORD-002", "ORD-003");
        });
        VTask<Integer> notificationsTask = VTask.of(() -> {
            sleep(150);
            return 5;
        });
        return Scope.<Object>allSucceed().fork(userTask).fork(ordersTask).fork(notificationsTask).join().map(results -> {
