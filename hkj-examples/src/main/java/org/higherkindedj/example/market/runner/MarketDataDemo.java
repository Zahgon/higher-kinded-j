...
public class MarketDataDemo {
    ...
    private static final List<Symbol> SYMBOLS = List.of(new Symbol("AAPL"), new Symbol("GOOGL"), new Symbol("MSFT"));
    ...
    private static void step1_GenerateTicksWithUnfold() {
        ...
        SimulatedExchangeFeed nyseFeed = new SimulatedExchangeFeed(Exchange.NYSE, SYMBOLS, 150.0, 0.002, 42L);
        ...
        for (PriceTick tick : ticks) {
            System.out.printf("    %s  bid=%-10s ask=%-10s vol=%-5s [%s]%n", tick.symbol(), tick.bid(), tick.ask(), tick.volume(), tick.exchange().name());
        }
    ...
    private static void step2_MergeExchangeFeeds() {
        ...
        SimulatedExchangeFeed nyse = new SimulatedExchangeFeed(Exchange.NYSE, SYMBOLS, 150.0, 0.002, 42L);
        ...
        for (PriceTick tick : ticks) {
            System.out.printf("    %-6s %-5s bid=%-10s [%s]%n", tick.exchange().name(), tick.symbol(), tick.bid(), tick.exchange().region());
        }
    ...
    private static void step3_ConcurrentEnrichment() {
        ...
        SimulatedExchangeFeed feed = new SimulatedExchangeFeed(Exchange.NYSE, SYMBOLS, 150.0, 0.002, 42L);
        TickEnricher enricher = new TickEnricher(new InMemoryReferenceDataService(0), new InMemoryFxRateService(0), 8);
        ...
        for (EnrichedTick et : enriched.subList(0, Math.min(5, enriched.size()))) {
            System.out.printf("    %-5s %-20s fx=%-6s midUSD=%-10s%n", et.tick().symbol(), et.instrument().name(), et.fxRate(), et.midInUsd());
        }
        if (enriched.size() > 5) {
    ...
    private static void step4_ParallelRiskAssessment() {
        ...
        List<RiskAssessment> assessed = riskPipeline.assess(enricher.enrich(feed.ticks().take(10))).toList().run();
        ...
        for (RiskAssessment ra : assessed) {
            ...
            System.out.printf("    %-5s risk=%.2f flags=[%s]%n", ra.tick().tick().symbol(), ra.riskScore(), flagStr);
        }
    ...
    private static void step5_WindowedAggregation() {
        ...
        TickEnricher enricher = new TickEnricher(new InMemoryReferenceDataService(0), new InMemoryFxRateService(0), 8);
        RiskPipeline riskPipeline = new RiskPipeline(new RiskCalculator(), 4);
        VStream<AggregatedView> views = WindowAggregator.aggregate(riskPipeline.assess(enricher.enrich(feed.ticks().take(20))), 5);
        ...
        for (AggregatedView view : results) {
            System.out.printf("    %-5s vwap=%-10s spread=%-10s vol=%-6s ticks=%-2d maxRisk=%.2f%n", view.symbol(), view.vwap(), view.spread(), view.totalVolume(), view.tickCount(), view.maxRiskScore());
        }
        System.out.println("  Key: chunk + fold gives windowed analytics without materialising all data.\n");
    }
    ...
    private static void step6_AnomalyDetectionAndAlerts() {
        ...
        SimulatedExchangeFeed feed = // Higher volatility
        new SimulatedExchangeFeed(Exchange.NYSE, SYMBOLS, 150.0, 0.01, 42L);
        TickEnricher enricher = new TickEnricher(new InMemoryReferenceDataService(0), new InMemoryFxRateService(0), 8);
        RiskPipeline riskPipeline = // Lower thresholds
        new RiskPipeline(new RiskCalculator(0.002, 3000), 4);
        ...
        AlertDispatcher dispatcher = new AlertDispatcher(List.of(new AlertDispatcher.AlertChannel("log", alert -> {
            logCount.incrementAndGet();
        }), new AlertDispatcher.AlertChannel("webhook", alert -> {
            webhookCount.incrementAndGet();
        })));
        VStream<AggregatedView> views = WindowAggregator.aggregate(riskPipeline.assess(enricher.enrich(feed.ticks().take(30))), 5);
        ...
        System.out.println("  Dispatched to: log(" + logCount.get() + "), webhook(" + webhookCount.get() + ")");
    ...
    private static void step7_RateLimitedPublishing() {
        ...
        VStream<PriceTick> throttled = VStreamThrottle.throttle(feed.ticks(), 3, Duration.ofMillis(100));
        long start = System.currentTimeMillis();
    ...
    private static void step8_ResilientFeeds() {
        ...
        VStream<PriceTick> primaryFeed = VStream.fail(new RuntimeException("NYSE feed: Connection refused"));
        // Fallback feed
        VStream<PriceTick> fallbackFeed = new SimulatedExchangeFeed(Exchange.LSE, SYMBOLS, 150.0, 0.002, 99L).ticks();
        ...
        for (PriceTick tick : ticks) {
            System.out.printf("    %-5s %-6s bid=%-10s%n", tick.symbol(), tick.exchange().name(), tick.bid());
        }
    ...
    private static void step9_FullPipeline() {
        ...
        System.out.println("└─────────────────────────────────────────────────────────────────┘");
        System.out.println("  Feeds ──→ Merge ──→ Enrich ──→ Risk ──→ Aggregate ──→ Detect ──→ Dispatch");
        System.out.println("  All stages compose lazily — nothing runs until the terminal operation.\n");
        // Create feeds
        SimulatedExchangeFeed nyse = new SimulatedExchangeFeed(Exchange.NYSE, SYMBOLS, 150.0, 0.005, 42L);
        ...
        AlertDispatcher dispatcher = new AlertDispatcher(List.of(new AlertDispatcher.AlertChannel("log", a -> alertsDispatched.incrementAndGet()), new AlertDispatcher.AlertChannel("webhook", a -> alertsDispatched.incrementAndGet())));
        PipelineConfig config = new PipelineConfig(8, 4, 5, 30);
        // Build the pipeline
        MarketDataPipeline pipeline = new MarketDataPipeline(List.of(nyse, lse, tse), enricher, riskPipeline, detector, dispatcher, config);
