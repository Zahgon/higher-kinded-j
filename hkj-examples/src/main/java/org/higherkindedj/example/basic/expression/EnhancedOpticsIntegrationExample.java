...
public class EnhancedOpticsIntegrationExample {
    ...
    // --- Lenses ---
    static final Lens<Department, List<Employee>> employeesLens = Lens.of(Department::employees, (dept, emps) -> new Department(dept.name(), emps, dept.budgetInCents()));

    static final Lens<Department, Integer> budgetLens = Lens.of(Department::budgetInCents, (dept, budget) -> new Department(dept.name(), dept.employees(), budget));

    static final Lens<Department, String> deptNameLens = Lens.of(Department::name, (dept, name) -> new Department(name, dept.employees(), dept.budgetInCents()));

    static final Lens<Employee, Integer> salaryLens = Lens.of(Employee::salaryInCents, (emp, salary) -> new Employee(emp.name(), salary, emp.department()));

    static final Lens<Employee, String> empNameLens = Lens.of(Employee::name, (emp, name) -> new Employee(name, emp.salaryInCents(), emp.department()));

    // --- Traversal ---
    static final Traversal<List<Employee>, Employee> empListTraversal = Traversals.forList();

    // --- Iso: cents <-> dollars ---
    static final Iso<Integer, Double> centsToDollars = Iso.of(cents -> cents / 100.0, dollars -> (int) (dollars * 100));
    ...
    private static void demonstrateTraverseOver() {
        ...
        List<Employee> invalidTeam = List.of(new Employee("Alice", 850000, "Engineering"), // zero salary fails validation
        new Employee("Intern", 0, "Engineering"), new Employee("Bob", 720000, "Engineering"));
        // Validation: each employee must have a positive salary
        // If all pass, the list is returned with a "[validated]" prefix on names
        Kind<MaybeKind.Witness, List<Employee>> validResult = ForState.withState(maybeMonad, MAYBE.just(validTeam)).traverseOver(empListTraversal, emp -> {
            if (emp.salaryInCents() <= 0) {
                return MAYBE.widen(Maybe.nothing());
            }
            return MAYBE.just(new Employee("[validated] " + emp.name(), emp.salaryInCents(), emp.department()));
        }).yield();
        Kind<MaybeKind.Witness, List<Employee>> invalidResult = ForState.withState(maybeMonad, MAYBE.just(invalidTeam)).traverseOver(empListTraversal, emp -> {
            if (emp.salaryInCents() <= 0) {
                return MAYBE.widen(Maybe.nothing());
            }
            return MAYBE.just(new Employee("[validated] " + emp.name(), emp.salaryInCents(), emp.department()));
        ...
        if (valid.isJust()) {
            valid.get().forEach(e -> System.out.println("    - " + e.name() + " ($" + e.salaryInCents() / 100.0 + ")"));
        }
    ...
    private static void demonstrateModifyThrough() {
        ...
        Kind<IdKind.Witness, List<Employee>> result = ForState.withState(idMonad, Id.of(team)).modifyThrough(empListTraversal, emp -> new Employee(emp.name(), (int) (emp.salaryInCents() * 1.10), emp.department())).yield();
        ...
        updatedTeam.forEach(e -> System.out.println("    - " + e.name() + ": $" + e.salaryInCents() / 100.0));
        System.out.println();
    }
    ...
    private static void demonstrateModifyThroughWithLens() {
        ...
        Monad<IdKind.Witness> idMonad = Instances.monad(id());
        List<Employee> team = sampleDepartment().employees();
        Kind<IdKind.Witness, List<Employee>> result = ForState.withState(idMonad, Id.of(team)).modifyThrough(empListTraversal, empNameLens, String::toUpperCase).yield();
        List<Employee> updatedTeam = IdKindHelper.ID.unwrap(result);
    ...
    private static void demonstrateModifyVia() {
        ...
        Kind<IdKind.Witness, Department> result = ForState.withState(idMonad, Id.of(dept)).modifyVia(budgetLens, centsToDollars, dollars -> dollars * 1.10).yield();
    ...
    private static void demonstrateUpdateVia() {
        ...
        Monad<IdKind.Witness> idMonad = Instances.monad(id());
        Department dept = sampleDepartment();
        Kind<IdKind.Witness, Department> result = ForState.withState(idMonad, Id.of(dept)).updateVia(budgetLens, centsToDollars, 75000.0).yield();
        Department updated = IdKindHelper.ID.unwrap(result);
        System.out.println("  Budget before: $" + centsToDollars.get(dept.budgetInCents()));
    ...
    private static void demonstrateThrough() {
        ...
        Kind<IdKind.Witness, String> result = For.from(idMonad, Id.of(budgetInCents)).through(centsToDollars).yield((cents, dollars) -> String.format("Budget: %d cents = $%.2f", cents, dollars));
    ...
    private static void demonstrateCombinedWorkflow() {
        ...
        Monad<IdKind.Witness> idMonad = Instances.monad(id());
        Department dept = sampleDepartment();
        Kind<IdKind.Witness, String> result = ForState.withState(idMonad, Id.of(dept)).// Step 1: Rename department
        update(deptNameLens, "Platform Engineering").// Step 2: Give all employees a 15% raise (traverse + lens + modifier)
        traverse(employeesLens, empListTraversal, emp -> Id.of(new Employee(emp.name(), (int) (emp.salaryInCents() * 1.15), "Platform Engineering"))).// Step 3: Increase budget by 20% working in dollars via Iso
        modifyVia(budgetLens, centsToDollars, dollars -> dollars * 1.20).// Step 4: Produce summary
        yield(d -> {
            double totalSalaries = d.employees().stream().mapToInt(Employee::salaryInCents).sum() / 100.0;
            double budget = centsToDollars.get(d.budgetInCents());
            StringBuilder sb = new StringBuilder();
            sb.append("Department: ").append(d.name()).append("\n");
            sb.append("  Employees:\n");
            d.employees().forEach(e -> sb.append("    - ").append(e.name()).append(" ($").append(String.format("%.2f", e.salaryInCents() / 100.0)).append(", ").append(e.department()).append(")\n"));
        ...
        System.out.println("  Original: " + dept.name() + " with budget $" + centsToDollars.get(dept.budgetInCents()));
