...
public class OptionalMonoidExample {
    ...
    private static void demonstrateConfigurationFallback() {
        ...
        List<Optional<String>> configSources = List.of(// Highest priority
        loadFromCommandLine(), // Second priority
        loadFromEnvironment(), // Third priority
        loadFromConfigFile(), // Fallback
        loadFromDefaults());
    ...
    private static void demonstrateFindingBestValue() {
        ...
        List<Optional<Integer>> measurements = List.of(Optional.of(23), // Sensor failure
        Optional.empty(), Optional.of(27), Optional.of(25), // Missing data
        Optional.empty());
        ...
        // Optional[27]
        System.out.println("Highest measurement: " + highest);
        // Optional[23]
        System.out.println("Lowest measurement: " + lowest);
    ...
    private static void demonstrateCustomComparators() {
        ...
        List<Optional<Product>> offers = List.of(Optional.of(new Product("Widget", 29.99)), Optional.empty(), Optional.of(new Product("Gadget", 49.99)), Optional.of(new Product("Gizmo", 19.99)));
        ...
        // Gadget at $49.99
        System.out.println("Most expensive: " + priciest);
        // Find product with longest name
        Monoid<Optional<Product>> longestName = Monoids.maximum(Comparator.comparing(p -> p.name().length()));
        Optional<Product> longest = longestName.combineAll(offers);
        // Gadget or Widget (both 6 chars)
        System.out.println("Longest name: " + longest);
        // Find product with shortest name
        Monoid<Optional<Product>> shortestName = Monoids.minimum(Comparator.comparing(p -> p.name().length()));
