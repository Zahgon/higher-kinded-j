...
public class ConstExample {
    ...
    private static void demonstrateBasicConstCreation() {
        ...
        System.out.println("Const<C, A> holds a value of type C. The type A is phantom (not stored).\n");
    ...
    private static void demonstratePhantomTypeProperty() {
        ...
        System.out.println("The second type parameter is 'phantom' - it exists only in the type signature.\n");
        ...
        Kind2<ConstKind2.Witness, Integer, Double> afterSecond = bifunctor.second(// This function defines the phantom type transformation
        s -> 3.14, CONST.widen2(originalConst));
    ...
    private static void demonstrateBifunctorOperations() {
        ...
        System.out.println("Const implements Bifunctor: first() transforms the constant, second() only changes the" + " phantom type.\n");
        Bifunctor<ConstKind2.Witness> bifunctor = ConstBifunctor.INSTANCE;
        ...
        Kind2<ConstKind2.Witness, Integer, Integer> afterFirst = bifunctor.first(String::length, CONST.widen2(original));
        ...
        Kind2<ConstKind2.Witness, String, Double> afterSecond = bifunctor.second(i -> 2.0, CONST.widen2(original));
        ...
        Kind2<ConstKind2.Witness, Integer, String> afterBimap = bifunctor.bimap(// Transforms the constant: "hello" -> 5
        String::length, // Phantom type transformation (null-safe)
        i -> "Phantom", CONST.widen2(original));
    ...
    private static void demonstrateFoldImplementation() {
        ...
        System.out.println("Const is perfect for folds: accumulate a value whilst traversing a structure.\n");
        ...
        Const<Integer, String> finalCount = items.stream().reduce(initialCount, // Each step: increment the count using mapFirst
        // The phantom type (String) represents the "shape" we're traversing
        (acc, item) -> acc.mapFirst(count -> count + 1), (c1, c2) -> c1.mapFirst(v1 -> v1 + c2.value()));
        ...
        Const<Integer, String> totalLength = items.stream().reduce(initialLength, (acc, item) -> acc.mapFirst(length -> length + item.length()), (c1, c2) -> c1.mapFirst(v1 -> v1 + c2.value()));
    ...
    private static void demonstrateGetterPattern() {
        ...
        System.out.println("Const enables the getter pattern used in lens libraries: extract without transforming.\n");
    ...
    private static void demonstrateValidationExtraction() {
        ...
        List<ValidationResult> results = List.of(new ValidationResult(true, List.of(), "Valid data 1"), new ValidationResult(false, List.of("Error A", "Error B"), null), new ValidationResult(true, List.of(), "Valid data 2"), new ValidationResult(false, List.of("Error C"), null));
        ...
        Const<Integer, ValidationResult> validCount = results.stream().reduce(new Const<>(0), (acc, result) -> new Const<Integer, ValidationResult>(result.isValid() ? acc.value() + 1 : acc.value()), (c1, c2) -> new Const<>(c1.value() + c2.value()));
