...
public class CoyonedaExample {
    ...
    private static void mapFusionExample() {
        ...
        Coyoneda<ListKind.Witness, Integer> step1 = coyo.map(x -> {
            System.out.println("  Step 1 (multiply by 2): processing " + x);
            return x * 2;
        });
        Coyoneda<ListKind.Witness, Integer> step2 = step1.map(x -> {
            System.out.println("  Step 2 (add 10): processing " + x);
            return x + 10;
        });
        Coyoneda<ListKind.Witness, String> step3 = step2.map(x -> {
    ...
    private static void chainedTransformationsExample() {
        ...
        Coyoneda<ListKind.Witness, String> pipeline = Coyoneda.lift(names).map(// Step 1: Trim whitespace
        String::trim).map(// Step 2: Uppercase
        String::toUpperCase).map(// Step 3: Add exclamation
        s -> s + "!").map(// Step 4: Wrap in brackets
        s -> "[" + s + "]");
        ...
        Coyoneda<MaybeKind.Witness, String> nothingPipeline = Coyoneda.lift(nothing).map(String::toUpperCase).map(s -> s + "!");
