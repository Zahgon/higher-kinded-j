...
public final class PCollectionsExample {
    ...
    private static void widenAndMap() {
        ...
        Kind<ListKind.Witness, String> formatted = Instances.monadZero(list()).map(p -> String.format("£%.2f", p / 100.0), kind);
    ...
    private static void flatMapAcrossPVectorAndPStack() {
        ...
        Function<Integer, Kind<ListKind.Witness, String>> expandViaPStack = id -> LIST.widen(ConsPStack.from(List.of("order-" + id + "-A", "order-" + id + "-B")));
        Kind<ListKind.Witness, String> expanded = Instances.monadZero(list()).flatMap(expandViaPStack, orders);
    ...
    private static void traverseValidatesEntireVector() {
        ...
        Function<Integer, Kind<OptionalKind.Witness, Integer>> mustBePositive = i -> OPTIONAL.widen(i > 0 ? Optional.of(i) : Optional.empty());
        Kind<OptionalKind.Witness, Kind<ListKind.Witness, Integer>> good = ListTraverse.INSTANCE.traverse(Instances.monadError(optional()), mustBePositive, goodInputs);
        Kind<OptionalKind.Witness, Kind<ListKind.Witness, Integer>> bad = ListTraverse.INSTANCE.traverse(Instances.monadError(optional()), mustBePositive, badInputs);
