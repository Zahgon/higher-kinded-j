...
public class AlternativeConfigExample {
    ...
    private static void demonstrateOrElseChain() {
        ...
        Kind<MaybeKind.Witness, ConfigValue> config = alt.orElseAll(readFromEnvironment(key), () -> readFromSystemProperty(key), () -> readFromConfigFile(key), () -> readFromDatabase(key), () -> getDefaultValue(key));
    ...
    private static void demonstrateOrElseAllIterable() {
        ...
        List<Function<String, Kind<MaybeKind.Witness, ConfigValue>>> registry = List.of(AlternativeConfigExample::readFromEnvironment, AlternativeConfigExample::readFromSystemProperty, AlternativeConfigExample::readFromConfigFile, AlternativeConfigExample::readFromDatabase, AlternativeConfigExample::getDefaultValue);
        // Map each strategy to its candidate result, then fold with orElseAll(Iterable).
        List<Kind<MaybeKind.Witness, ConfigValue>> candidates = registry.stream().map(source -> source.apply(key)).toList();
    ...
    private static void demonstrateGuard() {
        ...
        Kind<MaybeKind.Witness, ConfigValue> validated = Instances.monadError(maybe()).flatMap(value -> {
    ...
    private static void demonstrateLazyEvaluation() {
        ...
        Kind<MaybeKind.Witness, ConfigValue> primary = MAYBE.just(new ConfigValue("found-in-primary", ConfigSource.ENVIRONMENT_VARIABLE));
        Kind<MaybeKind.Witness, ConfigValue> result = alt.orElse(primary, () -> {
    ...
    private static Maybe<String> parseValue(String input) {
        Kind<MaybeKind.Witness, String> result = alt.orElseAll(parseBoolean(input), () -> parseInt(input), () -> MAYBE.just("String: " + input));
