...
public class TrampolineExample {
    ...
    private static void factorialExample() {
        ...
        Trampoline<BigInteger> factorial1000 = bigIntegerFactorial(BigInteger.valueOf(1000), BigInteger.ONE);
        BigInteger result = factorial1000.run();
        System.out.println("Factorial of 1000 (first 50 digits): " + result.toString().substring(0, 50) + "...");
        ...
        System.out.println("Factorial of 10,000 computed successfully (overflow occurred, but stack-safe!)");
        System.out.println();
    }
    ...
    private static void compositionExample() {
        ...
        Trampoline<Integer> computation = Trampoline.done(10).map(// 20
        x -> x * 2).flatMap(// 25
        x -> Trampoline.done(x + 5)).map(// 75
        x -> x * 3);
        System.out.println("Result: " + computation.run());
        // Chain multiple computations
        Trampoline<Integer> chained = Trampoline.done(5).flatMap(// 5! = 120
        x -> factorial(x, 1L).map(Long::intValue)).map(// 12
        x -> x / 10);
