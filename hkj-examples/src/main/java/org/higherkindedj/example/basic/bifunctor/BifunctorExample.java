...
public class BifunctorExample {
    ...
    private static void demonstrateEitherBifunctor() {
        ...
        Kind2<EitherKind2.Witness, String, Integer> doubledSuccess = bifunctor.second(n -> n * 2, wrappedSuccess);
        ...
        Kind2<EitherKind2.Witness, String, Integer> enhancedError = bifunctor.first(err -> "Error Code: " + err, wrappedError);
        ...
        Kind2<EitherKind2.Witness, Integer, String> transformed = bifunctor.bimap(// Transform error: string -> length
        String::length, // Transform success: number -> formatted string
        n -> "Result: " + n, wrappedEither);
    ...
    private static void demonstrateTuple2Bifunctor() {
        ...
        Kind2<Tuple2Kind2.Witness, Integer, Integer> nameLength = bifunctor.first(String::length, wrappedPerson);
        ...
        Kind2<Tuple2Kind2.Witness, String, String> ageFormatted = bifunctor.second(age -> age + " years old", wrappedPerson);
        System.out.println("After second(): " + TUPLE2.narrow2(ageFormatted));
        // Output: Tuple2(Alice, 30 years old)
        // Transform both simultaneously
        Kind2<Tuple2Kind2.Witness, String, String> formatted = bifunctor.bimap(// Transform first
        name -> "Name: " + name, // Transform second
        age -> "Age: " + age, wrappedPerson);
    ...
    private static void demonstrateValidatedBifunctor() {
        ...
        Kind2<ValidatedKind2.Witness, List<String>, String> transformedValid = bifunctor.second(n -> "Value: " + n, wrappedValid);
        ...
        Validated<List<String>, Integer> invalidData = Validated.invalid(List.of("TOO_SMALL", "OUT_OF_RANGE"));
        Kind2<ValidatedKind2.Witness, List<String>, Integer> wrappedInvalid = VALIDATED.widen2(invalidData);
        // Transform the error channel: add prefixes
        Kind2<ValidatedKind2.Witness, List<String>, Integer> enhancedErrors = bifunctor.first(errors -> errors.stream().map(e -> "ERROR: " + e).toList(), wrappedInvalid);
        ...
        Kind2<ValidatedKind2.Witness, String, String> normalised = bifunctor.bimap(errors -> "Validation failed: " + errors.size() + " errors", value -> "Success: " + value, wrappedInvalid);
    ...
    private static void demonstrateWriterBifunctor() {
        ...
        Kind2<WriterKind2.Witness, String, Integer> uppercaseLog = bifunctor.first(String::toUpperCase, wrappedWriter);
        ...
        Kind2<WriterKind2.Witness, String, String> formattedValue = bifunctor.second(n -> "Result: " + n, wrappedWriter);
        System.out.println("After second():  " + WRITER.narrow2(formattedValue));
        // Output: Writer(Calculated: , Result: 42)
        // Transform both channels
        Kind2<WriterKind2.Witness, List<String>, String> structured = bifunctor.bimap(// Wrap log in a list
        log -> List.of("[LOG]", log), // Format value
        value -> "Final: " + value, wrappedWriter);
    ...
    private static void apiResponseTransformation() {
        ...
        Function<UserData, ApiResponse> toApiResponse = user -> new ApiResponse(user.name(), user.email(), 200);
        Kind2<EitherKind2.Witness, ApiError, ApiResponse> apiResult = bifunctor.bimap(toApiError, toApiResponse, EITHER.widen2(internalResult));
    ...
    private static void validationPipeline() {
        ...
        Validated<List<String>, String> validationResult = Validated.invalid(List.of("ERR_001", "ERR_002"));
        // Transform to user-friendly messages
        Kind2<ValidatedKind2.Witness, String, String> userFriendly = bifunctor.bimap(errors -> "Please correct the following: " + String.join(", ", errors), value -> "✓ Validation passed: " + value, VALIDATED.widen2(validationResult));
    ...
    private static void dataMigration() {
        ...
        Kind2<Tuple2Kind2.Witness, Integer, String> migratedData = bifunctor.bimap(// Extract number from "USER_123"
        id -> Integer.parseInt(id.substring(5)), // Convert to ISO string
        timestamp -> Instant.ofEpochMilli(timestamp).toString(), TUPLE2.widen2(legacyData));
