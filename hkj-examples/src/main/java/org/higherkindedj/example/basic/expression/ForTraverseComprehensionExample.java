...
public class ForTraverseComprehensionExample {
    ...
    private static void traverseWithMaybeExample() {
        ...
        Kind<MaybeKind.Witness, List<Integer>> successResult = For.from(maybeMonad, MAYBE.just(Arrays.asList(1, 2, 3))).traverse(ListTraverse.INSTANCE, list -> LIST.widen(list), (Integer i) -> MAYBE.just(i * 2)).yield((original, traversed) -> LIST.narrow(traversed));
        ...
        Kind<MaybeKind.Witness, List<Integer>> failResult = For.from(maybeMonad, MAYBE.just(Arrays.asList(1, 2, 3))).traverse(ListTraverse.INSTANCE, list -> LIST.widen(list), (Integer i) -> i == 2 ? MAYBE.<Integer>nothing() : MAYBE.just(i * 2)).yield((original, traversed) -> LIST.narrow(traversed));
    ...
    private static void sequenceExample() {
        ...
        Kind<IdKind.Witness, List<Integer>> result = For.from(idMonad, Id.of(kindList)).sequence(ListTraverse.INSTANCE, Function.identity()).yield((original, sequenced) -> LIST.narrow(sequenced));
    ...
    private static void flatTraverseExample() {
        final MonadError<MaybeKind.Witness, Unit> maybeMonad = Instances.monadError(maybe());
        Kind<MaybeKind.Witness, List<Integer>> result = For.from(maybeMonad, MAYBE.just(Arrays.asList(1, 2))).flatTraverse(ListTraverse.INSTANCE, Instances.monadZero(list()), list -> LIST.widen(list), (Integer i) -> MAYBE.<Kind<ListKind.Witness, Integer>>just(LIST.widen(Arrays.asList(i, i * 10)))).yield((original, traversed) -> LIST.narrow(traversed));
    ...
    private static void forPathMaybeTraverseExample() {
        // Success case
        MaybePath<List<Integer>> successResult = ForPath.from(Path.just(Arrays.asList(1, 2, 3))).traverse(ListTraverse.INSTANCE, list -> LIST.widen(list), (Integer i) -> MAYBE.just(i * 2)).yield((original, traversed) -> LIST.narrow(traversed));
        Maybe<List<Integer>> successMaybe = successResult.run();
        System.out.println("MaybePath traverse (success): " + successMaybe);
        // Just([2, 4, 6])
        // Short-circuit case
        MaybePath<List<Integer>> failResult = ForPath.from(Path.just(Arrays.asList(1, 2, 3))).traverse(ListTraverse.INSTANCE, list -> LIST.widen(list), (Integer i) -> i == 2 ? MAYBE.<Integer>nothing() : MAYBE.just(i * 2)).yield((original, traversed) -> LIST.narrow(traversed));
    ...
    private static void forPathEitherTraverseExample() {
        // Success case
        EitherPath<String, List<Integer>> successResult = ForPath.from(Path.<String, List<Integer>>right(Arrays.asList(1, 2, 3))).traverse(ListTraverse.INSTANCE, list -> LIST.widen(list), (Integer i) -> EitherKindHelper.EITHER.widen(Either.<String, Integer>right(i * 2))).yield((original, traversed) -> LIST.narrow(traversed));
        System.out.println("EitherPath traverse (success): " + successResult.run());
        // Right([2, 4, 6])
        // Short-circuit with Left
        EitherPath<String, List<Integer>> failResult = ForPath.from(Path.<String, List<Integer>>right(Arrays.asList(1, 2, 3))).traverse(ListTraverse.INSTANCE, list -> LIST.widen(list), (Integer i) -> i == 2 ? EitherKindHelper.EITHER.widen(Either.<String, Integer>left("error at 2")) : EitherKindHelper.EITHER.widen(Either.<String, Integer>right(i * 2))).yield((original, traversed) -> LIST.narrow(traversed));
