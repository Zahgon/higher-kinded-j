...
public class EnhancedOrderWorkflowDemo {
    ...
    private static EnhancedOrderWorkflow createWorkflow() {
        return new EnhancedOrderWorkflow(new InMemoryCustomerService(), new InMemoryInventoryService(), new InMemoryDiscountService(), new InMemoryPaymentService(), new InMemoryShippingService(), new InMemoryNotificationService(), WorkflowConfig.defaults());
    }
    ...
    private static void runScenario1_SuccessfulOrderWithContext(EnhancedOrderWorkflow workflow) {
        ...
        ScopedValue.where(OrderContext.TRACE_ID, traceId).where(OrderContext.TENANT_ID, tenantId).where(OrderContext.DEADLINE, deadline).where(OrderContext.PRINCIPAL, user).where(OrderContext.ROLES, roles).run(() -> {
            var result = workflow.process(request).run().runSafe();
            result.fold(either -> either.fold(error -> {
    ...
    private static void runScenario2_ParallelInventoryCheck(EnhancedOrderWorkflow workflow) {
        ...
        long start = System.currentTimeMillis();
        ScopedValue.where(OrderContext.TRACE_ID, traceId).where(OrderContext.TENANT_ID, "demo-tenant").where(OrderContext.DEADLINE, Instant.now().plus(Duration.ofSeconds(30))).run(() -> {
            // Build validated order lines for the parallel check
            var orderId = OrderId.generate();
            var validatedLines = List.of(createValidatedOrderLine("PROD-001", 2), createValidatedOrderLine("PROD-002", 1));
            System.out.println("  [Invoking parallel warehouse checks on virtual threads]");
            System.out.println("  [Context (trace ID) automatically propagates to each thread]\n");
            // Actually invoke the parallel reservation method
            var result = workflow.reserveInventoryParallel(orderId, validatedLines).run().runSafe();
            result.fold(either -> either.fold(error -> {
    ...
    private static void runScenario3_MultiTenantIsolation(EnhancedOrderWorkflow workflow) {
        ...
        ScopedValue.where(OrderContext.TRACE_ID, OrderContext.generateTraceId()).where(OrderContext.TENANT_ID, "acme-corp").where(OrderContext.DEADLINE, Instant.now().plus(Duration.ofSeconds(30))).run(() -> {
            System.out.println("    Current tenant: " + OrderContext.TENANT_ID.get() + " [trace=" + OrderContext.shortTraceId() + "]");
            // Would process order here - each step sees "acme-corp" as tenant
        });
        // Process for Tenant B
        System.out.println("\n  Processing for Tenant B (widgets-inc):");
        ScopedValue.where(OrderContext.TRACE_ID, OrderContext.generateTraceId()).where(OrderContext.TENANT_ID, "widgets-inc").where(OrderContext.DEADLINE, Instant.now().plus(Duration.ofSeconds(30))).run(() -> {
            System.out.println("    Current tenant: " + OrderContext.TENANT_ID.get() + " [trace=" + OrderContext.shortTraceId() + "]");
    ...
    private static void runScenario4_DeadlineEnforcement(EnhancedOrderWorkflow workflow) {
        ...
        System.out.println("  Trace ID: " + traceId.substring(0, 8) + "...\n");
        ScopedValue.where(OrderContext.TRACE_ID, traceId).where(OrderContext.TENANT_ID, "demo-tenant").where(OrderContext.DEADLINE, tightDeadline).run(() -> {
            // Simulate some work
            try {
                // This exceeds the deadline
                Thread.sleep(100);
        ...
        ScopedValue.where(OrderContext.TRACE_ID, OrderContext.generateTraceId()).where(OrderContext.TENANT_ID, "demo-tenant").where(OrderContext.DEADLINE, generousDeadline).run(() -> {
            // Simulate some work
            try {
                Thread.sleep(100);
            } catch (InterruptedException e) {
            ...
            if (OrderContext.isDeadlineExceeded()) {
                ...
                System.out.println("  Still within deadline");
                System.out.println("  Remaining time: " + OrderContext.remainingTime().toMillis() + "ms");
            }
        });
        System.out.println();
    }
    ...
    private static OrderRequest createSampleRequest(String customerId, Optional<String> promoCode) {
        return new OrderRequest(customerId, List.of(new OrderLineRequest("PROD-001", 2), new OrderLineRequest("PROD-002", 1)), promoCode, new ShippingAddress("Test User", "123 Test Street", "London", "SW1A 1AA", "GB"), new PaymentMethod.CreditCard("4242424242424242", "12", "2025", "123"));
    }
}
