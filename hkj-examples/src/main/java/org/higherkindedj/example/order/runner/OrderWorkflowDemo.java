...
public class OrderWorkflowDemo {
    ...
    private static void runSuccessfulOrder(OrderWorkflow workflow) {
        var request = new OrderRequest(// Alice Smith, GOLD tier
        "CUST-001", List.of(new OrderLineRequest("PROD-001", 2), new OrderLineRequest("PROD-002", 1)), // No promo code
        Optional.empty(), new ShippingAddress("Alice Smith", "123 High Street", "London", "SW1A 1AA", "GB"), new PaymentMethod.CreditCard("4242424242424242", "12", "2025", "123"));
    ...
    private static void runInvalidCustomerOrder(OrderWorkflow workflow) {
        var request = new OrderRequest(// Non-existent customer
        "CUST-999", List.of(new OrderLineRequest("PROD-001", 1)), Optional.empty(), new ShippingAddress("Unknown Person", "456 Low Street", "Manchester", "M1 1AA", "GB"), new PaymentMethod.CreditCard("4242424242424242", "12", "2025", "123"));
        var result = workflow.process(request).run();
        result.fold(error -> {
    ...
    private static void runOrderWithPromoCode(OrderWorkflow workflow) {
        var request = new OrderRequest(// Bob Jones, SILVER tier
        "CUST-002", List.of(new OrderLineRequest("PROD-001", 3), new OrderLineRequest("PROD-003", 2)), // 20% off promo code
        Optional.of("SAVE20"), new ShippingAddress("Bob Jones", "789 Medium Street", "Birmingham", "B1 1AA", "GB"), new PaymentMethod.DebitCard("5555555555554444", "06", "2026"));
        var result = workflow.process(request).run();
        result.fold(error -> {
            System.out.println("Order failed: " + error.message());
            return null;
