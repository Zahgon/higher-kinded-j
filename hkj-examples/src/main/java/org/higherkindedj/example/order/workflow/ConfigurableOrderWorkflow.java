...
public class ConfigurableOrderWorkflow {
    ...
    @Nullable
    private final SplitShipmentWorkflow splitShipmentWorkflow;
     ...
     */
    public ConfigurableOrderWorkflow(CustomerService customerService, InventoryService inventoryService, DiscountService discountService, PaymentService paymentService, ShippingService shippingService, NotificationService notificationService, WarehouseService warehouseService, WorkflowConfig config) {
        this.customerService = customerService;
        this.shippingService = shippingService;
        this.baseWorkflow = new OrderWorkflow(customerService, inventoryService, discountService, paymentService, shippingService, notificationService, config);
        this.partialFulfilmentWorkflow = new PartialFulfilmentWorkflow(inventoryService, paymentService, shippingService, notificationService);
     ...
     * @param config workflow configuration
     */
    public ConfigurableOrderWorkflow(CustomerService customerService, InventoryService inventoryService, DiscountService discountService, PaymentService paymentService, ShippingService shippingService, NotificationService notificationService, WorkflowConfig config) {
        this.customerService = customerService;
        this.shippingService = shippingService;
        this.baseWorkflow = new OrderWorkflow(customerService, inventoryService, discountService, paymentService, shippingService, notificationService, config);
        this.partialFulfilmentWorkflow = new PartialFulfilmentWorkflow(inventoryService, paymentService, shippingService, notificationService);
     ...
     */
    public EitherPath<OrderError, PartialFulfilmentResult> processWithPartialFulfilment(ValidatedOrder order) {
        throw new UnsupportedOperationException("STUB: not implemented");
    }
     ...
     */
    public EitherPath<OrderError, SplitShipmentResult> processWithSplitShipments(ValidatedOrder order, InventoryReservation reservation) {
        throw new UnsupportedOperationException("STUB: not implemented");
    }
    ...
    private OrderResult attemptPartialFulfilment(OrderRequest request, OrderError originalError) {
        ...
        return validatedOrderResult.run().fold(buildError -> {
            throw new WorkflowException(buildError);
        }, validatedOrder -> executePartialFulfilment(validatedOrder, customer, originalError));
    }

    private OrderResult executePartialFulfilment(ValidatedOrder validatedOrder, Customer customer, OrderError originalError) {
        var partialResult = partialFulfilmentWorkflow.process(validatedOrder);
        return partialResult.run().fold(partialError -> {
    ...
    private EitherPath<OrderError, ValidatedOrder> buildValidatedOrderForPartialFulfilment(OrderRequest request, Customer customer) {
        // Validate shipping address first
        return Path.either(shippingService.validateAddress(request.shippingAddress())).map(validAddress -> createValidatedOrder(request, customer, validAddress));
    }

    private ValidatedOrder createValidatedOrder(OrderRequest request, Customer customer, ValidatedShippingAddress validAddress) {
        var lines = request.lines().stream().map(line -> ValidatedOrderLine.of(new ProductId(line.productId()), new Product(new ProductId(line.productId()), "Product " + line.productId(), "Description", Money.gbp("10.00"), "General", true), line.quantity())).toList();
        return new ValidatedOrder(OrderId.generate(), customer.id(), customer, lines, Optional.empty(), validAddress, request.paymentMethod(), ValidatedOrder.calculateSubtotal(lines), Instant.now());
    }

    private RetryPolicy createRetryPolicy() {
        var retryConfig = config.retryConfig();
        return RetryPolicy.of(retryConfig.maxRetries(), retryConfig.initialDelay(), retryConfig.maxDelay(), retryConfig.backoffMultiplier());
    }
