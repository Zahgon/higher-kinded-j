...
public class OrderWorkflow {
     ...
     */
    public record ProcessingState(ValidatedShippingAddress address, Customer customer, ValidatedOrder order, InventoryReservation reservation, DiscountResult discount, PaymentConfirmation payment, ShipmentInfo shipment, NotificationResult notification) {
         ...
         */
        static ProcessingState initial(ValidatedShippingAddress address, Customer customer, ValidatedOrder order) {
    ...
    static final Lens<ProcessingState, InventoryReservation> reservationLens = Lens.of(ProcessingState::reservation, (s, v) -> new ProcessingState(s.address(), s.customer(), s.order(), v, s.discount(), s.payment(), s.shipment(), s.notification()));

    static final Lens<ProcessingState, DiscountResult> discountLens = Lens.of(ProcessingState::discount, (s, v) -> new ProcessingState(s.address(), s.customer(), s.order(), s.reservation(), v, s.payment(), s.shipment(), s.notification()));

    static final Lens<ProcessingState, PaymentConfirmation> paymentLens = Lens.of(ProcessingState::payment, (s, v) -> new ProcessingState(s.address(), s.customer(), s.order(), s.reservation(), s.discount(), v, s.shipment(), s.notification()));

    static final Lens<ProcessingState, ShipmentInfo> shipmentLens = Lens.of(ProcessingState::shipment, (s, v) -> new ProcessingState(s.address(), s.customer(), s.order(), s.reservation(), s.discount(), s.payment(), v, s.notification()));

    static final Lens<ProcessingState, NotificationResult> notificationLens = Lens.of(ProcessingState::notification, (s, v) -> new ProcessingState(s.address(), s.customer(), s.order(), s.reservation(), s.discount(), s.payment(), s.shipment(), v));
     ...
     */
    public OrderWorkflow(CustomerService customerService, InventoryService inventoryService, DiscountService discountService, PaymentService paymentService, ShippingService shippingService, NotificationService notificationService, WorkflowConfig config) {
     ...
     */
    public OrderWorkflow(CustomerService customerService, InventoryService inventoryService, DiscountService discountService, PaymentService paymentService, ShippingService shippingService, NotificationService notificationService) {
        this(customerService, inventoryService, discountService, paymentService, shippingService, notificationService, WorkflowConfig.defaults());
    }
    ...
    // Workflow Steps
    // -------------------------------------------------------------------------
    private EitherPath<OrderError, ValidatedShippingAddress> validateShippingAddress(ShippingAddress address) {
    ...
    private EitherPath<OrderError, Customer> lookupAndValidateCustomer(CustomerId customerId) {
        ...
        Kind<EitherKind.Witness<OrderError>, Customer> result = For.from(monad, lift(lookupCustomer(customerId))).from(found -> lift(validateCustomerEligibility(found))).yield((found, validated) -> validated);
        return Path.either(EITHER.narrow(result));
    }

    private EitherPath<OrderError, ValidatedOrder> buildValidatedOrder(OrderId orderId, OrderRequest request, Customer customer, ValidatedShippingAddress validAddress) {
        var lines = request.lines().stream().map(line -> createValidatedLine(line.productId(), line.quantity())).toList();
        var subtotal = ValidatedOrder.calculateSubtotal(lines);
        return validatePromoCodeIfPresent(request.promoCode()).map(validatedPromoCode -> new ValidatedOrder(orderId, customer.id(), customer, lines, validatedPromoCode, validAddress, request.paymentMethod(), subtotal, Instant.now()));
    }

    private EitherPath<OrderError, Optional<PromoCode>> validatePromoCodeIfPresent(Optional<String> promoCode) {
        return promoCode.<EitherPath<OrderError, Optional<PromoCode>>>map(code -> Path.either(discountService.validatePromoCode(code)).map(Optional::of)).orElse(Path.right(Optional.empty()));
    }

    private ValidatedOrderLine createValidatedLine(String productIdStr, int quantity) {
        var productId = new ProductId(productIdStr);
        var product = new Product(productId, "Product " + productIdStr, "Description", Money.gbp("10.00"), "General", true);
        return ValidatedOrderLine.of(productId, product, quantity);
    }

    private Either<OrderError, InventoryReservation> reserveInventory(OrderId orderId, List<ValidatedOrderLine> lines) {
        return inventoryService.reserve(orderId, lines);
    }

    private Either<OrderError, DiscountResult> applyDiscounts(ValidatedOrder order, Customer customer) {
        return order.promoCode().<Either<OrderError, DiscountResult>>map(code -> discountService.applyPromoCode(code, order.subtotal())).orElseGet(() -> {
    ...
    private Either<OrderError, PaymentConfirmation> processPayment(ValidatedOrder order, DiscountResult discount) {
        return paymentService.processPayment(order.orderId(), discount.finalTotal(), order.paymentMethod());
    }

    private Either<OrderError, ShipmentInfo> createShipment(ValidatedOrder order, ValidatedShippingAddress address) {
        return shippingService.createShipment(order.orderId(), address, order.lines());
    }

    private Either<OrderError, NotificationResult> sendNotifications(ValidatedOrder order, Customer customer, DiscountResult discount) {
        return notificationService.sendOrderConfirmation(order.orderId(), customer, discount.finalTotal()).fold(error -> Either.right(NotificationResult.none()), Either::right);
    }
    ...
    private static OrderResult toOrderResult(ProcessingState s) {
        var auditLog = AuditLog.EMPTY.append(AuditLog.of("ORDER_CREATED", "Order " + s.order().orderId() + " created")).append(AuditLog.of("PAYMENT_PROCESSED", "Transaction " + s.payment().transactionId())).append(AuditLog.of("SHIPMENT_CREATED", "Tracking " + s.shipment().trackingNumber()));
        return new OrderResult(s.order().orderId(), s.order().customerId(), s.discount().finalTotal(), s.payment().transactionId(), s.shipment().trackingNumber(), s.shipment().estimatedDelivery(), auditLog);
    }
}
