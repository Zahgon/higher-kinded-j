...
public class SplitShipmentWorkflow {
     ...
     */
    public EitherPath<OrderError, SplitShipmentResult> createSplitShipments(ValidatedOrder order, InventoryReservation reservation) {
        throw new UnsupportedOperationException("STUB: not implemented");
    }

    private EitherPath<OrderError, SplitShipmentResult> createSingleShipment(ValidatedOrder order, InventoryReservation reservation) {
        return Path.either(shippingService.createShipment(order.orderId(), order.shippingAddress(), order.lines()).map(shipment -> SplitShipmentResult.single(order.orderId(), shipment)));
    }
     ...
     */
    private EitherPath<OrderError, List<ShipmentInfo>> createWarehouseShipments(OrderId orderId, Map<String, List<InventoryReservation.ReservedItem>> byWarehouse, ValidatedShippingAddress address) {
        ...
        for (var entry : byWarehouse.entrySet()) {
            accumulated = accumulated.via(shipments -> lookupWarehouseAndCreateShipment(orderId, entry.getKey(), entry.getValue(), address).map(shipment -> appendToList(shipments, shipment)));
        }
        return accumulated;
    }
     ...
     */
    private EitherPath<OrderError, ShipmentInfo> lookupWarehouseAndCreateShipment(OrderId orderId, String warehouseId, List<InventoryReservation.ReservedItem> items, ValidatedShippingAddress address) {
        return warehouseService.getWarehouse(warehouseId).map(warehouse -> createWarehouseShipment(orderId, warehouse, items, address)).orElseGet(() -> Path.left(OrderError.SystemError.unexpected("Warehouse not found: " + warehouseId, new IllegalStateException("Unknown warehouse"))));
    }
    ...
    private EitherPath<OrderError, ShipmentInfo> createWarehouseShipment(OrderId orderId, WarehouseInfo warehouse, List<InventoryReservation.ReservedItem> items, ValidatedShippingAddress address) {
        return ForPath.// Step 1: Create shipment from warehouse
        from(Path.either(shippingService.createShipmentFromWarehouse(orderId, warehouse, items, address))).// Step 2: Add warehouse shipping cost
        let(shipment -> Money.gbp(warehouse.shippingCost())).// Step 3: Update shipment with cost
