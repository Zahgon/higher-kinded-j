...
public class InMemoryShippingService implements ShippingService {
    ...
    private static final Set<String> EUROPEAN_COUNTRIES = Set.of("DE", "FR", "IT", "ES", "NL", "BE", "AT", "PL", "SE", "DK", "FI", "NO", "IE");
    ...
    @Override
    public Either<OrderError, ShipmentInfo> createShipment(OrderId orderId, ValidatedShippingAddress address, List<ValidatedOrderLine> lines) {
        throw new UnsupportedOperationException("STUB: not implemented");
    }

    @Override
    public Either<OrderError, ShipmentInfo> createShipmentFromWarehouse(OrderId orderId, WarehouseInfo warehouse, List<InventoryReservation.ReservedItem> items, ValidatedShippingAddress address) {
        throw new UnsupportedOperationException("STUB: not implemented");
    }

    private Money calculateShippingCost(ShippingZone zone) {
        var amount = switch(zone) {
            case DOMESTIC ->
                new BigDecimal("3.99");
            case EUROPE ->
                new BigDecimal("9.99");
