...
public final class ReaderPath<R, A> implements Chainable<A> {
    ...
    @Override
    public <B, C> ReaderPath<R, C> zipWith(Combinable<B> other, BiFunction<? super A, ? super B, ? extends C> combiner) {
        throw new UnsupportedOperationException("STUB: not implemented");
    }
     ...
     */
    public <B, C, D> ReaderPath<R, D> zipWith3(ReaderPath<R, B> second, ReaderPath<R, C> third, Function3<? super A, ? super B, ? super C, ? extends D> combiner) {
        throw new UnsupportedOperationException("STUB: not implemented");
    }
    ...
    /**
     * Lifts this computation into a larger environment via a {@link Getter}.
     *
     * <p>This is the optic-aware form of {@link #local(Function)}. It accepts any read-only optic
     * that can extract an {@code R} from a larger environment {@code R2} and returns a new {@code
     * ReaderPath<R2, A>} that runs this computation against the extracted sub-environment.
     *
     * <pre>{@code
     * Getter<AppEnv, DbConfig> dbConfig = Getter.of(AppEnv::dbConfig);
     * ReaderPath<DbConfig, User> loadUser = ...;
     *
     * ReaderPath<AppEnv, User> appUser = loadUser.magnify(dbConfig);
     * }</pre>
     *
     * <p><b>Naming convention:</b> {@code magnify} is reserved for optic overloads; the bare {@link
     * #local(Function)} overload remains available as the escape hatch for environment adaptations
     * that are not naturally expressed as optics. Both compile to the same underlying Reader
     * transformation; the names exist to signal the caller's intent at the use site.
     *
     * @param getter the {@link Getter} that extracts the original environment from a larger one; must
     *     not be null
     * @param <R2> the larger environment type
     * @return a ReaderPath that runs against the larger environment
     * @throws NullPointerException if {@code getter} is null
     */
    public <R2> ReaderPath<R2, A> magnify(Getter<R2, R> getter) {
        throw new UnsupportedOperationException("STUB: not implemented");
    }

    /**
     * Lifts this computation into a larger environment via a {@link FocusPath}.
     *
     * <p>Equivalent to {@link #magnify(Getter)} with {@code path.toLens()::get}. This overload lets
     * callers pass the {@link FocusPath} instances generated by {@code @GenerateFocus} directly,
     * without unwrapping the underlying optic.
     *
     * <pre>{@code
     * // Generated by @GenerateFocus on the AppEnv record
     * FocusPath<AppEnv, DbConfig> dbConfig = AppEnvFocus.dbConfig();
     * ReaderPath<DbConfig, User> loadUser = ...;
     *
     * ReaderPath<AppEnv, User> appUser = loadUser.magnify(dbConfig);
     * }</pre>
     *
     * @param path the {@link FocusPath} that extracts the original environment from a larger one;
     *     must not be null
     * @param <R2> the larger environment type
     * @return a ReaderPath that runs against the larger environment
     * @throws NullPointerException if {@code path} is null
     */
    public <R2> ReaderPath<R2, A> magnify(FocusPath<R2, R> path) {
        throw new UnsupportedOperationException("STUB: not implemented");
    }
