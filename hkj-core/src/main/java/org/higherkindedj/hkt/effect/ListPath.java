...
public final class ListPath<A> implements Chainable<A> {
     ...
     */
    @Override
    public <B, C> ListPath<C> zipWith(Combinable<B> other, BiFunction<? super A, ? super B, ? extends C> combiner) {
        throw new UnsupportedOperationException("STUB: not implemented");
    }
     ...
     */
    public <B, C, D> ListPath<D> zipWith3(ListPath<B> second, ListPath<C> third, Function3<? super A, ? super B, ? super C, ? extends D> combiner) {
        throw new UnsupportedOperationException("STUB: not implemented");
    }
    ...
    /**
     * Reduces the list to a single value of the same type using an identity element and an
     * associative combining operator.
     *
     * <p>This is the monoid-style fold. It mirrors {@code VStreamPath.fold} and {@code
     * VStreamContext.fold}, so the same reduction reads identically across sequence-like paths.
     *
     * <pre>{@code
     * int sum = ListPath.of(1, 2, 3).map(x -> x * 2).fold(0, Integer::sum); // 12
     * }</pre>
     *
     * @param identity the identity element returned for an empty list
     ...
     *
     * <p>Returns {@code monoid.empty()} for an empty list. This is the {@link
     * org.higherkindedj.hkt.Foldable Foldable}-style fold and matches {@code VStreamPath.foldMap}.
     *
     * <pre>{@code
     * String csv = ListPath.of(1, 2, 3).foldMap(Monoids.string(), i -> i + ",");
     * }</pre>
     *
