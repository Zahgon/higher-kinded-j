...
public final class TrampolineUtils {
     ...
     */
    public static <F extends WitnessArity<TypeArity.Unary>, A, B> Kind<F, List<B>> traverseListStackSafe(final List<A> list, final Function<? super A, ? extends Kind<F, ? extends B>> f, final Applicative<F> applicative) {
     ...
     */
    private static <F extends WitnessArity<TypeArity.Unary>, A, B> Trampoline<Kind<F, List<B>>> traverseListTrampoline(final List<A> list, final int index, final Kind<F, List<B>> accumulator, final Function<? super A, ? extends Kind<F, ? extends B>> f, final Applicative<F> applicative) {
        ...
        final Kind<F, List<B>> nextAccumulator = applicative.map2(accumulator, effectOfB, (listB, b) -> {
            // Create new list to ensure correctness for multi-branch Applicatives (e.g., List)
            List<B> newList = new ArrayList<>(listB);
            newList.add((B) b);
            return newList;
        });
        return Trampoline.defer(() -> traverseListTrampoline(list, index + 1, nextAccumulator, f, applicative));
    }
     ...
     */
    public static <F extends WitnessArity<TypeArity.Unary>, A> Kind<F, List<A>> sequenceStackSafe(final List<Kind<F, A>> effects, final Applicative<F> applicative) {
        throw new UnsupportedOperationException("STUB: not implemented");
    }
}
