...
public interface Selective<F extends WitnessArity<TypeArity.Unary>> extends Applicative<F> {
     ...
     *     appropriate handler.
     */
    default <A, B, C> Kind<F, C> branch(Kind<F, Choice<A, B>> fab, Kind<F, Function<A, C>> fl, Kind<F, Function<B, C>> fr) {
     ...
     */
    private <E, A> Kind<F, Choice<E, A>> selectOrElse(Kind<F, Choice<E, A>> first, Kind<F, Choice<E, A>> second) {
        Kind<F, Choice<E, Choice<E, A>>> transformed = map(choice -> choice.isRight() ? Selective.right(// Use Selective.right() twice
        Selective.right(choice.getRight())) : // Use Selective.left()
        Selective.left(choice.getLeft()), first);
        Kind<F, Function<E, Choice<E, A>>> getSecond = map(choiceA -> (Function<E, Choice<E, A>>) e -> choiceA, second);
     ...
     */
    default <E, A> Kind<F, Choice<E, A>> apS(Kind<F, Choice<E, A>> initial, List<Kind<F, Function<A, Choice<E, A>>>> functions) {
        throw new UnsupportedOperationException("STUB: not implemented");
    }
