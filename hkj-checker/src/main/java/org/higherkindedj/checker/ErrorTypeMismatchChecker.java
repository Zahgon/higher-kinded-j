...
public final class ErrorTypeMismatchChecker implements CheckVisitor {
    ...
    private static final Set<String> ERROR_TYPED = Set.of("org.higherkindedj.hkt.effect.EitherPath", "org.higherkindedj.hkt.effect.ValidationPath");
    ...
    private void inspect(MethodInvocationTree node, TreePath path) {
        ...
        if (receiverFqn == null || !ERROR_TYPED.contains(receiverFqn)) {
            // not an error-typed Path receiver
            return;
        }
        TypeMirror receiverE = firstTypeArg(receiver);
        if (!concrete(receiverE)) {
            // receiver error type is a type variable / wildcard: no false positives
            return;
        }
        ExpressionTree arg0 = node.getArguments().getFirst();
        List<TypeMirror> steps = lambdaForm ? LambdaReturns.lambdaReturnTypes(trees, path, arg0) : singleton(LambdaReturns.typeOf(trees, path, arg0));
        ...
        for (TypeMirror step : steps) {
            if (!receiverFqn.equals(declaredFqn(step))) {
                // different category — a separate concern, out of scope here
                continue;
            }
            TypeMirror stepE = firstTypeArg(step);
            if (!concrete(stepE)) {
                // type variable / wildcard / unresolved: no false positives
                continue;
            }
            if (!types.isAssignable(stepE, receiverE)) {
                trees.printMessage(severity, DiagnosticMessages.errorTypeMismatch(method, simpleName(receiverE), simpleName(stepE)), node, path.getCompilationUnit());
                return;
            }
        }
    }
    ...
    private static TypeMirror firstTypeArg(TypeMirror t) {
        return t instanceof DeclaredType dt && !dt.getTypeArguments().isEmpty() ? dt.getTypeArguments().getFirst() : null;
    }
