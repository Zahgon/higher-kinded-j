...
final class ForStepGenerator {
    ...
    // MonadicStepsN
    // ---------------------------------------------------------------------------
    private static void generateMonadicSteps(int n, int maxArity, ProcessingEnvironment processingEnv) throws IOException {
        ...
        sb.append("@Generated\n");
        sb.append("public final class ").append(className).append("<M extends WitnessArity<TypeArity.Unary>");
        for (int i = 0; i < n; i++) {
        ...
        sb.append("  ").append(className).append("(Monad<M> monad, Kind<M, Tuple").append(n).append("<");
        appendTypeParams(sb, n);
        sb.append(">> computation) {\n");
    ...
    // FilterableStepsN
    // ---------------------------------------------------------------------------
    private static void generateFilterableSteps(int n, int maxArity, ProcessingEnvironment processingEnv) throws IOException {
        boolean terminal = (n == maxArity);
        ...
        sb.append(">> computation;\n\n");
        // Constructor (package-private)
        sb.append("  ").append(className).append("(MonadZero<M> monad, Kind<M, Tuple").append(n).append("<");
        appendTypeParams(sb, n);
        sb.append(">> computation) {\n");
        sb.append("    this.monad = monad;\n");
        sb.append("    this.computation = computation;\n");
        ...
        if (!terminal) {
            ...
            sb.append("                matcher\n");
            sb.append("                    .apply(t)\n");
            sb.append("                    .map(").append(nextType.toLowerCase()).append(" -> monad.of(Tuple.of(");
            for (int i = 0; i < n; i++) {
                sb.append("t._").append(i + 1).append("(), ");
            }
            sb.append(nextType.toLowerCase()).append(")))\n");
    ...
    // ---------------------------------------------------------------------------
    private static void generateToStateSpread(StringBuilder sb, int n, String monadType, String returnType) {
    ...
    private static void generateToStateTuple(StringBuilder sb, int n, String monadType, String returnType) {
     ...
     */
    private static void generateParMethods(StringBuilder sb, int n, int maxArity, String monadType, String stepsPrefix) {
        for (int k = 2; k <= 4; k++) {
            int targetArity = n + k;
     ...
     * targetArity.
     */
    private static void generateParMethod(StringBuilder sb, int n, int k, int targetArity, String monadType, String stepsPrefix) {
     ...
     * A-L.
     */
    private static void generateTraverseMethods(StringBuilder sb, int n, String monadType, String stepsPrefix) {
        int next = n + 1;
        String nextClassName = stepsPrefix + next;
        // traverse()
        sb.append("  public <TT extends WitnessArity<TypeArity.Unary>, TC, TR> ").append(nextClassName).append("<M");
        for (int i = 0; i < n; i++) {
            sb.append(", ").append(TYPE_PARAMS[i]);
        }
        ...
        sb.append("  }\n\n");
        // sequence()
        sb.append("  public <TT extends WitnessArity<TypeArity.Unary>, TR> ").append(nextClassName).append("<M");
        for (int i = 0; i < n; i++) {
            sb.append(", ").append(TYPE_PARAMS[i]);
        }
