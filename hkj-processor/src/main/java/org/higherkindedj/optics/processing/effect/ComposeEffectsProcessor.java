...
@SupportedAnnotationTypes({ "org.higherkindedj.hkt.effect.annotation.ComposeEffects", "org.higherkindedj.hkt.effect.annotation.Handles" })
...
public class ComposeEffectsProcessor extends AbstractProcessor {
    ...
    private static final ClassName EITHERF_KIND = ClassName.get("org.higherkindedj.hkt.eitherf", "EitherFKind");

    private static final ClassName EITHERF_FUNCTOR = ClassName.get("org.higherkindedj.hkt.eitherf", "EitherFFunctor");

    private static final ClassName INJECT = ClassName.get("org.higherkindedj.hkt.inject", "Inject");

    private static final ClassName INJECT_INSTANCES = ClassName.get("org.higherkindedj.hkt.inject", "InjectInstances");
    ...
    private void processComposeEffects(RoundEnvironment roundEnv) {
        for (Element element : roundEnv.getElementsAnnotatedWith(ComposeEffects.class)) {
            ...
            if (components.size() < 2 || components.size() > 4) {
                error("@ComposeEffects requires 2-4 effect algebra fields, got " + components.size(), element);
                continue;
            }
            ...
            for (RecordComponentElement comp : components) {
                for (RecordComponentElement prev : seenComponents) {
                    if (processingEnv.getTypeUtils().isSameType(comp.asType(), prev.asType())) {
                        error("Duplicate effect algebra type: " + comp.asType() + " (field '" + comp.getSimpleName() + "'). Each effect algebra may only appear once.", comp);
                        hasDuplicates = true;
                        break;
                    }
                }
    ...
    private void writeSupport(TypeElement typeElement, List<? extends RecordComponentElement> components, Element element) {
        try {
            ...
            error("Failed to generate support class for " + typeElement.getQualifiedName() + ": " + e.getMessage(), element);
        }
    }

    private void generateSupport(String packageName, String baseName, TypeElement source, List<? extends RecordComponentElement> components) throws IOException {
        ...
        TypeVariableName typeF = TypeVariableName.get("F", ParameterizedTypeName.get(WITNESS_ARITY, ClassName.get("org.higherkindedj.hkt", "TypeArity", "Unary")));
        TypeSpec.Builder supportBuilder = TypeSpec.classBuilder(supportName).addAnnotation(NULL_MARKED).addAnnotation(GENERATED).addModifiers(Modifier.PUBLIC, Modifier.FINAL).addMethod(MethodSpec.constructorBuilder().addModifiers(Modifier.PRIVATE).build());
        ...
        supportBuilder.addJavadoc("Support infrastructure for the {@link $L} effect composition.\n\n" + "<p>Provides Inject instances, a composed Functor, and a BoundSet record\n" + "for using the composed effects in Free monad programs.\n\n" + "@see $L\n", baseName, baseName);
        JavaFile.builder(packageName, supportBuilder.build()).addFileComment("Generated by hkj-processor. Do not edit.").build().writeTo(processingEnv.getFiler());
    }
    ...
    private MethodSpec generateInjectMethod(String effectName, int position, int arity) {
        String methodName = "inject" + effectName.substring(0, 1).toUpperCase(Locale.ROOT) + effectName.substring(1);
        MethodSpec.Builder builder = MethodSpec.methodBuilder(methodName).addModifiers(Modifier.PUBLIC, Modifier.STATIC).returns(INJECT).addJavadoc("Creates an Inject instance for the {@code $L} effect.\n\n" + "@return An Inject instance for embedding the effect into the composed type\n", effectName);
    ...
    private CodeBlock buildInjectExpression(int position, int arity) {
        ...
        CodeBlock inner = isLast ? CodeBlock.of("$T.injectRight()", INJECT_INSTANCES) : CodeBlock.of("$T.injectLeft()", INJECT_INSTANCES);
    ...
    private MethodSpec generateFunctorMethod(List<String> effectNames, int arity) {
        MethodSpec.Builder builder = MethodSpec.methodBuilder("functor").addModifiers(Modifier.PUBLIC, Modifier.STATIC).returns(FUNCTOR).addJavadoc("Returns a composed Functor for the combined effect type.\n\n" + "@return The composed Functor instance\n");
        ...
        if (arity == 2) {
            builder.addStatement("return $T.of($LFunctor, $LFunctor)", EITHERF_FUNCTOR, effectNames.get(0), effectNames.get(1));
        } else if (arity == 3) {
            builder.addStatement("return $T.of($LFunctor, $T.of($LFunctor, $LFunctor))", EITHERF_FUNCTOR, effectNames.get(0), EITHERF_FUNCTOR, effectNames.get(1), effectNames.get(2));
        } else {
            // arity == 4
            builder.addStatement("return $T.of($LFunctor, $T.of($LFunctor, $T.of($LFunctor, $LFunctor)))", EITHERF_FUNCTOR, effectNames.get(0), EITHERF_FUNCTOR, effectNames.get(1), EITHERF_FUNCTOR, effectNames.get(2), effectNames.get(3));
        }
        return builder.build();
    }
    ...
    private TypeSpec generateBoundSetRecord(List<String> effectNames, int arity) {
        ...
        return TypeSpec.recordBuilder("BoundSet").addModifiers(Modifier.PUBLIC, Modifier.STATIC).addTypeVariable(typeF).recordConstructor(ctorBuilder.build()).addJavadoc("Convenience record holding Bound instances for all composed effects.\n\n" + "@param <F> The composed effect witness type\n").build();
    }
    ...
    private void processHandles(RoundEnvironment roundEnv) {
        for (Element element : roundEnv.getElementsAnnotatedWith(Handles.class)) {
            ...
            if (!algebraType.getModifiers().contains(Modifier.SEALED)) {
                error("@Handles references non-sealed interface: " + algebraType.getQualifiedName(), element);
                continue;
            }
            ...
            for (String expected : expectedHandlers) {
                if (!foundHandlers.contains(expected)) {
                    ...
                    error("No handler for operation: " + opName + ". Expected method: " + expected + "(" + algebraType.getSimpleName() + "." + opName + ")", element);
                }
            }
            // Report extra handlers
            for (String found : foundHandlers) {
                if (!expectedHandlers.contains(found)) {
                    warning("Handler " + found + " doesn't match any operation in " + algebraType.getSimpleName(), element);
                }
            }
        }
    }
