...
public class NavigatorClassGenerator {
    ...
    private static final ClassName FOCUS_PATH_CLASS = ClassName.get("org.higherkindedj.optics.focus", "FocusPath");

    private static final ClassName AFFINE_PATH_CLASS = ClassName.get("org.higherkindedj.optics.focus", "AffinePath");

    private static final ClassName TRAVERSAL_PATH_CLASS = ClassName.get("org.higherkindedj.optics.focus", "TraversalPath");

    // Optional types that widen to AffinePath
    private static final Set<String> OPTIONAL_TYPES = Set.of("java.util.Optional", "org.higherkindedj.hkt.maybe.Maybe");

    // Collection types that widen to TraversalPath
    private static final Set<String> COLLECTION_TYPES = Set.of("java.util.List", "java.util.Set", "java.util.Collection");
     ...
     */
    public NavigatorClassGenerator(ProcessingEnvironment processingEnv, Set<String> navigableTypes, int maxDepth, String[] includeFields, String[] excludeFields, List<TraversableGenerator> traversableGenerators) {
    ...
    private PathKind getFieldPathKindRecursive(TypeMirror type, int depth) {
        ...
        if (matched != null) {
            PathKind spiKind = switch(matched.getCardinality()) {
                case ZERO_OR_ONE ->
                    PathKind.AFFINE;
                case ZERO_OR_MORE ->
                    PathKind.TRAVERSAL;
            };
            PathKind innerKind = getInnerPathKind(declaredType, matched.getFocusTypeArgumentIndex(), depth);
     ...
     */
    public void generateNavigators(TypeSpec.Builder focusClassBuilder, TypeElement recordElement, int currentDepth) {
        throw new UnsupportedOperationException("STUB: not implemented");
    }
     ...
     */
    private void generateNavigatorsWithPathKind(TypeSpec.Builder focusClassBuilder, TypeElement recordElement, int currentDepth, PathKind currentPathKind) {
        ...
        for (RecordComponentElement component : components) {
            ...
            if (isNavigableType(fieldType)) {
                ...
                TypeSpec navigatorClass = generateNavigatorClass(component, recordElement, fieldTypeElement, currentDepth, widenedKind);
                ...
                TraversableGenerator generator = isHardcodedWideningType(fieldType) ? null : findSpiGenerator(fieldType);
                    ...
                    if (focusIdx < declaredType.getTypeArguments().size()) {
                        ...
                        if (isNavigableType(innerType)) {
                            ...
                            if (innerTypeElement != null) {
                                PathKind spiKind = switch(generator.getCardinality()) {
                                    case ZERO_OR_ONE ->
                                        PathKind.AFFINE;
                                    case ZERO_OR_MORE ->
                                        PathKind.TRAVERSAL;
                                ...
                                TypeSpec navigatorClass = generateNavigatorClass(component, recordElement, innerTypeElement, currentDepth, widenedKind);
                                focusClassBuilder.addType(navigatorClass);
                            }
                        }
                    }
     ...
     */
    private TypeSpec generateNavigatorClass(RecordComponentElement component, TypeElement sourceRecord, TypeElement targetRecord, int currentDepth, PathKind pathKind) {
        ...
        ParameterizedTypeName delegateType = ParameterizedTypeName.get(pathClass, sourceTypeVar, targetTypeName);
        String pathKindDescription = switch(pathKind) {
            case FOCUS ->
                "FocusPath";
            case AFFINE ->
                "AffinePath (optional navigation)";
            case TRAVERSAL ->
                "TraversalPath (collection navigation)";
        };
        TypeSpec.Builder navigatorBuilder = TypeSpec.classBuilder(navigatorClassName).addModifiers(Modifier.PUBLIC, Modifier.STATIC, Modifier.FINAL).addTypeVariable(sourceTypeVar).addJavadoc("Navigator for fluent access to {@code $L} fields.\n\n" + "<p>This navigator wraps a {@link $T} and provides direct navigation methods\n" + "for all fields of {@link $T}.\n\n" + "<p>Path type: $L\n\n" + "@param <S> the source type at the root of the navigation", componentName, pathClass, targetTypeName, pathKindDescription);
        // Add delegate field
        navigatorBuilder.addField(FieldSpec.builder(delegateType, "delegate", Modifier.PRIVATE, Modifier.FINAL).build());
        // Add constructor
        navigatorBuilder.addMethod(MethodSpec.constructorBuilder().addModifiers(Modifier.PUBLIC).addParameter(delegateType, "delegate").addStatement("this.delegate = java.util.Objects.requireNonNull(delegate)").build());
     ...
     */
    private void addDelegateMethods(TypeSpec.Builder navigatorBuilder, TypeVariableName sourceTypeVar, TypeName targetTypeName, ParameterizedTypeName delegateType, PathKind pathKind) {
        switch(pathKind) {
            case FOCUS ->
                addFocusPathDelegateMethods(navigatorBuilder, sourceTypeVar, targetTypeName, delegateType);
            case AFFINE ->
                addAffinePathDelegateMethods(navigatorBuilder, sourceTypeVar, targetTypeName, delegateType);
            case TRAVERSAL ->
                addTraversalPathDelegateMethods(navigatorBuilder, sourceTypeVar, targetTypeName, delegateType);
        }
    }
     ...
     */
    private void addFocusPathDelegateMethods(TypeSpec.Builder navigatorBuilder, TypeVariableName sourceTypeVar, TypeName targetTypeName, ParameterizedTypeName delegateType) {
        // get(S source) -> A
        navigatorBuilder.addMethod(MethodSpec.methodBuilder("get").addModifiers(Modifier.PUBLIC).addParameter(sourceTypeVar, "source").returns(targetTypeName).addStatement("return delegate.get(source)").addJavadoc("Extracts the focused value from the source.\n\n" + "@param source the source structure\n" + "@return the focused value").build());
        // set(A value, S source) -> S
        navigatorBuilder.addMethod(MethodSpec.methodBuilder("set").addModifiers(Modifier.PUBLIC).addParameter(targetTypeName, "value").addParameter(sourceTypeVar, "source").returns(sourceTypeVar).addStatement("return delegate.set(value, source)").addJavadoc("Creates a new source with the focused value replaced.\n\n" + "@param value the new value\n" + "@param source the source structure\n" + "@return a new structure with the updated value").build());
        // modify(Function<A, A> f, S source) -> S
        ParameterizedTypeName functionType = ParameterizedTypeName.get(ClassName.get(Function.class), targetTypeName, targetTypeName);
        navigatorBuilder.addMethod(MethodSpec.methodBuilder("modify").addModifiers(Modifier.PUBLIC).addParameter(functionType, "f").addParameter(sourceTypeVar, "source").returns(sourceTypeVar).addStatement("return delegate.modify(f, source)").addJavadoc("Creates a new source with the focused value transformed.\n\n" + "@param f the transformation function\n" + "@param source the source structure\n" + "@return a new structure with the modified value").build());
        // toLens() -> Lens<S, A>
        ParameterizedTypeName lensType = ParameterizedTypeName.get(ClassName.get(Lens.class), sourceTypeVar, targetTypeName);
        navigatorBuilder.addMethod(MethodSpec.methodBuilder("toLens").addModifiers(Modifier.PUBLIC).returns(lensType).addStatement("return delegate.toLens()").addJavadoc("Extracts the underlying lens.\n\n" + "@return the wrapped Lens").build());
        // toPath() -> FocusPath<S, A>
        navigatorBuilder.addMethod(MethodSpec.methodBuilder("toPath").addModifiers(Modifier.PUBLIC).returns(delegateType).addStatement("return delegate").addJavadoc("Returns the underlying FocusPath.\n\n" + "@return the wrapped FocusPath").build());
    }

    /**
     * Adds delegate methods for AffinePath navigators.
     */
    private void addAffinePathDelegateMethods(TypeSpec.Builder navigatorBuilder, TypeVariableName sourceTypeVar, TypeName targetTypeName, ParameterizedTypeName delegateType) {
        // getOptional(S source) -> Optional<A>
        ParameterizedTypeName optionalType = ParameterizedTypeName.get(ClassName.get(Optional.class), targetTypeName);
        navigatorBuilder.addMethod(MethodSpec.methodBuilder("getOptional").addModifiers(Modifier.PUBLIC).addParameter(sourceTypeVar, "source").returns(optionalType).addStatement("return delegate.getOptional(source)").addJavadoc("Extracts the focused value if present.\n\n" + "@param source the source structure\n" + "@return Optional containing the value, or empty if not focused").build());
        // set(A value, S source) -> S
        navigatorBuilder.addMethod(MethodSpec.methodBuilder("set").addModifiers(Modifier.PUBLIC).addParameter(targetTypeName, "value").addParameter(sourceTypeVar, "source").returns(sourceTypeVar).addStatement("return delegate.set(value, source)").addJavadoc("Creates a new source with the focused value replaced.\n\n" + "@param value the new value\n" + "@param source the source structure\n" + "@return a new structure with the updated value").build());
        // modify(Function<A, A> f, S source) -> S
        ParameterizedTypeName functionType = ParameterizedTypeName.get(ClassName.get(Function.class), targetTypeName, targetTypeName);
        navigatorBuilder.addMethod(MethodSpec.methodBuilder("modify").addModifiers(Modifier.PUBLIC).addParameter(functionType, "f").addParameter(sourceTypeVar, "source").returns(sourceTypeVar).addStatement("return delegate.modify(f, source)").addJavadoc("Modifies the focused value if present.\n\n" + "@param f the transformation function\n" + "@param source the source structure\n" + "@return a new structure with the modified value, or original if not focused").build());
        // matches(S source) -> boolean
        navigatorBuilder.addMethod(MethodSpec.methodBuilder("matches").addModifiers(Modifier.PUBLIC).addParameter(sourceTypeVar, "source").returns(TypeName.BOOLEAN).addStatement("return delegate.matches(source)").addJavadoc("Checks if this path focuses on a value in the given source.\n\n" + "@param source the source structure to test\n" + "@return true if a value is focused, false otherwise").build());
        // toPath() -> AffinePath<S, A>
        navigatorBuilder.addMethod(MethodSpec.methodBuilder("toPath").addModifiers(Modifier.PUBLIC).returns(delegateType).addStatement("return delegate").addJavadoc("Returns the underlying AffinePath.\n\n" + "@return the wrapped AffinePath").build());
    }

    /**
     * Adds delegate methods for TraversalPath navigators.
     */
    private void addTraversalPathDelegateMethods(TypeSpec.Builder navigatorBuilder, TypeVariableName sourceTypeVar, TypeName targetTypeName, ParameterizedTypeName delegateType) {
        // getAll(S source) -> List<A>
        ParameterizedTypeName listType = ParameterizedTypeName.get(ClassName.get(List.class), targetTypeName);
        navigatorBuilder.addMethod(MethodSpec.methodBuilder("getAll").addModifiers(Modifier.PUBLIC).addParameter(sourceTypeVar, "source").returns(listType).addStatement("return delegate.getAll(source)").addJavadoc("Extracts all focused values from the source.\n\n" + "@param source the source structure\n" + "@return list of all focused values (may be empty)").build());
        // setAll(A value, S source) -> S
        navigatorBuilder.addMethod(MethodSpec.methodBuilder("setAll").addModifiers(Modifier.PUBLIC).addParameter(targetTypeName, "value").addParameter(sourceTypeVar, "source").returns(sourceTypeVar).addStatement("return delegate.setAll(value, source)").addJavadoc("Creates a new source with all focused values replaced.\n\n" + "@param value the new value for all focused elements\n" + "@param source the source structure\n" + "@return a new structure with all focused values updated").build());
        // modifyAll(Function<A, A> f, S source) -> S
        ParameterizedTypeName functionType = ParameterizedTypeName.get(ClassName.get(Function.class), targetTypeName, targetTypeName);
        navigatorBuilder.addMethod(MethodSpec.methodBuilder("modifyAll").addModifiers(Modifier.PUBLIC).addParameter(functionType, "f").addParameter(sourceTypeVar, "source").returns(sourceTypeVar).addStatement("return delegate.modifyAll(f, source)").addJavadoc("Creates a new source with all focused values transformed.\n\n" + "@param f the transformation function\n" + "@param source the source structure\n" + "@return a new structure with all focused values modified").build());
        // count(S source) -> int
        navigatorBuilder.addMethod(MethodSpec.methodBuilder("count").addModifiers(Modifier.PUBLIC).addParameter(sourceTypeVar, "source").returns(TypeName.INT).addStatement("return delegate.count(source)").addJavadoc("Counts the number of focused elements.\n\n" + "@param source the source structure\n" + "@return the number of focused elements").build());
        // isEmpty(S source) -> boolean
        navigatorBuilder.addMethod(MethodSpec.methodBuilder("isEmpty").addModifiers(Modifier.PUBLIC).addParameter(sourceTypeVar, "source").returns(TypeName.BOOLEAN).addStatement("return delegate.isEmpty(source)").addJavadoc("Checks if the traversal focuses on no elements.\n\n" + "@param source the source structure\n" + "@return true if no elements are focused").build());
        // toPath() -> TraversalPath<S, A>
        navigatorBuilder.addMethod(MethodSpec.methodBuilder("toPath").addModifiers(Modifier.PUBLIC).returns(delegateType).addStatement("return delegate").addJavadoc("Returns the underlying TraversalPath.\n\n" + "@return the wrapped TraversalPath").build());
    }
    ...
    private static Set<String> getDelegateMethodNames(PathKind pathKind) {
        return switch(pathKind) {
            case FOCUS ->
                Set.of("get", "set", "modify", "toLens", "toPath");
            case AFFINE ->
                Set.of("getOptional", "set", "modify", "matches", "toPath");
            case TRAVERSAL ->
                Set.of("getAll", "setAll", "modifyAll", "count", "isEmpty", "toPath");
        };
    }
     ...
     */
    private void addNavigationMethods(TypeSpec.Builder navigatorBuilder, TypeElement targetRecord, TypeVariableName sourceTypeVar, int currentDepth, PathKind currentPathKind) {
        ...
        String targetPackage = processingEnv.getElementUtils().getPackageOf(targetRecord).getQualifiedName().toString();
        ...
        for (RecordComponentElement component : components) {
            ...
            if (delegateNames.contains(fieldName)) {
                processingEnv.getMessager().printMessage(Diagnostic.Kind.NOTE, "Navigator field '" + fieldName + "' in " + targetRecord.getSimpleName() + " collides with a delegate method name. " + "Use .toPath().via(" + targetFocusClassName + "." + fieldName + "().toLens()) as a workaround.", component);
                continue;
            }
            TypeMirror fieldType = component.asType();
            ...
            ParameterizedTypeName returnType = ParameterizedTypeName.get(pathClass, sourceTypeVar, fieldTypeName);
            String pathDescription = switch(widenedKind) {
                case FOCUS ->
                    "FocusPath";
                case AFFINE ->
                    "AffinePath";
            ...
            MethodSpec.Builder methodBuilder = MethodSpec.methodBuilder(fieldName).addModifiers(Modifier.PUBLIC).returns(returnType).addJavadoc("Navigates to the {@code $L} field.\n\n" + "@return a $L focusing on the {@code $L} field", fieldName, pathDescription, fieldName);
            if (fieldKind != PathKind.FOCUS) {
                ...
                generateWidenedNavigationMethod(methodBuilder, component, targetRecord, fieldName, fieldType, fieldKind, currentPathKind, widenedKind, sourceTypeVar, targetPackage, targetFocusClassName, currentDepth);
            } else {
                ...
                final CodeBlock viaStatement = buildViaStatement(currentPathKind, widenedKind, fieldKind, targetFocusClass, fieldName);
                ...
                if (currentDepth < maxDepth && isNavigableType(fieldType)) {
                    ...
                    if (fieldTypeElement != null) {
                        ...
                        ClassName navigatorClass = ClassName.get(targetPackage, targetFocusClassName, fieldNavigatorClassName);
                        ParameterizedTypeName navigatorType = ParameterizedTypeName.get(navigatorClass, sourceTypeVar);
                        methodBuilder.returns(navigatorType);
                        methodBuilder.addStatement("return new $T.$L<>($L)", targetFocusClass, fieldNavigatorClassName, viaStatement);
                    } else {
     ...
     */
    private void generateWidenedNavigationMethod(MethodSpec.Builder methodBuilder, RecordComponentElement component, TypeElement targetRecord, String fieldName, TypeMirror fieldType, PathKind fieldKind, PathKind currentPathKind, PathKind widenedKind, TypeVariableName sourceTypeVar, String targetPackage, String targetFocusClassName, int currentDepth) {
        TypeName targetRecordTypeName = TypeName.get(targetRecord.asType());
        // Build constructor args for the inline Lens setter lambda
        String constructorArgs = targetRecord.getRecordComponents().stream().map(c -> c.getSimpleName().toString().equals(fieldName) ? "newValue" : "source." + c.getSimpleName() + "()").collect(Collectors.joining(", "));
        ...
        ClassName pathClass = getPathClassName(widenedKind);
        ParameterizedTypeName innerReturnType = ParameterizedTypeName.get(pathClass, sourceTypeVar, innerTypeName);
        ...
        if (fieldKind == PathKind.TRAVERSAL && !isHardcodedWideningType(fieldType)) {
            ...
            if (currentDepth < maxDepth && innerTypeElement != null && innerTypeElement.getAnnotation(GenerateFocus.class) != null) {
                wrapInNavigator = true;
                String innerNavigatorClassName = capitalise(fieldName) + "Navigator";
                navigatorFromTargetFocus = ClassName.get(targetPackage, targetFocusClassName, innerNavigatorClassName);
            }
        }
        // Build the statement: delegate.via(Lens.of(Record::field, setter)).widen().kindWiden()
        // Optionally wrapped in: new Navigator<>(...)
        if (wrapInNavigator) {
            List<Object> allArgs = new ArrayList<>(List.of(navigatorFromTargetFocus, Lens.class, targetRecordTypeName, fieldName, targetRecordTypeName, constructorArgs));
            allArgs.addAll(wideningArgs);
            methodBuilder.returns(ParameterizedTypeName.get(navigatorFromTargetFocus, sourceTypeVar));
            methodBuilder.addStatement("return new $T<>(delegate.via($T.of($T::$L, (source, newValue) -> new $T($L)))" + wideningExpr + ")", allArgs.toArray());
        } else {
            List<Object> allArgs = new ArrayList<>(List.of(Lens.class, targetRecordTypeName, fieldName, targetRecordTypeName, constructorArgs));
            allArgs.addAll(wideningArgs);
            methodBuilder.addStatement("return delegate.via($T.of($T::$L, (source, newValue) -> new $T($L)))" + wideningExpr, allArgs.toArray());
        }
    }
     ...
     * a different package.
     */
    private CodeBlock buildViaStatement(PathKind currentKind, PathKind widenedKind, PathKind fieldKind, ClassName targetFocusClass, String fieldName) {
        ...
        for (TraversableGenerator generator : traversableGenerators) {
            if (generator.supports(type)) {
                if (matched != null && matched.priority() == generator.priority()) {
                    processingEnv.getMessager().printMessage(Diagnostic.Kind.WARNING, "Multiple TraversableGenerator SPI providers with equal priority (" + generator.priority() + ") support type " + type + ": " + matched.getClass().getName() + " and " + generator.getClass().getName() + ". Using the first match.");
     ...
     */
    public MethodSpec createNavigatorMethod(RecordComponentElement component, TypeElement recordElement, List<? extends RecordComponentElement> allComponents, TypeName recordTypeName) {
        throw new UnsupportedOperationException("STUB: not implemented");
    }
     ...
     */
    private String buildPathWidening(RecordComponentElement component, TypeMirror fieldType, PathKind fieldKind, List<Object> wideningArgs) {
        if (fieldKind == PathKind.FOCUS) {
            return "";
        }
        ...
        for (TraversableGenerator generator : traversableGenerators) {
            if (generator.supports(fieldType)) {
                ...
                if (!opticExpr.isEmpty()) {
                    String resolvedExpr = OpticExpressionResolver.resolve(opticExpr, generator.getRequiredImports(), wideningArgs);
