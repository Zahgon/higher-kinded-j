...
public class FocusProcessor extends AbstractProcessor {
    ...
    private void generateFocusFile(TypeElement recordElement, Set<String> navigableTypes) throws IOException {
        ...
        String navigatorNote = generateNavigators ? "\n\n<p>This class includes navigator classes for fluent cross-type navigation." : "";
        TypeSpec.Builder focusClassBuilder = TypeSpec.classBuilder(focusClassName).addAnnotation(generatedAnnotation).addJavadoc("Generated Focus DSL paths for {@link $T}.\n\n" + "<p>This class provides type-safe navigation paths for accessing and modifying\n" + "fields within {@code $L} instances using the Focus DSL.$L\n\n" + "<p>Do not edit this file; it is automatically generated.", ClassName.get(recordElement), recordName, navigatorNote).addModifiers(Modifier.PUBLIC, Modifier.FINAL).addMethod(MethodSpec.constructorBuilder().addModifiers(Modifier.PRIVATE).build());
        ...
        if (generateNavigators) {
            navigatorGenerator = new NavigatorClassGenerator(processingEnv, navigableTypes, maxNavigatorDepth, includeFields, excludeFields, traversableGenerators);
        }
        ...
        for (RecordComponentElement component : components) {
            ...
            if (navigatorGenerator != null) {
                method = navigatorGenerator.createNavigatorMethod(component, recordElement, components, recordTypeName);
            }
            // Fall back to standard FocusPath method if no navigator method was created
            if (method == null) {
                method = createFocusPathMethod(component, recordElement, components, recordTypeName, widenCollections);
            }
        ...
        JavaFile javaFile = JavaFile.builder(packageName, focusClassBuilder.build()).addFileComment("Generated by hkj-optics-processor. Do not edit.").build();
    ...
    private MethodSpec createFocusPathMethod(RecordComponentElement component, TypeElement recordElement, List<? extends RecordComponentElement> allComponents, TypeName recordTypeName, boolean widenCollections) {
        ...
        TypeName innerTypeName = pathTypeInfo.innerType != null ? pathTypeInfo.innerType : componentTypeName.box();
        ParameterizedTypeName returnTypeName = ParameterizedTypeName.get(pathClass, recordTypeName, innerTypeName);
        String pathDescription = getPathDescription(pathTypeInfo);
        String getMethodName = getPathGetMethod(pathTypeInfo);
        MethodSpec.Builder methodBuilder = MethodSpec.methodBuilder(componentName).addJavadoc("Creates a {@link $T} for the {@code $L} field of a {@link $T}.\n\n" + "<p>The returned path can be composed with other optics for deep navigation:\n" + "<pre>{@code\n" + "$L.$L().$L(instance);  // Get the $L value\n" + "$L.$L().set(newValue, instance);  // Set the $L value\n" + "$L.$L().modify(fn, instance);  // Transform the $L value\n" + "}</pre>\n\n" + "@return A non-null {@code $L<$T, $T>}.", pathClass, componentName, recordTypeName, recordElement.getSimpleName() + "Focus", componentName, getMethodName, componentName, recordElement.getSimpleName() + "Focus", componentName, componentName, recordElement.getSimpleName() + "Focus", componentName, componentName, pathDescription, recordTypeName, innerTypeName).addModifiers(Modifier.PUBLIC, Modifier.STATIC).returns(returnTypeName);
        ...
        String constructorArgs = allComponents.stream().map(c -> c.getSimpleName().toString().equals(componentName) ? "newValue" : "source." + c.getSimpleName() + "()").collect(Collectors.joining(", "));
        // Generate code based on path type widening
        String baseLens = String.format("$T.of($T.of($T::%s, (source, newValue) -> new $T(%s)))", componentName, constructorArgs);
        switch(pathTypeInfo.wideningType) {
            case OPTIONAL ->
                methodBuilder.addStatement("return " + baseLens + ".some()", FOCUS_PATH_CLASS, Lens.class, recordTypeName, recordTypeName);
            case COLLECTION ->
                methodBuilder.addStatement("return " + baseLens + ".each()", FOCUS_PATH_CLASS, Lens.class, recordTypeName, recordTypeName);
            case NULLABLE ->
                methodBuilder.addStatement("return " + baseLens + ".nullable()", FOCUS_PATH_CLASS, Lens.class, recordTypeName, recordTypeName);
            case KIND_EXACTLY_ONE ->
                {
                    ...
                    methodBuilder.addStatement("return " + baseLens + traverseOverCall + ".headOption()", FOCUS_PATH_CLASS, Lens.class, recordTypeName, recordTypeName);
                }
            ...
            case KIND_ZERO_OR_MORE ->
                {
                    ...
                    KindFieldInfo kindInfo = pathTypeInfo.kindInfo();
                    String traverseOverCall = buildTraverseOverCall(kindInfo);
                    methodBuilder.addStatement("return " + baseLens + traverseOverCall, FOCUS_PATH_CLASS, Lens.class, recordTypeName, recordTypeName);
                }
            case SPI_ZERO_OR_ONE ->
                {
                    ...
                    List<Object> args = new ArrayList<>(List.of(FOCUS_PATH_CLASS, Lens.class, recordTypeName, recordTypeName));
                    String resolvedExpr = OpticExpressionResolver.resolve(opticExpr, gen.getRequiredImports(), args);
                    methodBuilder.addStatement("return " + baseLens + ".some(" + resolvedExpr + ")", args.toArray());
                }
            case SPI_ZERO_OR_MORE ->
                {
                    ...
                    List<Object> args = new ArrayList<>(List.of(FOCUS_PATH_CLASS, Lens.class, recordTypeName, recordTypeName));
                    String resolvedExpr = OpticExpressionResolver.resolve(opticExpr, gen.getRequiredImports(), args);
                    methodBuilder.addStatement("return " + baseLens + ".each(" + resolvedExpr + ")", args.toArray());
                }
            case NESTED ->
                {
            ...
            default ->
                methodBuilder.addStatement("return " + baseLens, FOCUS_PATH_CLASS, Lens.class, recordTypeName, recordTypeName);
        }
    ...
    private String buildTraverseOverCall(KindFieldInfo kindInfo) {
        ...
        return String.format(".<%s, %s>traverseOver(%s)", witnessType, elementType, kindInfo.traverseExpression());
    }
    ...
    private String buildWideningChainExpression(List<WideningStep> chain, List<Object> args) {
        ...
        for (int i = 0; i < chain.size(); i++) {
            ...
            boolean nextStepNeedsTypeWitness = i + 1 < chain.size() && isParameterisedStep(chain.get(i + 1));
            switch(step.type()) {
                case OPTIONAL ->
                    {
    ...
    private static boolean isParameterisedStep(WideningStep step) {
        return step.type() == WideningType.SPI_ZERO_OR_ONE || step.type() == WideningType.SPI_ZERO_OR_MORE;
    }
     ...
     */
    private record WideningStep(WideningType type, KindFieldInfo kindInfo, TraversableGenerator spiGenerator, TypeMirror innerTypeMirror) {
    }
     ...
     */
    private record PathTypeInfo(ClassName pathClass, TypeName innerType, WideningType wideningType, List<WideningStep> wideningChain, KindFieldInfo kindInfo, TraversableGenerator spiGenerator) {
         ...
         */
        static PathTypeInfo forSpi(ClassName pathClass, TypeName innerType, WideningType wideningType, TraversableGenerator generator) {
            throw new UnsupportedOperationException("STUB: not implemented");
        }
         ...
         */
        static PathTypeInfo forNested(ClassName pathClass, TypeName innerType, List<WideningStep> chain) {
            throw new UnsupportedOperationException("STUB: not implemented");
        }
    }
     ...
     */
    private PathTypeInfo analyseFieldType(RecordComponentElement component, TypeMirror type, boolean widenCollections) {
        ...
        if (OPTIONAL_TYPES.contains(qualifiedName)) {
            ...
            Optional<PathTypeInfo> nested = analyseNestedContainer(new WideningStep(WideningType.OPTIONAL, null, null, innerTypeMirror), innerTypeMirror, widenCollections);
            if (nested.isPresent()) {
                return nested.get();
            }
        ...
        if (COLLECTION_TYPES.contains(qualifiedName)) {
            TypeMirror innerTypeMirror = getFirstTypeArgument(declaredType);
            Optional<PathTypeInfo> nested = analyseNestedContainer(new WideningStep(WideningType.COLLECTION, null, null, innerTypeMirror), innerTypeMirror, widenCollections);
            if (nested.isPresent()) {
                return nested.get();
            }
            TypeName innerType = extractTypeArgument(declaredType);
        ...
        if (kindInfo.isPresent()) {
            ...
            ClassName pathClass = switch(info.semantics()) {
                case EXACTLY_ONE, ZERO_OR_ONE ->
                    AFFINE_PATH_CLASS;
                case ZERO_OR_MORE ->
        ...
        if (matchedGenerator != null && matchedGenerator.getCardinality() == Cardinality.ZERO_OR_ONE) {
            ...
            Optional<PathTypeInfo> nested = analyseNestedContainer(new WideningStep(WideningType.SPI_ZERO_OR_ONE, null, matchedGenerator, innerTypeMirror), innerTypeMirror, widenCollections);
            if (nested.isPresent()) {
                return nested.get();
            }
            TypeName innerType = extractTypeArgumentAt(declaredType, typeArgIndex);
            return PathTypeInfo.forSpi(AFFINE_PATH_CLASS, innerType, WideningType.SPI_ZERO_OR_ONE, matchedGenerator);
        }
        // When widenCollections is enabled, also widen ZERO_OR_MORE SPI types to TraversalPath
        if (widenCollections && matchedGenerator != null && matchedGenerator.getCardinality() == Cardinality.ZERO_OR_MORE) {
            int typeArgIndex = matchedGenerator.getFocusTypeArgumentIndex();
            TypeMirror innerTypeMirror = getTypeArgumentAt(declaredType, typeArgIndex);
            Optional<PathTypeInfo> nested = analyseNestedContainer(new WideningStep(WideningType.SPI_ZERO_OR_MORE, null, matchedGenerator, innerTypeMirror), innerTypeMirror, widenCollections);
            if (nested.isPresent()) {
                return nested.get();
            }
            TypeName innerType = extractTypeArgumentAt(declaredType, typeArgIndex);
            return PathTypeInfo.forSpi(TRAVERSAL_PATH_CLASS, innerType, WideningType.SPI_ZERO_OR_MORE, matchedGenerator);
        }
        ...
        for (TraversableGenerator generator : traversableGenerators) {
            if (generator.supports(type)) {
                if (matchedGenerator != null && matchedGenerator.priority() == generator.priority()) {
                    if (component != null) {
                        processingEnv.getMessager().printMessage(Diagnostic.Kind.WARNING, "Multiple TraversableGenerator SPI providers with equal priority (" + generator.priority() + ") support type " + type + ": " + matchedGenerator.getClass().getName() + " and " + generator.getClass().getName() + ". Using the first match.", component);
                    }
     ...
     */
    private Optional<PathTypeInfo> analyseNestedContainer(WideningStep outerStep, TypeMirror innerTypeMirror, boolean widenCollections) {
     ...
     */
    private List<WideningStep> analyseNestedType(TypeMirror type, boolean widenCollections, int depth) {
    ...
    private ClassName computeComposedPathClass(List<WideningStep> chain) {
        ...
        for (WideningStep step : chain) {
            switch(step.type()) {
                case COLLECTION, KIND_ZERO_OR_MORE, SPI_ZERO_OR_MORE ->
                    hasTraversal = true;
        ...
        if (hasTraversal)
            return TRAVERSAL_PATH_CLASS;
        if (hasAffine)
            return AFFINE_PATH_CLASS;
    ...
    private String getPathDescription(PathTypeInfo info) {
        if (info.wideningType == WideningType.NESTED) {
            ...
            if (pathClass.equals(TRAVERSAL_PATH_CLASS))
                return "TraversalPath";
            if (pathClass.equals(AFFINE_PATH_CLASS))
                return "AffinePath";
            return "FocusPath";
        }
