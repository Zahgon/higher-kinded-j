...
public class SpecInterfaceGenerator {
     ...
     */
    private MethodSpec generateOpticMethod(OpticMethodInfo opticMethod, TypeMirror sourceType, TypeElement sourceTypeElement, String className) {
        ...
        MethodSpec.Builder methodBuilder = MethodSpec.methodBuilder(methodName).addModifiers(Modifier.PUBLIC, Modifier.STATIC).returns(returnType).addJavadoc("Creates a {@link $T} for the {@code $L} property of a {@link $T}.\n\n" + "@return A non-null optic instance.", getOpticClass(opticKind), methodName, TypeName.get(sourceType));
        ...
        if (opticKind == OpticKind.TRAVERSAL && opticMethod.traversalHint() == SpecAnalysis.TraversalHintKind.THROUGH_FIELD) {
            methodBuilder.addAnnotation(AnnotationSpec.builder(SuppressWarnings.class).addMember("value", "$S", "unchecked").build());
        }
     ...
     */
    private CodeBlock generateOpticBody(OpticMethodInfo opticMethod, TypeMirror sourceType, TypeMirror focusType, String className) {
        return switch(opticMethod.opticKind()) {
            case LENS ->
                generateLensBody(opticMethod, sourceType, focusType);
            case PRISM ->
                generatePrismBody(opticMethod, sourceType, focusType);
            case TRAVERSAL ->
                generateTraversalBody(opticMethod, sourceType, focusType, className);
            case AFFINE, ISO, GETTER, FOLD ->
                // These would need additional implementation
                CodeBlock.of("throw new $T(\"$L optics are not yet supported in spec interfaces\");\n", UnsupportedOperationException.class, opticMethod.opticKind());
        };
    }
     ...
     */
    private CodeBlock generateLensBody(OpticMethodInfo opticMethod, TypeMirror sourceType, TypeMirror focusType) {
        String fieldName = opticMethod.methodName();
        CopyStrategyInfo copyStrategyInfo = opticMethod.copyStrategyInfo();
        CodeBlock getterLambda = copyStrategyGenerator.generateGetterLambda(fieldName, copyStrategyInfo, sourceType);
        CodeBlock setterLambda = copyStrategyGenerator.generateSetterLambda(opticMethod.copyStrategy(), copyStrategyInfo, fieldName, sourceType, focusType);
        return CodeBlock.builder().add("return $T.of(\n", Lens.class).indent().add("$L,\n", getterLambda).add("$L", setterLambda).unindent().add(");\n").build();
    }
     ...
     * @return the code block
     */
    private CodeBlock generatePrismBody(OpticMethodInfo opticMethod, TypeMirror sourceType, TypeMirror focusType) {
        return prismGenerator.generatePrismReturnStatement(opticMethod.prismHint(), opticMethod.prismHintInfo(), sourceType, focusType);
    }
     ...
     * @return the code block
     */
    private CodeBlock generateTraversalBody(OpticMethodInfo opticMethod, TypeMirror sourceType, TypeMirror focusType, String className) {
        return traversalGenerator.generateTraversalReturnStatement(opticMethod.traversalHint(), opticMethod.traversalHintInfo(), sourceType, focusType, className);
    }
    ...
    private MethodSpec copyDefaultMethod(ExecutableElement defaultMethod, TypeMirror sourceType) {
        ...
        MethodSpec.Builder methodBuilder = MethodSpec.methodBuilder(methodName).addModifiers(Modifier.PUBLIC, Modifier.STATIC).returns(TypeName.get(defaultMethod.getReturnType())).addJavadoc("Copied from spec interface default method.\n\n" + "@return The optic instance.");
        ...
        defaultMethod.getParameters().forEach(param -> methodBuilder.addParameter(TypeName.get(param.asType()), param.getSimpleName().toString()));
     ...
     */
    private TypeName buildOpticReturnType(OpticKind opticKind, TypeMirror sourceType, TypeMirror focusType) {
    ...
    private ClassName getOpticClass(OpticKind opticKind) {
        return switch(opticKind) {
            case LENS ->
                ClassName.get(Lens.class);
            case PRISM ->
                ClassName.get(Prism.class);
            case TRAVERSAL ->
                ClassName.get(Traversal.class);
            case AFFINE ->
                ClassName.get("org.higherkindedj.optics", "Affine");
            case ISO ->
                ClassName.get("org.higherkindedj.optics", "Iso");
            case GETTER ->
                ClassName.get("org.higherkindedj.optics", "Getter");
            case FOLD ->
                ClassName.get("org.higherkindedj.optics", "Fold");
        };
    }
