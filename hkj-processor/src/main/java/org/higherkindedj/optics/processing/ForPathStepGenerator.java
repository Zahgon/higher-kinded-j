...
final class ForPathStepGenerator {
    ...
    private static final String[] TYPE_PARAMS = { "A", "B", "C", "D", "E", "F", "G", "H", "I", "J", "K", "L" };
    ...
    private static final String[] EITHER_VALUE_PARAMS = { "A", "B", "C", "D", "F", "G", "H", "I", "J", "K", "L", "N" };

    // Generic has extra F, so skip F: A, B, C, D, E, G, H, I, J, K, L, M
    private static final String[] GENERIC_VALUE_PARAMS = { "A", "B", "C", "D", "E", "G", "H", "I", "J", "K", "L", "M" };
    ...
    // =========================================================================
    private record PathTypeDescriptor(// e.g. "MaybePath"
    String pathTypeName, // e.g. "MaybePathSteps"
    String stepsPrefix, // e.g. "MaybeKind.Witness"
    String witnessType, // e.g. "MaybeMonad"
    String monadType, // e.g. "MaybeMonad.INSTANCE" or "EitherMonad.instance()"
    String monadAccess, // true if monad is static field
    boolean isStaticMonad, // e.g. "MaybeKindHelper.MAYBE"
    String kindHelperField, // e.g. "Path.maybe" -- will be combined with narrow/runKind
    String pathFactoryExpr, // true for Maybe, Optional, NonDet
    boolean filterable, // true for Either (E), Generic (F), and Free (F)
    boolean hasExtraTypeParam, // "E" for Either, "F" for Generic/Free, null otherwise
    String extraTypeParamName, // null for Either (unbounded), bound for Generic/Free
    String extraTypeParamBound, // true only for Generic (uses runKind, instance monad)
    boolean isGeneric, // true only for NonDet (uses NonDetPath.of)
    boolean isNonDet, // true only for Free (uses runKind, instance monad + functor)
    boolean isFree, // current hand-written max arity
    int currentMaxArity) {
    }

    private static final PathTypeDescriptor[] PATH_TYPES = { new PathTypeDescriptor("MaybePath", "MaybePathSteps", "MaybeKind.Witness", "MaybeMonad", "MaybeMonad.INSTANCE", true, "MaybeKindHelper.MAYBE", "Path.maybe", true, false, null, null, false, false, false, 1), new PathTypeDescriptor("OptionalPath", "OptionalPathSteps", "OptionalKind.Witness", "OptionalMonad", "OptionalMonad.INSTANCE", true, "OptionalKindHelper.OPTIONAL", "Path.optional", true, false, null, null, false, false, false, 1), new PathTypeDescriptor("EitherPath", "EitherPathSteps", "EitherKind.Witness<E>", "EitherMonad", "EitherMonad.instance()", false, "EitherKindHelper.EITHER", "Path.either", false, true, "E", null, false, false, false, 1), new PathTypeDescriptor("TryPath", "TryPathSteps", "TryKind.Witness", "TryMonad", "TryMonad.INSTANCE", true, "TryKindHelper.TRY", "Path.tryPath", false, false, null, null, false, false, false, 1), new PathTypeDescriptor("IOPath", "IOPathSteps", "IOKind.Witness", "IOMonad", "IOMonad.INSTANCE", true, "IOKindHelper.IO_OP", "Path.ioPath", false, false, null, null, false, false, false, 1), new PathTypeDescriptor("VTaskPath", "VTaskPathSteps", "VTaskKind.Witness", "VTaskMonad", "VTaskMonad.INSTANCE", true, "VTaskKindHelper.VTASK", "Path.vtaskPath", false, false, null, null, false, false, false, 1), new PathTypeDescriptor("IdPath", "IdPathSteps", "IdKind.Witness", "IdMonad", "IdMonad.instance()", false, "IdKindHelper.ID", "Path.idPath", false, false, null, null, false, false, false, 1), new PathTypeDescriptor("NonDetPath", "NonDetPathSteps", "ListKind.Witness", "ListMonad", "ListMonad.INSTANCE", true, "ListKindHelper.LIST", "NonDetPath.of", true, false, null, null, false, true, false, 1), new PathTypeDescriptor("FreePath", "FreePathSteps", "FreeKind.Witness<F>", "FreeMonad", null, false, "FreeKindHelper.FREE", "FreePath.of", false, true, "F", "F extends WitnessArity<TypeArity.Unary>", false, false, true, 1), new PathTypeDescriptor("GenericPath", "GenericPathSteps", "F", "Monad", null, false, null, "GenericPath.of", false, true, "F", "F extends WitnessArity<TypeArity.Unary>", true, false, false, 1) };

    // =========================================================================
    // Entry point
    // =========================================================================
    static void generate(int minArity, int maxArity, ProcessingEnvironment processingEnv) throws IOException {
    ...
    // =========================================================================
    private static void generatePathSteps(PathTypeDescriptor desc, int n, boolean terminal, ProcessingEnvironment processingEnv) throws IOException {
        ...
        // Class javadoc
        sb.append("/** Step ").append(n).append(" in a ").append(desc.pathTypeName).append(" comprehension. */\n");
        sb.append("@Generated\n");
        // Class declaration
    ...
    // Imports
    // =========================================================================
    private static void appendImports(StringBuilder sb, PathTypeDescriptor desc, int n, boolean terminal) {
        ...
        // par() can target up to n+4, max 12
        int maxTuple = Math.min(n + 4, 12);
    ...
    private static void appendFields(StringBuilder sb, PathTypeDescriptor desc, int n) {
        if (desc.isFree) {
            ...
            sb.append("  private static final ").append(desc.monadType).append(" MONAD = ").append(desc.monadAccess).append(";\n");
        } else {
            // Id: static final from factory method
            sb.append("  private static final ").append(desc.monadType).append(" MONAD = ").append(desc.monadAccess).append(";\n");
        }
        sb.append("  private final Kind<").append(desc.witnessType).append(", Tuple").append(n).append("<");
    ...
    // Constructor
    // =========================================================================
    private static void appendConstructor(StringBuilder sb, PathTypeDescriptor desc, String className, int n) {
    ...
    private static void appendFromBody(StringBuilder sb, PathTypeDescriptor desc, int n, int next, String nextType, String monadVar) {
    ...
    // =========================================================================
    private static void appendWhenMethod(StringBuilder sb, PathTypeDescriptor desc, String className, int n) {
        ...
        sb.append("            t -> predicate.test(t) ? ").append(m).append(".of(t) : ").append(m).append(".zero(),\n");
        sb.append("            computation);\n");
    ...
    private static void appendMatchMethod(StringBuilder sb, PathTypeDescriptor desc, int n) {
        ...
        sb.append("                matcher\n");
        sb.append("                    .apply(t)\n");
        sb.append("                    .map(").append(nextType.toLowerCase()).append(" -> ").append(m).append(".of(Tuple.of(");
        for (int i = 0; i < n; i++) {
            sb.append("t._").append(i + 1).append("(), ");
        }
        sb.append(nextType.toLowerCase()).append(")))\n");
     ...
     */
    private static void appendParMethods(StringBuilder sb, PathTypeDescriptor desc, int n, boolean terminal) {
     ...
     * targetArity.
     */
    private static void appendParMethod(StringBuilder sb, PathTypeDescriptor desc, int n, int k, int targetArity) {
        String[] vp = valueParams(desc);
        ...
        sb.append("    Kind<").append(desc.witnessType).append(", Tuple").append(targetArity).append("<");
    ...
    private static void appendTraverseMethods(StringBuilder sb, PathTypeDescriptor desc, int n) {
        ...
        String nextClassName = desc.stepsPrefix + next;
        // traverse()
        sb.append("  public <TT extends WitnessArity<TypeArity.Unary>, TC, TR> ").append(nextClassName).append("<");
        ...
        sb.append("tb),\n");
        sb.append("                    traversable.traverse(").append(m).append(", f, extractor.apply(t))),\n");
        sb.append("            computation);\n");
        sb.append("    return new ").append(nextClassName).append("<>(");
        appendConstructorArgs(sb, desc);
        sb.append("newComp);\n");
        sb.append("  }\n\n");
        // sequence()
        sb.append("  public <TT extends WitnessArity<TypeArity.Unary>, TR> ").append(nextClassName).append("<");
        if (desc.isGeneric || desc.isFree) {
            sb.append("F");
            for (int i = 0; i < n; i++) {
                sb.append(", ").append(vp[i]);
        ...
        sb.append("tb),\n");
        sb.append("                    traversable.sequenceA(").append(m2).append(", extractor.apply(t))),\n");
        sb.append("            computation);\n");
        sb.append("    return new ").append(nextClassName).append("<>(");
        appendConstructorArgs(sb, desc);
        sb.append("newComp);\n");
        ...
        appendValueTypeParams(sb, desc, n);
        sb.append(">, Kind<TT, TC>> extractor,\n");
        sb.append("      Function<TC, Kind<").append(desc.witnessType).append(", Kind<TT, TR>>> f) {\n");
        sb.append("    Objects.requireNonNull(traversable, \"traversable must not be null\");\n");
        ...
        sb.append("            t -> {\n");
        sb.append("              Kind<").append(desc.witnessType).append(", Kind<TT, Kind<TT, TR>>> traversed =\n");
        sb.append("                  traversable.traverse(").append(m3).append(", f, extractor.apply(t));\n");
    ...
    private static void appendYieldTuple(StringBuilder sb, PathTypeDescriptor desc, int n) {
        String returnType = yieldReturnType(desc);
        sb.append("  public <R> ").append(returnType).append(" yield(Function<Tuple").append(n).append("<");
        appendValueTypeParams(sb, desc, n);
        sb.append(">, R> f) {\n");
        String m = monadVar(desc, sb);
        sb.append("    Kind<").append(desc.witnessType).append(", R> result =\n");
        sb.append("        ").append(m).append(".map(\n");
        sb.append("            t -> Objects.requireNonNull(f.apply(t), \"").append(YIELD_NULL_MSG).append("\"),\n");
        sb.append("            computation);\n");
        appendReturnPath(sb, desc);
        sb.append("  }\n");
    }
    ...
    private static void appendReturnPath(StringBuilder sb, PathTypeDescriptor desc) {
        if (desc.isFree) {
            ...
            sb.append("    return NonDetPath.of(").append(desc.kindHelperField).append(".narrow(result));\n");
        } else {
            sb.append("    return ").append(desc.pathFactoryExpr).append("(").append(desc.kindHelperField).append(".narrow(result));\n");
        }
    }
