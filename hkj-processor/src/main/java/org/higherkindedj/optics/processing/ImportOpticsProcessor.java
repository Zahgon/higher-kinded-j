...
public class ImportOpticsProcessor extends AbstractProcessor {
    ...
    private void processSpecInterface(TypeElement specInterface) {
        ...
        if (targetPackage.isEmpty()) {
            targetPackage = processingEnv.getElementUtils().getPackageOf(specInterface).getQualifiedName().toString();
        }
        note("Target package: " + targetPackage, specInterface);
        SpecInterfaceAnalyser analyser = new SpecInterfaceAnalyser(processingEnv.getTypeUtils(), processingEnv.getElementUtils(), processingEnv.getMessager());
        ...
        SpecInterfaceGenerator generator = new SpecInterfaceGenerator(processingEnv.getFiler(), processingEnv.getMessager());
    ...
    private void processTypeAnnotation(TypeElement typeElement) {
        ...
        String targetPackage = annotation.targetPackage();
        if (targetPackage.isEmpty()) {
            targetPackage = processingEnv.getElementUtils().getPackageOf(typeElement).getQualifiedName().toString();
        }
        boolean allowMutable = annotation.allowMutable();
    ...
    private void processType(TypeElement typeElement, String targetPackage, boolean allowMutable, Element sourceElement) {
        TypeKindAnalyser typeAnalyser = new TypeKindAnalyser(processingEnv.getTypeUtils());
        ExternalLensGenerator lensGenerator = new ExternalLensGenerator(processingEnv.getFiler(), processingEnv.getMessager());
        ExternalPrismGenerator prismGenerator = new ExternalPrismGenerator(processingEnv.getFiler(), processingEnv.getMessager());
        TypeAnalysis analysis = typeAnalyser.analyseType(typeElement);
        switch(analysis.typeKind()) {
            case RECORD ->
                lensGenerator.generateForRecord(analysis, targetPackage);
            case SEALED_INTERFACE ->
                prismGenerator.generateForSealedInterface(analysis, targetPackage);
            case ENUM ->
                prismGenerator.generateForEnum(analysis, targetPackage);
            case WITHER_CLASS ->
                {
                    if (analysis.hasMutableFields() && !allowMutable) {
                        error("Type '" + typeElement.getQualifiedName() + "' has mutable fields (setters). " + "Lens laws may not hold for mutable types. " + "Either use allowMutable = true to acknowledge this limitation, " + "or create a spec interface for explicit control.", sourceElement);
                        return;
                    }
            ...
            case UNSUPPORTED ->
                {
                    if (analysis.hasMutableFields()) {
                        error("Type '" + typeElement.getQualifiedName() + "' is a mutable class without wither methods. " + "Cannot generate lenses for types that don't support immutable updates. " + "Consider using a spec interface to define custom copy logic.", sourceElement);
                    } else {
                        error("Type '" + typeElement.getQualifiedName() + "' is not a record, sealed interface, enum, or class with wither methods. " + "Cannot determine how to generate optics for this type.", sourceElement);
                    }
                }
        }
    }
    ...
    private List<TypeElement> getClassArrayFromAnnotation(Element element) {
        ...
        for (AnnotationMirror mirror : element.getAnnotationMirrors()) {
            ...
            if (annotationElement.getQualifiedName().contentEquals("org.higherkindedj.optics.annotations.ImportOptics")) {
        ...
        for (Map.Entry<? extends ExecutableElement, ? extends AnnotationValue> entry : importOpticsMirror.getElementValues().entrySet()) {
