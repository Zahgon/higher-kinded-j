...
public class PathProcessor extends AbstractProcessor {
    ...
    private static final ClassName MAYBE_PATH = ClassName.get("org.higherkindedj.hkt.effect", "MaybePath");

    private static final ClassName EITHER_PATH = ClassName.get("org.higherkindedj.hkt.effect", "EitherPath");

    private static final ClassName TRY_PATH = ClassName.get("org.higherkindedj.hkt.effect", "TryPath");

    private static final ClassName VALIDATION_PATH = ClassName.get("org.higherkindedj.hkt.effect", "ValidationPath");

    private static final ClassName OPTIONAL_PATH = ClassName.get("org.higherkindedj.hkt.effect", "OptionalPath");
    ...
    private void generateBridgeClass(TypeElement interfaceElement) throws IOException {
        ...
        String defaultPackage = processingEnv.getElementUtils().getPackageOf(interfaceElement).getQualifiedName().toString();
        ...
        TypeSpec.Builder classBuilder = TypeSpec.classBuilder(bridgeClassName).addAnnotation(GENERATED).addJavadoc("Generated Path bridge for {@link $T}.\n\n<p>Do not edit.\n", interfaceClassName).addModifiers(Modifier.PUBLIC, Modifier.FINAL);
        // Add delegate field
        classBuilder.addField(FieldSpec.builder(interfaceClassName, "delegate", Modifier.PRIVATE, Modifier.FINAL).build());
        // Add constructor
        classBuilder.addMethod(MethodSpec.constructorBuilder().addModifiers(Modifier.PUBLIC).addParameter(interfaceClassName, "delegate").addJavadoc("Creates a new Path bridge wrapping the given delegate.\n\n" + "@param delegate the service to wrap; must not be null\n").addStatement("this.delegate = $T.requireNonNull(delegate, $S)", OBJECTS, "delegate must not be null").build());
        ...
        // Write the file
        JavaFile javaFile = JavaFile.builder(packageName, classBuilder.build()).addFileComment("Generated by PathProcessor. Do not edit.").build();
        javaFile.writeTo(processingEnv.getFiler());
    }

    private MethodSpec createBridgeMethod(ExecutableElement method, PathVia pathVia) {
        String methodName = pathVia.name().isEmpty() ? method.getSimpleName().toString() : pathVia.name();
        String doc = pathVia.doc();
        TypeMirror returnType = method.getReturnType();
        PathTypeMapping mapping = determinePathType(returnType);
        if (mapping == null) {
            error("Unsupported return type for @PathVia: " + returnType + ". Supported types: Optional, Maybe, Either, Try, Validated, IO.", method);
            return null;
        }
        MethodSpec.Builder methodBuilder = MethodSpec.methodBuilder(methodName).addModifiers(Modifier.PUBLIC).returns(mapping.pathType());
        // Add documentation
        if (!doc.isEmpty()) {
            methodBuilder.addJavadoc("$L\n\n", doc);
        }
        methodBuilder.addJavadoc("@return Path-wrapped result from {@link $L#$L}\n", method.getEnclosingElement().getSimpleName(), method.getSimpleName());
        // Copy parameters
        ...
        if (mapping.requiresSemigroup()) {
            methodBuilder.addParameter(ParameterizedTypeName.get(SEMIGROUP, mapping.errorType()), "semigroup");
            paramNames.add("semigroup");
        }
        // Build the method body
        String delegateCall = "delegate." + method.getSimpleName() + "(" + String.join(", ", paramNames.subList(0, method.getParameters().size())) + ")";
        if (mapping.requiresSemigroup()) {
            methodBuilder.addStatement("return $T.$L($L, semigroup)", PATH, mapping.factoryMethod(), delegateCall);
        } else {
    ...
    private PathTypeMapping determinePathType(TypeMirror returnType) {
        ...
        String qualifiedName = typeElement instanceof TypeElement te ? te.getQualifiedName().toString() : typeName;
        List<? extends TypeMirror> typeArgs = declaredType.getTypeArguments();
        return switch(qualifiedName) {
            case "java.util.Optional" ->
                new PathTypeMapping(ParameterizedTypeName.get(OPTIONAL_PATH, typeArgOrObject(typeArgs, 0)), "optional", false, null);
            case "org.higherkindedj.hkt.maybe.Maybe" ->
                new PathTypeMapping(ParameterizedTypeName.get(MAYBE_PATH, typeArgOrObject(typeArgs, 0)), "maybe", false, null);
            case "org.higherkindedj.hkt.either.Either" ->
                {
                    ...
                    yield new PathTypeMapping(ParameterizedTypeName.get(EITHER_PATH, errorType, valueType), "either", false, null);
                }
            case "org.higherkindedj.hkt.trymonad.Try" ->
                new PathTypeMapping(ParameterizedTypeName.get(TRY_PATH, typeArgOrObject(typeArgs, 0)), "tryPath", false, null);
            case "org.higherkindedj.hkt.validated.Validated" ->
                {
                    TypeName errorType = typeArgOrObject(typeArgs, 0);
                    TypeName valueType = typeArgOrObject(typeArgs, 1);
                    yield new PathTypeMapping(ParameterizedTypeName.get(VALIDATION_PATH, errorType, valueType), "validated", true, errorType);
                }
            case "org.higherkindedj.hkt.io.IO" ->
                new PathTypeMapping(ParameterizedTypeName.get(IO_PATH, typeArgOrObject(typeArgs, 0)), "io", false, null);
            default ->
                null;
        };
    }
    ...
    private static TypeName typeArgOrObject(List<? extends TypeMirror> typeArgs, int index) {
        return index < typeArgs.size() ? TypeName.get(typeArgs.get(index)) : ClassName.get(Object.class);
    }
     ...
     */
    private record PathTypeMapping(TypeName pathType, String factoryMethod, boolean requiresSemigroup, TypeName errorType) {
    }
}
