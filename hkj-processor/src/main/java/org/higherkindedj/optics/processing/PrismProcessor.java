...
public class PrismProcessor extends AbstractProcessor {
    ...
    private void generatePrismsFile(TypeElement sumTypeElement) throws IOException {
        ...
        String defaultPackage = processingEnv.getElementUtils().getPackageOf(sumTypeElement).getQualifiedName().toString();
        ...
        TypeSpec.Builder prismsClassBuilder = TypeSpec.classBuilder(prismsClassName).// Add the @Generated annotation to the class
        addAnnotation(generatedAnnotation).addJavadoc("Generated optics for {@link $T}. Do not edit.", ClassName.get(sumTypeElement)).addModifiers(Modifier.PUBLIC, Modifier.FINAL).addMethod(MethodSpec.constructorBuilder().addModifiers(Modifier.PRIVATE).build());
        ...
        if (sumTypeElement.getModifiers().contains(Modifier.SEALED)) {
            for (var permittedSubclass : sumTypeElement.getPermittedSubclasses()) {
                var subtypeElement = (TypeElement) processingEnv.getTypeUtils().asElement(permittedSubclass);
                prismsClassBuilder.addMethod(createPrismMethodForSubtype(sumTypeElement, subtypeElement));
            }
        } else // Handle Enums
        if (sumTypeElement.getKind() == ElementKind.ENUM) {
            List<VariableElement> enumConstants = sumTypeElement.getEnclosedElements().stream().filter(e -> e.getKind() == ElementKind.ENUM_CONSTANT).map(VariableElement.class::cast).collect(Collectors.toList());
            for (VariableElement enumConstant : enumConstants) {
                prismsClassBuilder.addMethod(createPrismMethodForEnum(sumTypeElement, enumConstant));
            }
        }
        JavaFile.builder(packageName, prismsClassBuilder.build()).addFileComment("Generated by hkj-optics-processor. Do not edit.").build().writeTo(processingEnv.getFiler());
    }
    ...
    private MethodSpec createPrismMethodForEnum(TypeElement enumType, VariableElement enumConstant) {
        ...
        return MethodSpec.methodBuilder(methodName).addJavadoc("Creates a {@link $T} that focuses on the {@code $L} constant of the {@link $T}" + " enum.\n\n" + "@return A non-null {@code Prism<$T, $T>}.", Prism.class, enumConstant.getSimpleName(), enumClassName, enumClassName, enumClassName).addModifiers(Modifier.PUBLIC, Modifier.STATIC).returns(prismTypeName).addStatement("return $T.of(source -> source == $T.$L ? $T.of(source) : $T.empty(), value -> value)", Prism.class, enumClassName, enumConstant.getSimpleName(), Optional.class, Optional.class).build();
    }
