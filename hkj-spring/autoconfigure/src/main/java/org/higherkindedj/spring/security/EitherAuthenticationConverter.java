...
public class EitherAuthenticationConverter implements Converter<Jwt, AbstractAuthenticationToken> {
    ...
    private Either<AuthConversionError, Collection<GrantedAuthority>> extractAuthorities(Jwt jwt) {
        try {
            ...
            if (claim == null) {
                return Either.left(new AuthConversionError("Missing authorities claim: " + authoritiesClaimName));
            }
            if (claim instanceof Collection<?> claimCollection) {
                List<GrantedAuthority> authorities = claimCollection.stream().filter(item -> item instanceof String).map(item -> (String) item).map(role -> new SimpleGrantedAuthority(authorityPrefix + role)).collect(Collectors.toList());
                return Either.right(authorities);
            }
            return Either.left(new AuthConversionError("Invalid authorities claim type: " + claim.getClass().getName()));
        } catch (Exception e) {
            return Either.left(new AuthConversionError("Error extracting authorities: " + e.getMessage()));
        }
    }
     ...
     */
    private AbstractAuthenticationToken createAuthenticationToken(Jwt jwt, Collection<GrantedAuthority> authorities) {
